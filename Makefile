...
unit-unlocked: unit-rollcall preflight $(ROOT)/build/test_cadence $(ROOT)/build/test_avclock $(ROOT)/build/test_frame_ledger $(ROOT)/build/test_mraudio_status $(ROOT)/build/test_av_bimodal_latch $(ROOT)/build/test_osd_menu $(ROOT)/build/test_osd_control $(ROOT)/build/test_last_frame_latch $(ROOT)/build/test_playback_overlay $(ROOT)/build/test_input_mailbox $(ROOT)/build/test_pixel_format $(ROOT)/build/test_main_guard $(ROOT)/build/test_death_breadcrumb $(ROOT)/build/test_crash_guard_death $(ROOT)/build/test_status_telemetry $(ROOT)/build/test_resolve $(ROOT)/build/test_log_redact $(ROOT)/build/test_pms_timeline $(ROOT)/build/test_plextv_device $(ROOT)/build/test_companion_eof $(ROOT)/build/test_companion_plant_seek $(ROOT)/build/test_gdm_resources_parity $(ROOT)/build/pms_baseline_probe $(ROOT)/build/test_h264_bitstream_source $(ROOT)/build/test_bitstream_ring_lifecycle $(ROOT)/build/test_frame_store_math $(ROOT)/build/test_coded_size_adopt $(ROOT)/build/test_ffmpeg_vf $(ROOT)/build/test_frame_store_sdram_sim $(ROOT)/build/test_frame_store_ddr_prefetch_sim $(ROOT)/build/test_ddr_want_y_hblank_thrash $(ROOT)/build/test_ddr_bank_mailbox_phys $(ROOT)/build/test_core_identity $(ROOT)/build/test_ddr_playback_bank_select $(ROOT)/build/test_ddr_scanout_multiframe $(ROOT)/build/test_sdram_memtest_sim $(ROOT)/build/test_sdram_mailbox $(ROOT)/build/test_annexb_count $(ROOT)/build/test_sps_parse $(ROOT)/build/test_slice_hdr $(ROOT)/build/test_cavlc_dc $(ROOT)/build/test_idct_quant $(ROOT)/build/test_p3_host_recon_vectors $(ROOT)/build/test_p3_idct_reference_model $(ROOT)/build/test_p3_inter_pred_vectors $(ROOT)/build/extract_h264_golden
	...
	$(ROOT)/build/test_crash_guard_death
	bash $(ROOT)/tests/unit/test_daemon_exit_rc0.sh
...
$(ROOT)/build/test_core_identity: $(ROOT)/tests/unit/test_core_identity.cpp \
		$(ROOT)/host/libmisterplex/core_identity.hpp \
		$(ROOT)/host/libmisterplex/mailbox_abi_spec.hpp \
		$(ROOT)/host/libmisterplex/input_mailbox.hpp \
		$(ROOT)/host/libmisterplex/ddr_frame_layout.hpp \
		$(ROOT)/assets/core_identity_stamp.json
	@mkdir -p $(ROOT)/build
	$(CXX) $(CXXFLAGS) -I$(ROOT)/host -o $@ $(ROOT)/tests/unit/test_core_identity.cpp
...
$(ROOT)/build/test_crash_guard_death: $(ROOT)/tests/unit/test_crash_guard_death.cpp \
		$(ROOT)/arm/misterplexd/fpga_spi.cpp $(ROOT)/arm/misterplexd/fpga_spi.hpp \
		$(ROOT)/arm/misterplexd/death_breadcrumb.cpp $(ROOT)/arm/misterplexd/death_breadcrumb.hpp
	@mkdir -p $(ROOT)/build
	$(CXX) $(CXXFLAGS) -I$(ROOT)/arm/misterplexd -pthread -o $@ \
		$(ROOT)/tests/unit/test_crash_guard_death.cpp $(ROOT)/arm/misterplexd/fpga_spi.cpp \
		$(ROOT)/arm/misterplexd/death_breadcrumb.cpp
...
$(ROOT)/build/test_av_bimodal_latch: $(ROOT)/tests/unit/test_av_bimodal_latch.cpp \
		$(ROOT)/host/libmisterplex/av_bimodal_latch.hpp \
		$(ROOT)/host/libmisterplex/mraudio_status.hpp
	@mkdir -p $(ROOT)/build
	$(CXX) $(CXXFLAGS) -I$(ROOT)/host -o $@ $(ROOT)/tests/unit/test_av_bimodal_latch.cpp
