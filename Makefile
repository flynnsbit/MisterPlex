...
unit-unlocked: unit-rollcall preflight $(ROOT)/build/test_cadence $(ROOT)/build/test_avclock $(ROOT)/build/test_mraudio_status $(ROOT)/build/test_osd_menu $(ROOT)/build/test_last_frame_latch $(ROOT)/build/test_playback_overlay $(ROOT)/build/test_input_mailbox $(ROOT)/build/test_pixel_format $(ROOT)/build/test_main_guard $(ROOT)/build/test_death_breadcrumb $(ROOT)/build/test_status_telemetry $(ROOT)/build/test_resolve $(ROOT)/build/test_log_redact $(ROOT)/build/test_pms_timeline $(ROOT)/build/test_companion_eof $(ROOT)/build/test_companion_plant_seek $(ROOT)/build/pms_baseline_probe $(ROOT)/build/test_h264_bitstream_source $(ROOT)/build/test_bitstream_ring_lifecycle $(ROOT)/build/test_frame_store_math $(ROOT)/build/test_coded_size_adopt $(ROOT)/build/test_frame_store_sdram_sim $(ROOT)/build/test_frame_store_ddr_prefetch_sim $(ROOT)/build/test_sdram_memtest_sim $(ROOT)/build/test_sdram_mailbox $(ROOT)/build/test_annexb_count $(ROOT)/build/test_sps_parse $(ROOT)/build/test_slice_hdr $(ROOT)/build/test_cavlc_dc $(ROOT)/build/test_idct_quant $(ROOT)/build/test_p3_host_recon_vectors $(ROOT)/build/test_hybrid_compose_host $(ROOT)/build/test_plxo_post_copy $(ROOT)/build/test_present_stall $(ROOT)/build/test_p3_idct_reference_model $(ROOT)/build/test_p3_inter_pred_vectors $(ROOT)/build/extract_h264_golden
	...
	$(ROOT)/build/test_hybrid_compose_host
	$(ROOT)/build/test_plxo_post_copy
	$(ROOT)/build/test_present_stall
...
$(ROOT)/build/test_hybrid_compose_host: $(ROOT)/tests/unit/test_hybrid_compose_host.cpp \
		$(ROOT)/host/libmisterplex/hybrid_compose.hpp
	@mkdir -p $(ROOT)/build
	$(CXX) $(CXXFLAGS) -o $@ $(ROOT)/tests/unit/test_hybrid_compose_host.cpp

$(ROOT)/build/test_plxo_post_copy: $(ROOT)/tests/unit/test_plxo_post_copy.cpp \
		$(ROOT)/host/libmisterplex/mailbox_abi_spec.hpp
	@mkdir -p $(ROOT)/build
	$(CXX) $(CXXFLAGS) -o $@ $(ROOT)/tests/unit/test_plxo_post_copy.cpp

$(ROOT)/build/test_present_stall: $(ROOT)/tests/unit/test_present_stall.cpp \
		$(ROOT)/host/libmisterplex/present_stall.hpp
	@mkdir -p $(ROOT)/build
	$(CXX) $(CXXFLAGS) -o $@ $(ROOT)/tests/unit/test_present_stall.cpp
...
$(ROOT)/build/death_capture_supervisor: $(ROOT)/tools/death_capture_supervisor.c
	@mkdir -p $(ROOT)/build
	$(CC) -O2 -Wall -o $@ $<

$(ROOT)/build/death_capture_victim: $(ROOT)/tools/death_capture_victim.c
	@mkdir -p $(ROOT)/build
	$(CC) -O2 -Wall -o $@ $<
