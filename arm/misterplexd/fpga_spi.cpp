...
// Product Main path on DE10-Nano. Session suspend matches this EXACTLY via
// argv[0] OR resolved /proc/<pid>/exe (never substring — ERROR 14).
...
ProductMainLocate locateUniqueProductMain(pid_t* outPid) {
    ...
    if (pids.size() > 1) {
        std::fprintf(stderr, "main: SUSPEND_MAIN_DURING_PLAY refuse — %zu pids path=%s",
...
void FpgaSpi::resumeStrandedMain() {
    // Never fight SPI window or intentional playback suspend.
...
void FpgaSpi::installCrashGuard() {
    // SIGKILL: supervisor resume + startup resumeStrandedMain.
