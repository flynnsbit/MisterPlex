...
| P3-3p | P-slice inter prediction / motion compensation | **RTL PRIMITIVES + INTEGRATED STREAM_PATH SIM DONE / DIAG HARDWARE QUEUE READY** | W-REL: `docs/phase3-inter-rtl.md`; product RTL `h264_inter_pred.sv` listed in `files.qip`; Verilator `test_p3_inter_rtl_sim.sh` in `make unit` compares real RTL against `inter_mc_v1.json` (`mv_cases=6`, `partition_cases=10`, `frame_mv_cases=9090`) and red-checks bad interpolation rounding plus bad partition MV. New integrated gate `test_p3_inter_stream_path_rtl_sim.sh` elaborates product `stream_path.sv` + parsers + `decode_stub` + inter RTL, feeds paced multi-NAL fixtures (`nals=4 frames=1`; `nals=15 idr=1 p=11 frames=12`), proves the stub returns idle after each VCL with all MB1 MV/luma/chroma/fetch bands green, and red-checks a testbench-only bad diagnostic pixel fault. Visual default is trusted again only for its 160×120 ROI on rollback; this branch changes the MB1 inter witness, so hardware grading needs telemetry first plus direct/captured MB1 band evidence or a branch-specific golden. `decode_stub.sv` instantiates the inter block in the shipped artifact and paints top-row MB1 as four green/red bands (aggregate signature `0x57`, MV band includes a partition witness). Server-side PMS XML now delivers Baseline/L3.0: `profile_idc=66`, CAVLC, `max_num_ref_frames=1`, `i=6 p=294 b=0`, coded 624×480/display 618×480. Exact-flag x264 proxy with `partitions=none` measured P MBs: intra 4.8%, P16×16 17.6%, sub-MB 0%, skip 77.7%; RTL rung now supports P_Skip + partition-aware P_L0_16x16/P_L0_16x8/P_L0_8x16/P_8x8/sub-MB MV prediction primitives. Parsed P MB syntax is not yet driving MC; next step consumes shared P MB goldens/ref-store latency. One 624×480 YUV420 ref+current=898,560 B; +present=1,347,840 B; 25 fps YUV path ~40 MB/s before overhead (~50–70 MB/s planning), comfortable for 80 MHz DDR/YUV, not SDRAM. |
