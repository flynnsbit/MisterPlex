...
- (done **W-TIME `3065e00` FBAR harness fix**) distinct capture-failure diagnostic + dedicated exit **20**: `CAPTURE_FAILED ... reason=no_frame` / `FBAR_CAPTURE_FAILED ... exit=20`; forced-failure proof `FBAR_FORCED_NOCAPTURE_EXIT=20`. An instrument that could not distinguish "failed to measure" from "measured wrong" now can.
