...
| Full-frame intra prediction exactness | **UNSUBSTANTIATED / RETIRED TWICE** | The former “300/300 exact”/`maeY=0` frame-wide green was measured through the RGB565 diagnostic/presentation path with border masking. The later native-I420 ratchet (`624x480 12f` intra `510/1170`, Y MAE `17.765057`; `320x240 12f` intra `155/300`, Y MAE `6.050521`; wcap `207/300`) is also retired because its FFmpeg reference silently had in-loop deblocking enabled while RTL output did not. Current status waits for no-deblock (`-skip_loop_filter all`) native-I420 ratchets with loop-filter state declared/refused in provenance. |
