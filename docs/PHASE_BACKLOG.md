...
## ACTIVE — P3-PHASE1A-320 FPGA H.264 320×240 I/P (**IN_PROGRESS / glass rematch** 2026-08-24)

Pickup SoT: [`docs/PHASE1A_PICKUP.md`](PHASE1A_PICKUP.md) · lessons: [`docs/PHASE1A_320_FIT_LESSONS.md`](PHASE1A_320_FIT_LESSONS.md).
Branch `wip/phase1a-320-fit13-lessons`. **720p24 PARKED.** Do not thrash `07f54d9f`. Do not invent glass PASS.
...
## PARKED — P3-720P24 unique 1280×720 @ 24 fps (**PARKED / unique24 FAIL** 2026-08-24)

**Parked 2026-08-24:** CoS named Phase 1a 320 rematch as the living product. Do not chase 23.9. Do not thrash true480 `07f54d9f`. Historical harvest below is unchanged.

**Harvest 16:39Z:** unique24=**FAIL**. Exclusive **FREE**. **FIT_GO=NO**. **PLAY=NO** this tick. Living product `/media/fat/Plex.rbf` **`112bb651` SWAPVB**. Utility `_Utility/Plex.rbf` **`07f54d9f` UNTOUCHED** (not the living unique24 lab). INI `video_mode` **74250** 720p60 (HDMI24 reverted). Honest: ac95 **15.9/13.05** glassOK; **112bb 15.7/13.38** (`/tmp/pfps-720p24-112bb651.txt`) and **15.4/12.87**; HDMI24 **14.5/12.31 HURT CLOSED**. Isolated decode ~32; `copy_us` ~12ms; vfps=pfps; produce under play is limiter. **CLOSED:** doorbell PHYS, HDMI24 modeline, SWAPVB cousin, CATCH. Next product = produce-quiet design (FPGA line-prefetch vs ffmpeg HPS DDR fight). Parent has **NOT** named **FIT_GO**. **SKIP_RESTORE=YES**. Daemon do-not-kill. **PATH_SDRAM_PLUS_DDR** stays **DEFER**. **P4-DISPLAY / P4-720P-MIX** stay **TODO**. Soft-skip ≠ PASS. **15.7 ≠ 23.9**. **13.38 ≠ 23.9**. **15.4 ≠ 23.9**. **14.5 ≠ 23.9**. Cite `/tmp/misterplex-loop-status.txt` `/tmp/misterplex-agent-112bb-replay.txt` `/tmp/pfps-720p24-112bb651.txt` `/tmp/pfps-720p24-112bb651-hdmi24.txt`.

SoT for 720p24 (this dirty decode `main` is not the product tree). Case law: worktree `/home/shawn/Projects/MisterPlex-wt-480p-lessons` · branch `lessons/true480-720p24` · `docs/LESSONS.md` **L44–L56** · `docs/720p24-rbf.md`. **P3-TRUE480** v0.4.1 glass **`07f54d9f` stays**. Display raster / 720p mix hang: **P4-DISPLAY** / **P4-720P-MIX** (**TODO**; **RASTER-CMD IMPL_OK** + **RECIPE_OK** not lab-gated; **no HDMI**; **UNPLUGGED**; **RECIPE not run**; **MIX-CONTENT-BANK DESIGN_OK**; **CLOSED-STOP IMPL_OK**; **GLASS-MAX IMPL_OK** + **W-cli-glass IMPL_OK** CAP-only + **RETRY-GLASS-CAP IMPL_OK** + **W-unit-r UNIT_OK** not lab-gated; ARM **`6f1afebe` NOT_DEPLOYED**; **`b6bc183f`/`903400bc`/`b08aed9c` STALE**). Freddo MPEG2 **L56**: YUV-until-scan already ours; next is gather + A/V resync (`Memory/lab/status/FREDDO_YUV_LAST_RGB.md`). Idle L4 first-kick / missing chevron (do not re-derive): `Memory/lab/status/IDLE_L4_FIRST_KICK.md`. Live dual misterplexd (do **not** kill): `Memory/lab/status/DUAL_MISTERPLEXD_EBFE4A12.md`. Standalone PROBE_REAL flaky (diag 338-runs ≠ official pair; hugepage dead; idle **minSlots=1**): `Memory/lab/status/ARENA_ALLOC_FLAKY.md`.

| | |
|--|--|
| **Status** | **IN_PROGRESS** — unique **pfps FAIL** (gate ≥23.9). **Not DONE.** Soft-skip ≠ PASS. **24 Hz beam ≠ 24 unique fps.** Harvest **16:39Z:** unique24=**FAIL**. Exclusive **FREE**. **FIT_GO=NO**. **PLAY=NO**. Living lab **`112bb651` SWAPVB** scored FAIL (15.7/13.38 and 15.4/12.87; HDMI24 14.5/12.31 HURT CLOSED). Utility **`07f54d9f` UNTOUCHED**. INI **74250** 720p60. Isolated decode ~32; `copy_us` ~12ms; vfps=pfps; produce is limiter. **CLOSED:** doorbell PHYS, HDMI24, SWAPVB cousin, CATCH. Next = produce-quiet (FPGA line-prefetch vs ffmpeg HPS DDR). Parent has **NOT** named **FIT_GO**. **SKIP_RESTORE=YES**. Daemon do-not-kill. **PATH_SDRAM_PLUS_DDR DEFER**. **P4 stay TODO**. Soft-skip ≠ PASS. Cite `/tmp/misterplex-loop-status.txt` `/tmp/misterplex-agent-112bb-replay.txt` `/tmp/pfps-720p24-112bb651.txt` `/tmp/pfps-720p24-112bb651-hdmi24.txt`. **Chevron ≠ unique-24.** Harvest **22:42Z**: **W-retry-cap IMPL_OK** RETRY-GLASS-CAP (`main.cpp` **`28b158a4`**; retry=`glassMax(content)`; needle updated; **≠ mix PASS ≠ T7** — `/tmp/misterplex-agent-W-retry-cap.txt`). **W-unit-r UNIT_OK** MAKE_UNIT_RC=0 post-RETRY (~150s; supersedes **W-unit-q STALE**; farm **UP**; play-file **160×120** stays; **UNIT_OK ≠ T7 ≠ pfps** — `/tmp/misterplex-agent-W-unit-r.txt` + `/tmp/misterplex-unit-r.log`). **W-comp-pkg3 PKG_OK** ARM **`6f1afebe` NOT_DEPLOYED** (**`903400bc`/`b6bc183f`/`b08aed9c` STALE**; **PKG ≠ deploy ≠ T7**). **W-t7-ready READY_CARD** 8/8 (**PLAY_RAN=NO**; **READY_CARD ≠ T7 PASS**). **W-docs-r / W-backlog-r 22:34Z SUPERSEDED** for Next (said RETRY next; now landed). HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; **≠ FAIL ≠ PASS**). **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK+READY_CARD** **`95bff255`**. Lab **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016** do **not** kill. Next = T7 **PARENT** sidecar after reload **j** (no new binary); HDMI = **PARENT** after user plug + ask + deploy **`6f1afebe`**; **no Quartus**. Soft-skip ≠ PASS. **P4 stay TODO**. Cite lessons SoT + `/tmp/misterplex-agent-W-retry-cap.txt` + `/tmp/misterplex-agent-W-unit-r.txt` + `/tmp/misterplex-unit-r.log` + `/tmp/misterplex-agent-W-comp-pkg3.txt` + `/tmp/misterplex-agent-W-t7-ready.txt`. Harvest **22:34Z** (SUPERSEDED for Next): **W-cli-glass IMPL_OK** CAP-only (`present_bank.hpp` **`e3c429a7`**; targeted play-file **160×120** RC=0; **≠ mix PASS ≠ T7**). **W-unit-q UNIT_OK** MAKE_UNIT_RC=0 post-CAP (supersedes **W-unit-p FAIL** / **W-unit-n STALE** / **W-unit-o** farm-down; **UNIT_OK ≠ T7 ≠ pfps**). **W-hdmi-hold HOLD_OK** HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; missing video4 **≠ FAIL ≠ PASS**; **RECIPE not run**). **W-pkg-hold HOLD_OK** **`903400bc` STALE**; **W-comp-pkg3** rebuilding (report **ABSENT**; **not PKG_OK**; do **not** deploy). **W-docs-q / W-backlog-q 22:22Z SUPERSEDED** for Next. **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK** **`95bff255`**. Lab **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016** do **not** kill. Next was **RETRY-GLASS-CAP** + ARM pkg after CAP (**landed 22:42Z**); T7 = **PARENT** sidecar after reload **j**; HDMI = **PARENT** after user plug + ask + post-CAP rebuild + deploy. Soft-skip ≠ PASS. **P4 stay TODO**. Cite lessons SoT. Harvest **22:22Z**: **W-unit-p FAIL** MAKE_UNIT_RC=2 farm **UP** — CLI **160×120** snapped to **320×240** by `glassMax` + `contentResolutionFromSize` (not farm-down; **not UNIT_OK**). **W-comp-pkg2** **`903400bc`** now **STALE** vs CAP-only (**HOLD**; **PKG ≠ deploy**). **W-retry-note CARD_OK** **RETRY-GLASS-CAP**. **W-farm-note CARD_OK**. **W-docs-p / W-backlog-p 22:12Z SUPERSEDED** for Next. Later **22:27Z**: **W-cli-glass IMPL_OK** CAP-only (**≠ UNIT_OK ≠ mix PASS** — lessons `/tmp/misterplex-agent-W-cli-glass.txt`). HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; missing video4 **≠ FAIL ≠ PASS**; **no HDMI eyes-on** until user plugs). **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK** **`95bff255`**. Lab **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016** do **not** kill. Next was **PARENT** sidecar T7 after reload **j** (**W-unit-q UNIT_OK** MAKE_UNIT_RC=0 **≠ T7**); T7 = **PARENT** sidecar after reload **j**; HDMI = **PARENT** after user plug + ask + deploy. Soft-skip ≠ PASS. **P4 stay TODO**. Cite lessons SoT. Harvest **22:12Z**: **W-glass-max IMPL_OK** (`present_bank.hpp` `glassMax`; true480 720→640×480 doorbell **`0x300FF000`**; L4 stays **`0x3047F000`**; **not deployed**; **GLASS-MAX ≠ mix PASS ≠ T7**). **W-unit-o FAIL** MAKE_UNIT_RC=2 farm SSH rc=4 (then DOWN; parent re-pin **FARM_SSH=UP**; **not UNIT_OK**). **W-comp-pkg PKG_OK** ARM **`b6bc183f` STALE** vs GLASS-MAX (**≠ T7 ≠ HDMI PASS**; **not deployed**). **W-t7-book BOOK_OK** sidecar **PREP_OK+BOOK_OK** **no new binary** (`/tmp/t7-l4.conf` md5 **`95bff255`**; **BOOK_OK ≠ T7 PASS**). **W-docs-o / W-backlog-o 22:02Z SUPERSEDED** for Next (said GLASS-MAX next; now landed). **T7 FAIL MIX** pfps=**NONE**. Later **22:16Z**: **W-unit-p FAIL** MAKE_UNIT_RC=2 play-file decode size ≠ CLI 160×120 (`/tmp/misterplex-unit-p.log`; farm **UP**; **not UNIT_OK**; farm-down ≠ this FAIL). **W-comp-pkg2 PKG_OK** **`903400bc`** NOT_DEPLOYED (**PKG ≠ deploy**). Lab disk **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016** do **not** kill. Next = post-GLASS-MAX unit; T7 = **PARENT**; HDMI = **PARENT** after **`903400bc`** deploy. **P4 stay TODO**. Loop later **22:16Z** HDMI=**UNPLUGGED** (no grab; **≠ FAIL ≠ PASS**). Cite lessons SoT `/tmp/misterplex-agent-W-glass-max.txt` + `/tmp/misterplex-agent-W-unit-o.txt` + `/tmp/misterplex-agent-W-comp-pkg.txt` + `/tmp/misterplex-agent-W-comp-pkg2.txt` + `/tmp/misterplex-unit-p.log`. Harvest **22:02Z**: **T7 FAIL MIX** (pfps=NONE). Sidecar **PREP_OK** `/tmp/t7-l4.conf` md5 **`95bff255`**; sidecar retry **no new binary** (**W-review-p4 REVIEW_OK**). **W-closed-stop IMPL_OK** (**IMPL_OK ≠ mix PASS**). **W-hdmi-p4 RECIPE_OK** (**RECIPE_OK ≠ HDMI PASS**). **P4-DISPLAY** **TODO** (**RASTER-CMD IMPL_OK** + **RECIPE_OK**; not lab-gated). **P4-720P-MIX** **TODO** (**CLOSED-STOP IMPL_OK**; **MIX-GLASS-MAX** next — **landed 22:12Z**). **W-unit-n UNIT_OK STALE** vs CLOSED-STOP; **W-unit-o** running. Lab disk **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016** do **not** kill. Next was **MIX-GLASS-MAX** + post-CLOSED-STOP unit (**landed 22:12Z**). T7 = **PARENT** (sidecar no new binary). HDMI recipe = **PARENT** after companion deploy. Harvest **21:52Z**: **T7 FAIL MIX** (pfps=NONE). Sidecar **PREP_OK READY=YES** `/tmp/t7-l4.conf` md5 **`95bff255`** (**W-sidecar-prep**; **PREP_OK ≠ T7**). **CLI_DECODE_WINS HOST_OK** (**W-decode-wins**; not deployed; **HOST_OK ≠ T7**). **P4-DISPLAY** **TODO** (**RASTER-CMD IMPL_OK** not lab-gated; **W-raster-cmd**; **IMPL_OK ≠ HDMI PASS**). **P4-720P-MIX** **TODO** (**MIX-CONTENT-BANK DESIGN_OK**; **CLOSED-STOP** next; **W-720p-mix**). **W-unit-m UNIT_OK STALE** pending **W-unit-n**. Lab disk **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016** do **not** kill. Next was post-impl unit + **CLOSED-STOP** + HDMI recipe (**landed 22:02Z**); T7 was **PARENT** after reload **j**. Harvest **21:40Z**: **T7 FAIL** class=**MIX HOST** (**W-t7-rca**); **ACK_PATH=LIVE** (**W-plxj-rtl**; j PLXJ `@0x3047F130`); **HOST_DESIGN_OK T7_SIDECAR_L4_CONF** (**W-osd-720p** ≠ T7 PASS); **W-score-k** `968b828a` BUILD_OK STA floors PASS chevron PASS, **T7 FAIL**, native raster FAIL. Disk **`07f54d9f`** (21:31Z); j kept `Plex.720p24.968b828a.rbf`. **P4-DISPLAY** was **RASTER-CMD** design **TODO**. Next was implement **W-raster-cmd** + **W-decode-wins** (**landed 21:52Z**). Next T7 play was **PARENT** after sidecar + reload **j**. **FIT_GO=NO**. Exclusive **FREE**. Daemon **24016** do **not** kill. Prior **21:29Z**: lab **`968b828a`** (slot720p24j) CORE=Plex; HDMI chevron back; **T7 FAIL** PLXJ ACK timeout **0 frames**; **W-unit-j UNIT_OK ≠ T7**. |
| **Lab RBF** | Living unique24 lab `/media/fat/Plex.rbf` **`112bb651` SWAPVB scored FAIL** (16:39Z; 15.7/13.38 + 15.4/12.87; HDMI24 14.5/12.31 HURT CLOSED; **≠ 23.9**; **SKIP_RESTORE=YES**). Utility `_Utility/Plex.rbf` **`07f54d9f` UNTOUCHED** (true480 backup; **not** the living unique24 lab). Historical **j `968b828a` DEPLOY_OK** HDMI chevron **ORANGE_PX=11947** while it was `Plex.rbf` (W-j-score 21:24–21:27Z; card `Memory/lab/status/SLOT720P24J_WHAT_IT_GAVE.md`; `/tmp/plex-hdmi-eyes/j.png`). **21:31Z pin:** default `/media/fat/_Utility/Plex.rbf` = **`07f54d9f`** (user restore); j **kept** `Plex.720p24.968b828a.rbf` md5 **`968b828a8ec572bccf43b7bc2d31625a`** size **2937308**. **true480 backup stays** `07f54d9f`. **i `15bd5db2`** BUILD_OK TIMING_OK DEPLOY HDMI **BLACK** (superseded). Historical **h `ebfe4a12`** T5 EXIT=4 **DEPLOY_OK=NO** (W-proto-h3) superseded. **Chevron ≠ unique-24.** OSD still **640×480@59.9** under **1280×720@24** (**P4-DISPLAY** TODO). **T7 FAIL** cite `/tmp/misterplex-agent-W-t7-j.txt` (PLAY_RC=1; play did not run; pfps=NONE). Do not deploy **`17dd3b56`** / **`0bcc6081`**. |
| **pfps** | Living **`112bb651` unique24 FAIL** — **15.7/13.38** (`/tmp/pfps-720p24-112bb651.txt`; 16:34Z play-only F@118=YES presented=237 drops=0) and **15.4/12.87**; HDMI24 **14.5/12.31 HURT CLOSED**; ac95 **15.9/13.05** glassOK. Isolated decode ~32; `copy_us` ~12ms; vfps=pfps. **Do not invent ≥23.9.** Soft-skip ≠ PASS. Historical **T7 FAIL / not measured on j** — play-file 15s **0 frames**. `misterplexd.t7j.log`: **PLXJ ACK timeout want=16:9 token=1 last=4:3 token=0**. OSD at start **240p/240p decode=320x240**. **Do not invent ≥23.9.** Last measured unique **15.47** on e. Chevron **≠** unique-24. |
| **Dead** | Host knobs / present-policy / **PL330 `CH_RDWR_ERR`** / STREAM@24 / PLL 29.7 / luck redeploy **`17dd3b56`**. **Branch A STREAM exclusive is not first**. |
| **Loop** | Harvest **16:39Z:** unique24=**FAIL**. Exclusive **FREE**. **FIT_GO=NO**. **PLAY=NO**. Next product = produce-quiet (FPGA line-prefetch vs ffmpeg HPS DDR). Parent has **NOT** named **FIT_GO**. **SKIP_RESTORE=YES**. Living RBF **`112bb651`**. Utility **`07f54d9f` untouched**. **P4 stay TODO**. **PATH_SDRAM_PLUS_DDR DEFER**. Play loop **STOPPED**. Fleet **`720p24-j-chevron-unblocked`**. Exclusive **FREE**. **FIT_GO=NO**. Harvest **22:42Z**: **W-retry-cap IMPL_OK** RETRY-GLASS-CAP (`main.cpp` **`28b158a4`**; retry=`glassMax(content)`; needle updated; **≠ mix PASS ≠ T7** — `/tmp/misterplex-agent-W-retry-cap.txt`). **W-unit-r UNIT_OK** MAKE_UNIT_RC=0 post-RETRY (~150s; supersedes **W-unit-q STALE**; farm **UP**; play-file **160×120** stays; **UNIT_OK ≠ T7 ≠ pfps** — `/tmp/misterplex-agent-W-unit-r.txt` + `/tmp/misterplex-unit-r.log`). **W-comp-pkg3 PKG_OK** ARM **`6f1afebe` NOT_DEPLOYED** (**`903400bc`/`b6bc183f`/`b08aed9c` STALE**; **PKG ≠ deploy ≠ T7**). **W-t7-ready READY_CARD** 8/8 (**PLAY_RAN=NO**; **READY_CARD ≠ T7 PASS**). **W-docs-r / W-backlog-r 22:34Z SUPERSEDED** for Next (said RETRY next; now landed). HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; **≠ FAIL ≠ PASS**). **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK+READY_CARD** **`95bff255`**. Lab **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016** do **not** kill. Next = T7 **PARENT** sidecar after reload **j** (no new binary); HDMI = **PARENT** after user plug + ask + deploy **`6f1afebe`**; **no Quartus**. Soft-skip ≠ PASS. **P4 stay TODO**. Harvest **22:34Z** (SUPERSEDED for Next): **W-cli-glass IMPL_OK** CAP-only (**≠ mix PASS**). **W-unit-q UNIT_OK** MAKE_UNIT_RC=0 post-CAP (**≠ T7**). **W-hdmi-hold HOLD_OK** HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; **≠ FAIL ≠ PASS**). **W-pkg-hold HOLD_OK** **`903400bc` STALE**; **W-comp-pkg3** rebuilding (report **ABSENT**; do **not** deploy). **W-docs-q / W-backlog-q 22:22Z SUPERSEDED** for Next. **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK** **`95bff255`**. Disk **`07f54d9f`**. Daemon **24016** do **not** kill. Next was **RETRY-GLASS-CAP** + ARM pkg after CAP (**landed 22:42Z**); T7 = **PARENT**; HDMI = **PARENT** after user plug. Harvest **22:22Z**: **W-unit-p FAIL** RC=2 CLI **160×120**→**320×240** `glassMax`+`contentResolutionFromSize` farm **UP** (**not UNIT_OK**); **`903400bc`** now **STALE**; **RETRY-GLASS-CAP** named; **W-docs-p / W-backlog-p 22:12Z SUPERSEDED**; HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; **≠ FAIL ≠ PASS**; **no HDMI eyes-on** until user plugs). Later **22:27Z** **W-cli-glass IMPL_OK** CAP-only (**≠ UNIT_OK**). **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK** **`95bff255`**. Disk **`07f54d9f`**. Daemon **24016** do **not** kill. Next was **PARENT** sidecar T7 after reload **j** (**W-unit-q UNIT_OK** MAKE_UNIT_RC=0 **≠ T7**); T7 = **PARENT** sidecar after reload **j**; HDMI = **PARENT** after user plug + ask + deploy. Harvest **22:12Z**: **W-glass-max IMPL_OK**; **W-unit-o FAIL** MAKE_UNIT_RC=2 farm SSH rc=4 (parent re-pin **FARM_SSH=UP**); **W-comp-pkg PKG_OK** ARM **`b6bc183f` STALE** vs GLASS-MAX; **W-t7-book BOOK_OK** sidecar **PREP_OK+BOOK_OK** **`95bff255`**; **W-docs-o / W-backlog-o 22:02Z SUPERSEDED** for Next. Later **22:16Z**: **W-unit-p FAIL** RC=2 (CLI 160×120; farm **UP**; **not UNIT_OK**); **W-comp-pkg2 PKG_OK** **`903400bc`** NOT_DEPLOYED. Disk **`07f54d9f`**; j kept. Daemon **24016** do **not** kill. Next was post-GLASS-MAX unit (**named 22:22Z W-cli-glass + W-unit-q**); T7 = **PARENT**; HDMI = **PARENT** after **`903400bc`**. Loop later **22:16Z** HDMI=**UNPLUGGED** (no grab; **≠ FAIL ≠ PASS**). Harvest **22:02Z**: **W-closed-stop IMPL_OK**; **W-review-p4 REVIEW_OK** sidecar retry **no new binary**; **W-hdmi-p4 RECIPE_OK**; **W-unit-n UNIT_OK STALE**; **W-unit-o** running. Disk **`07f54d9f`**; j kept. Daemon **24016** do **not** kill. Next was **MIX-GLASS-MAX** + post-CLOSED-STOP unit (**landed 22:12Z**); T7 = **PARENT**; HDMI recipe = **PARENT** after companion deploy. Harvest **21:52Z**: **RASTER-CMD IMPL_OK** not lab-gated; **CLI_DECODE_WINS HOST_OK** not deployed; sidecar **PREP_OK**; **MIX-CONTENT-BANK DESIGN_OK**; **W-unit-m STALE** pending **W-unit-n**. Disk **`07f54d9f`**; j kept. Daemon **24016** do **not** kill. Next was post-impl unit + **CLOSED-STOP** + HDMI recipe (**landed 22:02Z**). T7 was **PARENT**. Harvest **21:40Z**: **ACK_PATH=LIVE** (**W-plxj-rtl**); **HOST_DESIGN_OK T7_SIDECAR_L4_CONF** (**W-osd-720p**); **W-score-k SCORE_OK** BUILD_OK+STA+chevron, **T7 FAIL**, native raster FAIL. Disk **`07f54d9f`**. Next was implement **W-raster-cmd** + **W-decode-wins** (**landed 21:52Z**). Next T7 play was **PARENT** after sidecar + reload **j**. Daemon **24016** do **not** kill. **P4-DISPLAY** stays **TODO** (**RASTER-CMD IMPL_OK** not lab-gated). **Chevron ≠ unique-24.** **DEPLOY_OK ≠ pfps PASS.** Prior **21:29Z**: **W-lab-plxd IDLE_SWAP=YES** fd=3 disp=1 src=**0x03c00000**; **T7 FAIL** cite `/tmp/misterplex-agent-W-t7-j.txt` + lab `misterplexd.t7j.log` **PLXJ ACK timeout 16:9 vs last 4:3** **0 frames** (play did not run; pfps=NONE). Daemon now **24016** (play worker bounced 22654 — do **not** kill). **W-unit-j UNIT_OK** MAKE_UNIT_RC=0 **≠ T7**. **P4-DISPLAY / P4-720P-MIX** stay **TODO** (OSD **640×480@59.9**). Next was PLXJ ACK RCA (host vs fabric vs raster) + P4-DISPLAY design; **no Quartus**; **no second menu**; **no second play**. **Chevron ≠ unique-24.** **IDLE_SWAP ≠ T7.** **DEPLOY_OK ≠ pfps PASS.** Prior **21:18Z**: lab **`968b828a` DEPLOY_OK** HDMI chevron **ORANGE_PX=11947**; T7 was RUNNING. Prior **20:53Z**: Exclusive **FREE**. **FIT_GO=consumed-h / NO**. Harvest **20:53Z**: **W-port-rev REV_OK**. **W-inv-port NEEDLE_OK**. **W-prefit HOLD_OK** `PORT_OK_NOT_FIT_GO.md` **FIT_GO** stays **NO**. **W-port-rtl PORT_OK** source-only (prior); live **ebfe4a12** still undriven; **FABRIC_PASS=NO**. **W-proto-h21 / W-docs-h20 / W-backlog-h11** 20:45Z **SUPERSEDED** for harvest/Next. Unchanged: exclusive **FREE**; **FIT_GO=consumed-h**; T5 **EXIT=4** **DEPLOY_OK=NO**; T6 MATCH **ebfe4a12**; T7 **NOT_RUN**; **13881** FABRIC=1 AUTHORIZED_ON do **not** bounce. **PORT_OK ≠ FABRIC_PASS.** **REV_OK ≠ FABRIC_PASS.** **NEEDLE_OK ≠ FABRIC_PASS.** **HOLD_OK ≠ READY_YES.** Next = named port-land freeze md5s + post-port make unit; **no Quartus**; **no bounce**; **no compact**; **no W-flag-off**. **PORT_OK ≠ FABRIC_PASS.** **REV_OK ≠ FABRIC_PASS.** **NEEDLE_OK ≠ FABRIC_PASS.** **HOLD_OK ≠ READY_YES.** Live **ebfe4a12** cannot swap. **T7 BLOCKED.** Prior **20:45Z**: **W-port-rtl PORT_OK** source-only clk_ddr ports; live **ebfe4a12** still undriven; **FABRIC_PASS=NO**. **W-10030-pin PIN_OK** Warning **10030** L956-957. **W-stale-gate CARD_OK**. **W-proto-h20 / W-docs-h19 / W-backlog-h10** 20:35Z **SUPERSEDED** for harvest/Next (said port land LIVE). Unchanged: exclusive **FREE**; **FIT_GO=consumed-h**; T5 **EXIT=4** **DEPLOY_OK=NO**; T6 MATCH **ebfe4a12**; T7 **NOT_RUN**; **UNIT_OK=YES** historical; **13881** FABRIC=1 AUTHORIZED_ON do **not** bounce. **PORT_OK ≠ FABRIC_PASS.** **PIN_OK ≠ FABRIC_PASS.** **CARD_OK ≠ FABRIC_PASS.** Next = review+needle PORT_OK + HOLD exclusive; **no Quartus**; **no bounce**; **no compact**; **no W-flag-off**. **PORT_OK ≠ FABRIC_PASS.** Live **ebfe4a12** cannot swap. **T7 BLOCKED.** Prior **20:35Z**: **W-swap-rca2 RCA_OK** undriven `fabric_copy_*` Warning **10030** vs db=**0xa0001733**; **fd=0**; **FABRIC_PASS=NO**. **W-token-ro TOKEN_OK** SOURCE=**MATCH** LIVE_LATCH=**UNKNOWN**. **W-match-card CARD_OK** MATCH **0x04100000** play_pair=**NO**. **W-proto-h19 / W-docs-h18 / W-backlog-h9** 20:27Z **SUPERSEDED** for harvest/Next. Next = clk_ddr port passthrough (**W-port-rtl**) then parent exclusive only; **no bounce**; **no compact**; **no W-flag-off**. Live **ebfe4a12** cannot swap. **T7 BLOCKED.** Prior **20:27Z**: **W-plxp-re2 POKE_LIVE** PLXP src=**0x04100000**; PLXD **fd=0**. **W-maps-13881 MAPS_OK** n338=1 legal **0x04100000 MATCH** poke/log; play_pair=NO. **W-twin-re2 TWIN_OK** 13881 FABRIC=1. Combined MATCH **≠ FABRIC_PASS**. **POKE_LIVE ≠ FABRIC_PASS.** **MAPS_MATCH ≠ T7.** Next = swap RCA **fd=0**; **no bounce**; **no compact**. Prior **20:17Z**: **W-unit-re6 UNIT_OK** MAKE_UNIT=0 (`/tmp/misterplex-agent-W-unit-re6.txt`). **W-bin-re BIN_RE=NEW** live **13881** exe **54ff33a0** == lessons; **10420/10411 DEAD** (not this worker) — do **not** kill 13881/13872. **W-probe-hold** `PROBE_COMPACT_HOLD.md`. Next = RO PLXP on 13881; **no bounce**; **no compact**. Prior **20:09Z**: **W-reresolve RERESOLVE_OK** sendDdrFrame always live-resolve; stale 0x01200000 not poked; CACHED=0 INV=0; **NOT deployed** (`/tmp/misterplex-agent-W-reresolve.txt`). **W-mlock-card** `MLOCK_PFN_NOT_PINNED.md` — do not compact_memory while 10420 holds slots. **W-deploy-hold HOLD_OK** `COMPANION_DEPLOY_HOLD.md` READY=NO. Next = `make unit` after reresolve; **no deploy**; **no bounce**; **no compact**. **RERESOLVE_OK ≠ FABRIC_PASS.** **HOLD_OK ≠ READY_YES.** Prior **20:01Z**: **W-slot-rca RCA_OK** poke **0x01200000** = frozen alloc-time **firstPfn<<12** (PFN 0x1200); **no** pageIndex store; paintIdle never re-walks (`/tmp/misterplex-agent-W-slot-rca.txt`). **W-slot-fix NOFIX** objdump 9fff2e76 phys=firstPfn<<12 virt=base+pageIndex*4k; do **not** bounce for a field-swap. **W-swap-card** `POKE_SEEN_WRONG_SRC.md`. Next = host **re-resolve** live PFN from `slot.virt` before poke; **no deploy**. Prior **19:49Z**: **W-bin-pin BIN_OK MATCH_WIRE** live misterplexd **9fff2e76** == lessons ARM. **W-daemon-maps MAPS_OK DAEMON_REAL** 10420 n338_legal=**2** first **0x0562a000** second **0x05a00000**; log slot **0x01200000 UNMAPPED**. **W-plxp-ro POKE_SEEN FABRIC_PASS=NO** PLXP src=**0x01200000** magic OK; PLXD **fd=0** frozen; PLXF live. **W-twin-upd** card restamp TWIN_GONE 10411/10420. Next = RCA why poke src is VA-offset **0x01200000** not proven PFN phys; **no bounce**. Prior **19:39Z**: **W-r1-probe2 pair REAL** slots **0x04500000/0x04100000** + **idle1 REAL** **0x03d00000** retries=4 binary `b11b251e` n=1 (**≠ daemon REAL ≠ T7**; `/tmp/misterplex-agent-W-r1-probe2.txt`). **W-unit-re4 UNIT_OK** post-both INV=0 CACHED=0 MAKE_UNIT=0. **W-twin-re TWIN_GONE** 4898/4918/watch DEAD; live **10411/10420 FABRIC=1** owns :3005 (do **not** kill); log `src_phys=REAL slot=0x01200000 how=arena` **≠ FABRIC_PASS**. Next = pin on-box bin + walk 10420 pagemap + RO PLXP; **no bounce**. Prior **19:31Z**: **W-inv-min1 INV_OK+UNIT_OK** live `113817fd` needle minSlots=1 (INV_PY=0 INV_SH=0 MAKE_UNIT=0; supersedes freeze-only UNIT_OK; `/tmp/misterplex-agent-W-inv-min1.txt`). **W-r1-retry RETRY_OK** `kFabricDirectArenaRetries=4` hdr `9c0277c2` TEST_RC=0 host-only (not on-box yet; `/tmp/misterplex-agent-W-r1-retry.txt`). **W-arena-rca RCA_OK** `Memory/lab/status/ARENA_ALLOC_FLAKY.md`. **W-proto-h12/W-docs-h11/W-backlog-h3** 19:23Z stale vs live UNIT_OK+retries. Next = on-box probe2 (minSlots=2 and idle 1); **no daemon bounce**; no W-flag-off. Prior **19:23Z**: **W-unit-arena UNIT_OK** freeze `c804acf9` INV=0 MAKE_UNIT=0 (**live tree INV_PY=1** after sibling minSlots=1 dropped `idleFabric_.real()`; do **not** harvest UNIT_OK onto `113817fd`; `/tmp/misterplex-agent-W-unit-arena.txt`). **W-r1-re STUB Repeatable=NO** — first-run PROBE_REAL **flaky** (diag arena32 n338_legal=3 then official remmap STUB; `/tmp/misterplex-agent-W-r1-re.txt`). **W-twin-card TWIN_CARD** 4898 :3005 NO_FABRIC + 4918 FABRIC=1 bind-fail STUB; `Memory/lab/status/DUAL_MISTERPLEXD_EBFE4A12.md`. **W-proto-h11/W-docs-h10/W-backlog-h2** 19:13Z stale vs flaky+INV_NOW. Next = retune INV for minSlots=1 + arena mmap retries; **no daemon bounce**; no W-flag-off. Prior **19:13Z**: **W-r1-probe REAL** standalone arena pair **0x03c00000/0x03800000** (hugepage EINVAL; memalign longest 32; 32 MiB arena; daemon **not** bounced; **≠ T7 ≠ FABRIC_PASS**; `/tmp/misterplex-agent-W-r1-probe.txt`). **W-idle-maps MAPS_OK** live daemon max_contig=**1** LEGAL_338=**0** still **STUB** (twin 4898 :3005 NO_FABRIC + 4918 FABRIC=1 bind-fail still STUB; do **not** kill). **W-idle-arena ARENA_OK** paintIdle → `idleFabric_` host-wire; INV/MAKE **UNVERIFIED**. **W-proto-h10/W-docs-h9/W-backlog-h** 18:59Z stamps (src_phys STUB — **stale vs probe REAL**). **UNIT_OK ≠ pfps.** **PROBE_REAL ≠ pfps.** Next = unit-verify idle wire + probe reconfirm; **no daemon bounce**; no W-flag-off. Prior **18:59Z**: **W-inv-paint INV_OK+UNIT_OK** MAKE_UNIT_RC=0 paintIdle 6-arg needle (supersedes **W-unit-re2 MAKE_UNIT_RC=2**; `/tmp/misterplex-agent-W-inv-paint.txt`). **W-flag-off2 FLAG_OFF** daemon `fabric_direct=0` (prior MUST_OFF invert; **this tick parent AUTHORIZED ON** ebfe4a12-only — **do not spawn W-flag-off**; **do not kill** current env). **W-pages-r1 PAGES_OK** WithOff helper hdr **33e8f842** ≠ isolated **e61988e7** **NO MERGE** ≠ REAL. **W-geom-re GEOM_OK** GEOM=0 L4=0. **W-proto-h9/W-docs-h8** stale vs UNIT_OK+FLAG_OFF. **UNIT_OK ≠ pfps PASS.** Next = on-device REAL src_phys / idle arena (standalone probe; no daemon bounce; no W-flag-off). Prior **18:47Z**: **W-unit-re2 MAKE_UNIT_RC=2** (rtl_invariants paintIdle 6-arg; lint-run UNIT_OK **STALE**). **W-kick-make MAKEFILE_OK**. **W-r1-heap HEAP_OK** hdr 17c17fdf ≠ isolated e61988e7. **W-idle-mem IDLE_CARD**. **W-fabric-h** companion+flag ON **without REAL** → pokePlxp never / PLXD fd=0 **FABRIC_PASS=NO**. **W-proto-h8/W-docs-h7** stale vs RC=2. Prior **18:39Z**: **W-lint-run LINT_OK+UNIT_OK** GEOM=0 L4=0 LINT=0 MAKE_UNIT=0 (`/tmp/misterplex-agent-W-lint-run.txt`). **W-r1-run MOCK_OK** TEST_RC=0 hdr `9589e573` ≠ isolated `e61988e7` **NO MERGE**. **W-idle-kick KICK_OK** host/unit first-kick mailbox (not deployed). **W-osd-size SIZE_OK** applyOsd setDecodeSize (not deployed). **W-proto-h7 PROTO_OK** (stale vs later RCs). **W-docs-h6 DOCS_OK**. Prior **18:29Z**: **W-idle-rca RCA_OK** Logo I420 in L4 both banks; **PLXD fd=0**; first-kick **SPI FAIL** → beam BLACK; glass ≠ PASS (`/tmp/misterplex-agent-W-idle-rca.txt`). **W-idle-ebfe** superseded. **W-lint-fab PATCH_OK** FIFO_DEPTH `!= 0`; **LINT_RC not taken**. **W-r1-run MOCK_OK** TEST_RC=0 MAKE_RC=0 lessons hdr `9589e573` ≠ isolated `e61988e7` **NO MERGE**; mock GREEN ≠ REAL ≠ T7 (`/tmp/misterplex-agent-W-r1-run.txt`). **W-r1-mock** RC now taken. **W-resync-l56 RESYNC_CARD** `Memory/lab/status/AV_RESYNC_AFTER_GATHER.md`. **W-yuv-scan SCAN_INV** present store already scan-order I420; product path ARM FFmpeg; no RGB565. **W-proto-h6 PROTO_OK**. **W-osd-size SIZE_OK** applyOsd `setDecodeSize` on 1280×720 present (lessons; persist 720p no longer leaves outW=640; `/tmp/misterplex-agent-W-osd-size.txt`). P4 stay **TODO**. Prior **18:17Z**: **W-geom-l4 GEOM_OK** MAKE_UNIT last proven **RC=2**. P4 stay **TODO**. Next = exec lint+unit + R1 mock RC + host first-kick (no deploy). **T7** only after REAL. **ZERO** second Quartus / additional menu. Soft-skip ≠ PASS. **BUILD_OK+DEPLOY ≠ pfps PASS.** |

**Next-after-B (user path, DEFER):** present scanout on the **SDRAM stick**, ARM decode/ring on **HPS DDR3** — `Memory/lab/status/PATH_SDRAM_PLUS_DDR.md`. Stick banks are **I420**; RGB only on the beam (L56). `17dd3b56` tri-states the stick; not a conf switch. Exclusive **FREE** after g TIMING_FAIL — still **DEFER** SDRAM path; next exclusive only after clk_pix close design + parent FIT_GO.

**Do not** invent P3-720P24 PASS. **Do not** thrash `17dd3b56` or raise product `clk_sys`. Residual / WIDE / 3l2 below are unrelated.
...
| P3-3l2 | Inv quant + IDCT first 4×4 | **RTL MODULE + REF MODEL DONE / RTL SIM PENDING** | W-REL: `h264_iq_idct_4x4.sv` + `test_p3_idct_reference_model` in `make unit`; test is an honest C++ reference/source-integration check, not a Verilog simulator. It compares MB0 block0 dequant/post-IDCT/recon samples (`y00=73`, mean=62, coeff_csum=0x14), checks all 16 MB0 pred+IDCT→recon blocks, and grades all 300 `frame_mae_v1.csv` rows (`maeY=0`). Red perturb fails at `dequant[1]`; frame-MAE perturb fails at MB17. Parent/w-cap reports P3-3l1 residual checksum closed on silicon 2026-07-26; next gate is real Verilator/Icarus execution before product IDCT wire-up/fit. |
...
| P3-3p | P-slice inter prediction / motion compensation | **RTL PRIMITIVES + INTEGRATED STREAM_PATH SIM DONE / DIAG HARDWARE QUEUE READY** | W-REL: `docs/phase3-inter-rtl.md`; product RTL `h264_inter_pred.sv` listed in `files.qip`; Verilator `test_p3_inter_rtl_sim.sh` in `make unit` compares real RTL against `inter_mc_v1.json` and red-checks bad interpolation rounding. New integrated gate `test_p3_inter_stream_path_rtl_sim.sh` elaborates product `stream_path.sv` + parsers + `decode_stub` + inter RTL, feeds paced multi-NAL fixtures (`nals=4 frames=1`; `nals=15 idr=1 p=11 frames=12`), proves the stub returns idle after each VCL with all MB1 MV/luma/chroma/fetch bands green, and red-checks a testbench-only bad diagnostic pixel fault. Hardware grading must not use the unproven no-argument visual-gate default; require direct screen/capture evidence or a re-proven explicit visual config. `decode_stub.sv` instantiates the inter block in the shipped artifact and paints top-row MB1 as four green/red bands (aggregate signature `0x56`). Server-side PMS XML now delivers Baseline/L3.0: `profile_idc=66`, CAVLC, `max_num_ref_frames=1`, `i=6 p=294 b=0`, coded 624×480/display 618×480. Exact-flag x264 proxy with `partitions=none` measured P MBs: intra 4.8%, P16×16 17.6%, sub-MB 0%, skip 77.7%; RTL rung intentionally supports P_Skip + P_L0_16x16 only. Parsed P MB syntax is not yet driving MC; next step consumes shared P MB goldens/ref-store latency. One 624×480 YUV420 ref+current=898,560 B; +present=1,347,840 B; 25 fps YUV path ~40 MB/s before overhead (~50–70 MB/s planning), comfortable for 80 MHz DDR/YUV, not SDRAM. |


| P3-SPI | SPI F1 only ~9fps — keep as fallback | DONE | |
| P3-TRUE480 | Production 240p/480p glass (v0.4.1) | **DONE** (eyes-on) | Worktree `MisterPlex-wt-480p-lessons` `480p` @ `a6ba15d`; RBF **`07f54d9f`**. Do not thrash this pair to chase 720p. |
| P3-720P24 | Stable 1280×720 @ 24 unique fps | **PARKED / unique24 FAIL** | Harvest **16:39Z:** unique24=**FAIL**. Exclusive **FREE**. **FIT_GO=NO**. **PLAY=NO**. Living lab **`112bb651` SWAPVB** scored FAIL (15.7/13.38 + 15.4/12.87; HDMI24 14.5/12.31 HURT CLOSED). Utility **`07f54d9f` UNTOUCHED** (not living unique24 lab). Next = produce-quiet (FPGA line-prefetch vs ffmpeg HPS DDR). Parent has **NOT** named **FIT_GO**. **SKIP_RESTORE=YES**. **PATH_SDRAM_PLUS_DDR DEFER**. **P4 stay TODO**. Soft-skip ≠ PASS. Cite `/tmp/misterplex-loop-status.txt` `/tmp/misterplex-agent-112bb-replay.txt` `/tmp/pfps-720p24-112bb651.txt` `/tmp/pfps-720p24-112bb651-hdmi24.txt`. Harvest **22:42Z**: **W-retry-cap IMPL_OK** RETRY-GLASS-CAP (`main.cpp` **`28b158a4`**; retry=`glassMax(content)`; needle updated; **≠ mix PASS ≠ T7** — `/tmp/misterplex-agent-W-retry-cap.txt`). **W-unit-r UNIT_OK** MAKE_UNIT_RC=0 post-RETRY (~150s; supersedes **W-unit-q STALE**; farm **UP**; play-file **160×120** stays; **UNIT_OK ≠ T7 ≠ pfps** — `/tmp/misterplex-agent-W-unit-r.txt` + `/tmp/misterplex-unit-r.log`). **W-comp-pkg3 PKG_OK** ARM **`6f1afebe` NOT_DEPLOYED** (**`903400bc`/`b6bc183f`/`b08aed9c` STALE**; **PKG ≠ deploy ≠ T7**). **W-t7-ready READY_CARD** 8/8 (**PLAY_RAN=NO**; **READY_CARD ≠ T7 PASS**). **W-docs-r / W-backlog-r 22:34Z SUPERSEDED** for Next (said RETRY next; now landed). HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; **≠ FAIL ≠ PASS**). **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK+READY_CARD** **`95bff255`**. Lab **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016** do **not** kill. Next = T7 **PARENT** sidecar after reload **j** (no new binary); HDMI = **PARENT** after user plug + ask + deploy **`6f1afebe`**; **no Quartus**. Soft-skip ≠ PASS. **P4 stay TODO**. Harvest **22:34Z** (SUPERSEDED for Next): **W-cli-glass IMPL_OK** CAP-only (**≠ mix PASS**). **W-unit-q UNIT_OK** MAKE_UNIT_RC=0 post-CAP (**≠ T7**). **W-hdmi-hold HOLD_OK** HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; **≠ FAIL ≠ PASS**). **W-pkg-hold HOLD_OK** **`903400bc` STALE**; **W-comp-pkg3** rebuilding (report **ABSENT**; do **not** deploy). **W-docs-q / W-backlog-q 22:22Z SUPERSEDED** for Next. **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK** **`95bff255`**. Disk **`07f54d9f`**. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016**. Next was **RETRY-GLASS-CAP** + ARM pkg after CAP (**landed 22:42Z**); T7 = **PARENT**; HDMI = **PARENT** after user plug. **P4 stay TODO.** Harvest **22:22Z**: **W-unit-p FAIL** RC=2 CLI **160×120**→**320×240** `glassMax`+`contentResolutionFromSize` farm **UP** (**not UNIT_OK**); **`903400bc`** now **STALE**; **RETRY-GLASS-CAP** named; **W-docs-p / W-backlog-p 22:12Z SUPERSEDED**; later **22:27Z** **W-cli-glass IMPL_OK** CAP-only (**≠ UNIT_OK**); HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; **≠ FAIL ≠ PASS**). **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK** **`95bff255`**. Disk **`07f54d9f`**. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016**. Next was **PARENT** sidecar T7 after reload **j** (**W-unit-q UNIT_OK** MAKE_UNIT_RC=0 **≠ T7**); T7 = **PARENT** sidecar after reload **j**. **P4 stay TODO.** Harvest **22:12Z**: **T7 FAIL MIX** pfps=NONE; **W-glass-max IMPL_OK**; **W-unit-o FAIL** MAKE_UNIT_RC=2 farm SSH rc=4 (parent re-pin **FARM_SSH=UP**); **W-comp-pkg PKG_OK** ARM **`b6bc183f` STALE** vs GLASS-MAX; **W-t7-book BOOK_OK** sidecar **PREP_OK+BOOK_OK** **`95bff255`**. Later **22:16Z**: **W-unit-p FAIL** RC=2 (farm **UP**; **not UNIT_OK**); **W-comp-pkg2 PKG_OK** **`903400bc`** NOT_DEPLOYED. Disk **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016**. Next was post-GLASS-MAX unit (**named 22:22Z W-cli-glass + W-unit-q**); T7 = **PARENT**; HDMI = **PARENT** after **`903400bc`**. **P4 stay TODO.** Harvest **22:02Z**: **T7 FAIL MIX**; sidecar **PREP_OK** retry **no new binary**; **CLOSED-STOP IMPL_OK**; **RECIPE_OK**; **W-unit-n UNIT_OK STALE**; **W-unit-o** running. Disk **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016**. Next was **MIX-GLASS-MAX** + post-CLOSED-STOP unit (**landed 22:12Z**); T7 = **PARENT**; HDMI recipe = **PARENT** after companion deploy. **P4 stay TODO.** Harvest **21:52Z**: **T7 FAIL MIX**; sidecar **PREP_OK**; **CLI_DECODE_WINS HOST_OK** not deployed; **RASTER-CMD IMPL_OK** not lab-gated; **MIX-CONTENT-BANK DESIGN_OK**; **W-unit-m STALE** pending **W-unit-n**. Disk **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016**. Next was post-impl unit + **CLOSED-STOP** + HDMI recipe (**landed 22:02Z**); T7 was **PARENT**. Harvest **21:40Z**: **T7 FAIL** class=**MIX HOST** (**W-t7-rca**); **ACK_PATH=LIVE** (**W-plxj-rtl**); **HOST_DESIGN_OK T7_SIDECAR_L4_CONF** (**W-osd-720p** ≠ T7 PASS); **W-score-k** `968b828a` BUILD_OK STA floors PASS chevron PASS, **T7 FAIL**, native raster FAIL. Disk **`07f54d9f`**; j kept `Plex.720p24.968b828a.rbf`. Next was implement **W-raster-cmd** + **W-decode-wins** (**landed 21:52Z**). Next T7 play was **PARENT** after sidecar + reload **j**. Lab **`968b828a` DEPLOY_OK** HDMI chevron **ORANGE_PX=11947**. **i `15bd5db2` HDMI BLACK**. Exclusive **FREE**. **FIT_GO=NO**. **T7 FAIL** `/tmp/misterplex-agent-W-t7-j.txt` (PLAY_RC=1; pfps=NONE). **W-unit-j UNIT_OK ≠ T7**. **Chevron ≠ unique-24.** **Not DONE.** **P4-DISPLAY / P4-720P-MIX** stay **TODO**. Prior Loop **21:29Z**. Next was **PLXJ RCA** not exclusive + P4-DISPLAY; **no Quartus**; **no second menu**. Prior **20:53Z**: **T5 EXIT=4**. **T6 FILE MATCH**. **DEPLOY_OK=NO**. **T7 NOT RUN**. Harvest **20:53Z**: **W-port-rev REV_OK**. **W-inv-port NEEDLE_OK**. **W-prefit HOLD_OK** `PORT_OK_NOT_FIT_GO.md` **FIT_GO** stays **NO**. **W-port-rtl PORT_OK** source-only (prior); live **ebfe4a12** still undriven; **FABRIC_PASS=NO**. **W-proto-h21 / W-docs-h20 / W-backlog-h11** 20:45Z SUPERSEDED. Next = named port-land freeze md5s + post-port make unit; **no Quartus**; **no bounce**; **no compact**; **no W-flag-off**. **PORT_OK ≠ FABRIC_PASS.** **REV_OK ≠ FABRIC_PASS.** **NEEDLE_OK ≠ FABRIC_PASS.** **HOLD_OK ≠ READY_YES.** Live **ebfe4a12** cannot swap. **T7 BLOCKED.** Prior **20:45Z**: **W-port-rtl PORT_OK** source-only clk_ddr ports; live **ebfe4a12** still undriven; **FABRIC_PASS=NO**. **W-10030-pin PIN_OK** Warning **10030** L956-957. **W-stale-gate CARD_OK**. **W-proto-h20 / W-docs-h19 / W-backlog-h10** 20:35Z SUPERSEDED (said port land LIVE). Next = review+needle PORT_OK + HOLD exclusive; **no Quartus**; **no bounce**; **no compact**; **no W-flag-off**. **PORT_OK ≠ FABRIC_PASS.** Live **ebfe4a12** cannot swap. **T7 BLOCKED.** Prior **20:35Z**: **W-swap-rca2 RCA_OK** Warning **10030** undriven `fabric_copy_*` vs db=**0xa0001733**; **fd=0 FABRIC_PASS=NO**. **W-token-ro TOKEN_OK** SOURCE=MATCH LIVE_LATCH=UNKNOWN. **W-match-card CARD_OK** MATCH **0x04100000** play_pair=NO. **W-proto-h19 / W-docs-h18 / W-backlog-h9** 20:27Z SUPERSEDED. Next = clk_ddr port passthrough (**W-port-rtl**) then parent exclusive only; **no bounce**; **no compact**; **no W-flag-off**. Live **ebfe4a12** cannot swap. **T7 BLOCKED.** **RCA_OK ≠ FABRIC_PASS.** Prior **20:27Z**: **W-plxp-re2 POKE_LIVE** src=**0x04100000**. **W-maps-13881** MATCH pagemap. **fd=0 FABRIC_PASS=NO**. Next = swap RCA **fd=0**; **no bounce**; **no compact**. Do **not** W-flag-off / compact. L56: gather+resync; idle YUV; no RGB565. **FIT_GO=consumed-h / NO**. Soft-skip ≠ PASS. **POKE_LIVE ≠ FABRIC_PASS.** **Not DONE.** |

## Phase 4 (UX)
| ID | Item | Status | Notes |
|----|------|--------|-------|
| P4-SCRUB | Scrubber/playqueue edge cases | **DONE** | G-p4-dirty `ade6915`; C-unit14..21 reconfirm |
| P4-DISPLAY | F12 Display must change MiSTer HDMI/VGA raster | **TODO** (RASTER-CMD **IMPL_OK** + **RECIPE_OK**; **not lab-gated**) | Harvest **22:42Z**: HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; missing video4 **≠ FAIL ≠ PASS**; **RECIPE not run**). **W-hdmi-hold HOLD_OK**. PKG **`6f1afebe` NOT_DEPLOYED** (**PKG ≠ deploy ≠ HDMI PASS**); **`903400bc`/`b6bc183f`/`b08aed9c` STALE**. Next HDMI = **PARENT** after user plug + ask + deploy **`6f1afebe`**; **no Quartus**. Harvest **22:34Z** (SUPERSEDED for Next): HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; missing video4 **≠ FAIL ≠ PASS**; **RECIPE not run**). **W-hdmi-hold HOLD_OK**. **`903400bc` STALE**; **W-comp-pkg3** rebuilding (report **ABSENT**; do **not** deploy). Next HDMI was **PARENT** after user plug + ask + post-CAP rebuild + deploy (**pkg landed `6f1afebe`**). Harvest **22:22Z**: HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; missing video4 **≠ FAIL ≠ PASS**; **no HDMI eyes-on** until user plugs). **RECIPE not run**. PKG **`903400bc`** now **STALE** (**HOLD**). Next HDMI = **PARENT** after user plug + ask + rebuild + deploy (not `b6bc183f`). Harvest **22:12Z**: ARM **`b6bc183f` STALE** vs GLASS-MAX (**W-comp-pkg PKG_OK**; **not deployed**; **PKG_OK STALE ≠ HDMI PASS**). Later **22:16Z**: **W-comp-pkg2 PKG_OK** **`903400bc`** NOT_DEPLOYED (**PKG ≠ deploy**). Loop later **22:16Z** HDMI=**UNPLUGGED** (no grab; **≠ FAIL ≠ PASS**). Harvest **22:02Z**: **W-hdmi-p4 RECIPE_OK** (not run). **RECIPE_OK ≠ HDMI PASS.** **Writing cmd ≠ HDMI PASS.** Next HDMI = **PARENT** after **`903400bc`** deploy (not `b6bc183f`) then recipe on **`07f54d9f`**. Prior **21:52Z**: **W-raster-cmd IMPL_OK** — F12 Display → `/dev/MiSTer_cmd` `video_mode` 6/5/0 (240p/480p/720p). Content owns DECODE; Display owns raster only. **IMPL_OK ≠ HDMI PASS.** **FIT_GO=NO**. Disk default **`07f54d9f`**. Lessons **L55**. Cite `/tmp/misterplex-agent-W-raster-cmd.txt` · `/tmp/misterplex-agent-W-hdmi-p4.txt`. Prior **21:40Z**: **W-display-k DESIGN_OK**. |
| P4-720P-MIX | 720p Display ≠ content hangs play | **TODO** (design named **MIX-CONTENT-BANK**; **CLOSED-STOP** + **GLASS-MAX** host impl landed, **not lab-gated**) | Harvest **22:42Z**: **RETRY-GLASS-CAP IMPL_OK** + **W-cli-glass CAP IMPL_OK** + **W-unit-r UNIT_OK** (**≠ mix PASS ≠ T7**; **not lab-gated**). PKG **`6f1afebe` NOT_DEPLOYED**. `GLASS_MAX_DEAD_RETRY.md` **STALE** vs IMPL. Play matrix still open. Harvest **22:34Z** (SUPERSEDED for Next): **W-cli-glass IMPL_OK** CAP-only + **W-unit-q UNIT_OK** MAKE_UNIT_RC=0 (**≠ mix PASS ≠ T7**). **RETRY-GLASS-CAP** next (W-retry-cap report **ABSENT**; **not IMPL_OK**). **`903400bc` STALE**; **W-comp-pkg3** rebuilding. Harvest **22:22Z**: glassMax snap-up FAIL **160×120→320×240** by `contentResolutionFromSize` (**W-unit-p**; farm **UP**; **not UNIT_OK**; **SUPERSEDED** by CAP+UNIT_OK). **RETRY-GLASS-CAP** still next. Later **22:27Z** **W-cli-glass IMPL_OK** CAP-only (**≠ mix PASS**; later **W-unit-q UNIT_OK**). Harvest **22:12Z**: **W-glass-max IMPL_OK** — `present_bank.hpp` `glassMax`; true480 + 1280×720 → 640×480 doorbell `0x300FF000`; L4 stays `0x3047F000`; **not deployed**. Residual dead retry left (`Memory/lab/status/GLASS_MAX_DEAD_RETRY.md`). **GLASS-MAX ≠ mix PASS ≠ T7.** ARM **`b6bc183f` STALE**; later **`903400bc`** PKG_OK NOT_DEPLOYED. Play matrix still open. Harvest **22:02Z**: **W-closed-stop IMPL_OK** (not lab-gated). **W-review-p4 REVIEW_OK** sidecar retry **no new binary**; next host **MIX-GLASS-MAX** (**landed 22:12Z**). **W-unit-n UNIT_OK STALE**. **CLOSED-STOP ≠ mix PASS ≠ T7.** Prior **21:52Z**: **W-720p-mix DESIGN_OK**. Content owns DECODE/PMS/bank; Display owns raster. Policy **UPSCALE-ASCAL**. **DESIGN_OK ≠ mix PASS.** Same glass: 240/240, 240/480, 480/480 play OK. Display 720p while content is not 720p → Plex Web waiting/buffering forever. Recover: 480/480 + F12 Reset. Soft-skip ≠ PASS. Lessons **L55**. Cite `/tmp/misterplex-agent-W-glass-max.txt` · `/tmp/misterplex-agent-W-closed-stop.txt` · `/tmp/misterplex-agent-W-review-p4.txt`. |
| P4-HZ | Match-source-Hz modeline (docs only OK) | DEFER | Related: P4-DISPLAY needs a real raster switch; switchres still unwired |
...
24. **P3-720P24** — **IN_PROGRESS / unique24 FAIL**. Harvest **16:39Z:** unique24=**FAIL**. Exclusive **FREE**. **FIT_GO=NO**. **PLAY=NO**. Living lab **`112bb651` SWAPVB** scored FAIL (15.7/13.38 + 15.4/12.87; HDMI24 14.5/12.31 HURT CLOSED). Utility **`07f54d9f` UNTOUCHED**. Next = produce-quiet (FPGA line-prefetch vs ffmpeg HPS DDR). Parent has **NOT** named **FIT_GO**. **SKIP_RESTORE=YES**. **PATH_SDRAM_PLUS_DDR DEFER**. **P4 stay TODO**. Soft-skip ≠ PASS. Harvest **22:42Z**: **W-retry-cap IMPL_OK** RETRY-GLASS-CAP (`main.cpp` **`28b158a4`**; retry=`glassMax(content)`; needle updated; **≠ mix PASS ≠ T7** — `/tmp/misterplex-agent-W-retry-cap.txt`). **W-unit-r UNIT_OK** MAKE_UNIT_RC=0 post-RETRY (~150s; supersedes **W-unit-q STALE**; farm **UP**; play-file **160×120** stays; **UNIT_OK ≠ T7 ≠ pfps** — `/tmp/misterplex-agent-W-unit-r.txt` + `/tmp/misterplex-unit-r.log`). **W-comp-pkg3 PKG_OK** ARM **`6f1afebe` NOT_DEPLOYED** (**`903400bc`/`b6bc183f`/`b08aed9c` STALE**; **PKG ≠ deploy ≠ T7**). **W-t7-ready READY_CARD** 8/8 (**PLAY_RAN=NO**; **READY_CARD ≠ T7 PASS**). **W-docs-r / W-backlog-r 22:34Z SUPERSEDED** for Next (said RETRY next; now landed). HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; **≠ FAIL ≠ PASS**). **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK+READY_CARD** **`95bff255`**. Lab **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016** do **not** kill. Next = T7 **PARENT** sidecar after reload **j** (no new binary); HDMI = **PARENT** after user plug + ask + deploy **`6f1afebe`**; **no Quartus**. Soft-skip ≠ PASS. **P4 stay TODO**. Harvest **22:34Z** (SUPERSEDED for Next): **W-cli-glass IMPL_OK** CAP-only (**≠ mix PASS**). **W-unit-q UNIT_OK** MAKE_UNIT_RC=0 post-CAP (supersedes **W-unit-p FAIL** / **W-unit-n STALE** / **W-unit-o** farm-down; **UNIT_OK ≠ T7**). **W-hdmi-hold HOLD_OK** HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; **≠ FAIL ≠ PASS**). **W-pkg-hold HOLD_OK** **`903400bc` STALE**; **W-comp-pkg3** rebuilding (report **ABSENT**; do **not** deploy). **W-docs-q / W-backlog-q 22:22Z SUPERSEDED** for Next. **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK** **`95bff255`**. Disk **`07f54d9f`**. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016**. Next was **RETRY-GLASS-CAP** + ARM pkg after CAP (**landed 22:42Z**); T7 = **PARENT**; HDMI = **PARENT** after user plug. **Not DONE.** **P4 stay TODO.** Harvest **22:22Z**: **W-unit-p FAIL** RC=2 CLI **160×120**→**320×240** `glassMax`+`contentResolutionFromSize` farm **UP** (**not UNIT_OK**); **`903400bc`** now **STALE**; **RETRY-GLASS-CAP** named; **W-docs-p / W-backlog-p 22:12Z SUPERSEDED**; later **22:27Z** **W-cli-glass IMPL_OK** CAP-only (**≠ UNIT_OK**); HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; **≠ FAIL ≠ PASS**). **T7 FAIL MIX** pfps=NONE; sidecar **PREP+BOOK** **`95bff255`**. Disk **`07f54d9f`**. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016**. Next was **PARENT** sidecar T7 after reload **j** (**W-unit-q UNIT_OK** MAKE_UNIT_RC=0 **≠ T7**); T7 = **PARENT** sidecar after reload **j**. **Not DONE.** **P4 stay TODO.** Harvest **22:12Z**: **T7 FAIL MIX** pfps=NONE; **W-glass-max IMPL_OK**; **W-unit-o FAIL** farm-down; sidecar **PREP_OK+BOOK_OK** **`95bff255`** no new binary; pkg **`b6bc183f` STALE**. Later **22:16Z**: **W-unit-p FAIL** RC=2 (farm **UP**; **not UNIT_OK**); **W-comp-pkg2 PKG_OK** **`903400bc`** NOT_DEPLOYED. Disk **`07f54d9f`**; j kept. Exclusive **FREE**. **FIT_GO=NO**. Daemon **24016**. Next was post-GLASS-MAX unit (**named 22:22Z W-cli-glass + W-unit-q**); T7 = **PARENT**; HDMI = **PARENT** after **`903400bc`**. **Not DONE.** **P4 stay TODO.** Cite lessons SoT.
25. **P4-DISPLAY** — **TODO** (**RASTER-CMD IMPL_OK** + **RECIPE_OK**; not lab-gated). Harvest **22:42Z**: HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; missing video4 **≠ FAIL ≠ PASS**; **RECIPE not run**). **W-hdmi-hold HOLD_OK**. PKG **`6f1afebe` NOT_DEPLOYED** (**PKG ≠ deploy ≠ HDMI PASS**); **`903400bc`/`b6bc183f`/`b08aed9c` STALE**. Next HDMI = **PARENT** after user plug + ask + deploy **`6f1afebe`**; **no Quartus**. Harvest **22:34Z** (SUPERSEDED for Next): HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; missing video4 **≠ FAIL ≠ PASS**; **RECIPE not run**). **W-hdmi-hold HOLD_OK**. **`903400bc` STALE**; **W-comp-pkg3** rebuilding (report **ABSENT**; do **not** deploy). Next HDMI was **PARENT** after user plug + ask + post-CAP rebuild + deploy (**pkg landed `6f1afebe`**). Harvest **22:22Z**: HDMI=**UNPLUGGED** `/dev/video4` **absent** (no grab; missing video4 **≠ FAIL ≠ PASS**; **no HDMI eyes-on** until user plugs). **RECIPE not run**. Companion **`b6bc183f` STALE**; **`903400bc`** now **STALE** vs CAP-only (**HOLD**). Loop later **22:16Z** HDMI=**UNPLUGGED** (no grab; **≠ FAIL ≠ PASS**). F12 Display → `video_mode` 6/5/0. **Writing cmd ≠ HDMI PASS.** **RECIPE_OK ≠ HDMI PASS.** Next HDMI = **PARENT** after **`6f1afebe`** deploy (not `903400bc` / `b6bc183f`). True480 `07f54d9f` stays. Lessons **L55**.
26. **P4-720P-MIX** — **TODO** (**MIX-CONTENT-BANK DESIGN_OK**; **CLOSED-STOP IMPL_OK**; **GLASS-MAX IMPL_OK**; **RETRY-GLASS-CAP IMPL_OK** + **W-unit-r UNIT_OK**; ≠ mix PASS ≠ T7; **not lab-gated**). Harvest **22:42Z**: **RETRY-GLASS-CAP IMPL_OK** + **W-cli-glass CAP IMPL_OK** + **W-unit-r UNIT_OK** (**≠ mix PASS ≠ T7**; **not lab-gated**). PKG **`6f1afebe` NOT_DEPLOYED**. Harvest **22:34Z** (SUPERSEDED for Next): **RETRY-GLASS-CAP** next (W-retry-cap report **ABSENT**). **`903400bc` STALE**. Harvest **22:22Z**: snap-up FAIL **160×120→320×240** by `glassMax`+`contentResolutionFromSize` (**W-unit-p**; farm **UP**; **SUPERSEDED** by CAP+UNIT_OK). Later **22:27Z** **W-cli-glass IMPL_OK** CAP-only (**≠ mix PASS**; later **W-unit-q UNIT_OK**). Residual dead retry left (`GLASS_MAX_DEAD_RETRY.md`). Display 720p ≠ content hangs play (spinner). ARM **`b6bc183f` STALE**; **`903400bc`** PKG_OK NOT_DEPLOYED. Sidecar retry **no new binary**. 240/240, 240/480, 480/480 OK. Recover 480/480 + Reset. Lessons **L55**.
