...
| P3-3p | P-slice inter prediction / motion compensation | **PRODUCT DPB/MC + DEBLOCK SEAM LIVENESS GREEN / NATIVE INTER QUALITY RED** | W-REL: `docs/phase3-inter-rtl.md`; product RTL `h264_inter_pred.sv`, `h264_p_slice_modes.sv`, `h264_dpb.sv`, and `h264_deblock_writeback_ctrl` are in the stream-path liveness build. DPB budget closes at 987 B/MB = 1,154,790 B/frame = 28.87 MB/s at 25fps; DDR first, SDRAM escape hatch. `test_p3_dpb_mc_rtl_sim.sh` now prints Scope first and proves MC reads the **post-deblock committed DPB** tap: filtered I420 writeback, frame-boundary promotion, IDR invalidation, clamped 21x21/9x9 fetch, 16x16 MC (`mc_mb_fraction=1/1170`), edge fetch (`fetch_mb_fraction=2/1170`), deblock seam ordering, and partition masks for 16x8/8x16/8x8/8x4/4x8/4x4; red-checks early deblock MB commit, early ref-ready/frame_done, clamp, MC arithmetic, early ref publication, and partition mask. `test_p3_inter_rtl_sim.sh` prints Scope first and compares real RTL against `inter_mc_v1.json` plus stress sweeps (`mv_cases=6`, `partition_cases=10`, `frame_mv_cases=9090`, `frame_mv_mbs=3510/1170`, `luma_qpel=16+128`, `chroma_epel=3+1024`, `fetch=5+3510`) and red-checks bad interpolation rounding plus bad partition MV. Both MC gates explicitly do **not** cover QPc because they are prediction-only/no-QP. `test_h264_p_slice_modes_rtl_sim.sh` covers P_Skip, P_L0_16x16, P_L0_16x8, P_L0_8x16, P_8x8/P_8x8ref0, sub-MB 8x8/8x4/4x4, intra-in-P and unsupported mode classification; red-check swaps 16x8 and fails. `slice_hdr_parser.sv` handles non-IDR ref-marking/ref-idx bits before QP and parses P `mb_skip_run` + first P MB type; shared multi-NAL raw `nalu=15 slice=11 idle_between_vcl=1 recon_sig_3b_cycles=39780 p_first_mb_seen=11 p_first_modes=8/2/1 p_first_bad=0`, with forced-`recon_sig=0` red-check. `decode_stub` routes IDR invalidation and reference promotion through product `h264_deblock_writeback_ctrl` (`filtered_sample_valid` before `filtered_mb_valid`, terminal commit then `frame_boundary`, `ref_ready_pulse` → DPB `frame_done`). `test_stream_path_full_frame_compare.sh` now emits both the old refused RGB565 diagnostic candidate and a native-I420 DPB/MC candidate with inter MB metadata; current 320×240 score is `intra=0/300 inter=0/3300`, first_bad_inter frame 1 MB0 Y `got=32 ref=80 abs=48`, `mb_type=P_L0_16x16`, `mv_l0=(0,0)`; 624×480 scorer self-check is `intra=0/1170 inter=0/12870`, first_bad_inter `got=32 ref=77 abs=45`; both use a declared diagnostic filtered reference state. W-SWAP-MC now has `h264_decode_core` consuming `h264_inter_mc_part` directly for full 21x21/9x9/9x9 P16 windows: `test_h264_decode_core_p16z_rtl_sim.sh` covers 3/1170 MBs with 603 DPB reads/MB and red-checks pred/residual/MV/RBSP/scan/U-V faults; `test_h264_decode_core_real_slice_rtl_sim.sh` covers 2/1170 real-content MBs and red-checks U/V reference swaps. However `h264_decode_core` is still staging/bench-only until W-DECODE wires it under `stream_path`; product reachability by `check_rtl_module_instantiations.py` remains the merge gate. Full native-I420 inter reconstruction, real decoded/deblocked reference pictures, parsed MV deltas, residual add, and all-P-MB traversal remain open. |
