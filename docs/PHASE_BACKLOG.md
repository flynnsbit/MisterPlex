...
- Green rollback `57674f2e`: exact ROI pixels `19200/19200`, MAE `[0,0,0]`, max_abs `0`, `rc=0`.
  **CAVEAT (measured by W-E2E-O5): this green is vacuous as a decode claim.** `19200 = 160×120` is the
  `compare_box 11,0,160,120`, and that ROI in the golden is **flat background** (9 distinct colours,
  luma std 5.35). The golden depicts the Plex chevron **idle screen**, not the decode of the bitstream
  its provenance names — host-decoding that bitstream yields bright testsrc2 colour bars (luma std
  65.53, MB0 16×16 std 53.22; overall mean luma 123 vs the golden's 23.6, correlation **ncc −0.0735**).
  So `19200/19200 MAE 0` means flat background matched flat background. It is evidence about the
  display/present path only. Use `scripts/prove_decoded_frame.py` for any decode claim.
...
| Visual hardware golden `plex_real_baseline_320x240_57674f2e_mjpeg720_golden.png` | **REFUTED AS DECODE EVIDENCE / OWNER W-E2E-O5** | W-E2E-O5 host-decoded the bitstream the sidecar names (`p3_host_recon/plex_real_baseline_320x240_1f.264`) and compared it to the golden: the reference decode is bright testsrc2 colour bars (luma std 65.53, 204 distinct luma, MB0 16×16 std 53.22, mean luma 123); the golden is the Plex chevron **idle screen** (mean luma 23.6) whose declared "MB0" ROI `11,0,160,120` is flat background (9 distinct colours, std 5.35). Correlation **ncc −0.0735** — uncorrelated. The `19200/19200 MAE 0` green therefore matched flat background against flat background. This artifact is evidence about display/present only and **must not be cited as evidence that any core decoded anything**. Decode claims must use `scripts/prove_decoded_frame.py`, which requires agreement with the host decode of the pushed bitstream. |
