...
| P3-3m | Inter prediction scope + host goldens | **SCOPE/MODEL DONE / RTL TODO** | W-REL: `docs/phase3-inter-prediction.md`; checked-in P16×16-only Baseline vector `plex_inter_p16_baseline_320x240_12f.264` (27653 B, md5 `fe5ba815…`), `pframe1_mb_v1.json`, `frame_mae_v1.csv`, and `test_p3_inter_pred_vectors` in `make unit`. Survey: available Baseline assets are I/P-only/no-B but natural encodes use quarter-pel plus some 16x8/8x16/8x8; narrow target needs PMS/w-a4 profile constraints (`bframes=0`, `ref=1`, `weightp=0`, ideally P16×16). DDR3 reference store required for 480p; BRAM/SDRAM not viable. |
