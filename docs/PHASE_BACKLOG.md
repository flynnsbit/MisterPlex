...
| P-slice / motion compensation full-frame output | **MEASURED RED / OWNER W-REL** | Product `stream_path` parser now drives DPB/MC liveness through the product deblock writeback commit barrier (`recon_sig_3b_cycles=39780`, forced-`recon_sig=0` red). Full-frame output is still diagnostic RGB565 converted back to I420, so it is not a native decode PASS; against `ffmpeg -skip_loop_filter all`, 320×240 P frames measure `0/3300` MB exact with Y MAE `76.468417` (`colorspace=I420_FROM_RGB565`). Standalone DPB/MC RTL tests remain evidence-backed. |
