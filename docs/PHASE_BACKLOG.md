...
- [ ] G-AV9 retune `Video delay` default under the audible clock — eyes-on TNG S1E1 `40868` @ `offset=234000` with `av_offset_ms=0`, report the value that locks the lips. Expect a small residual (display video processing is typically tens of ms, so a **negative** trim is plausible), not the old +80. Baking it in needs one RBF fit to relabel the list.
