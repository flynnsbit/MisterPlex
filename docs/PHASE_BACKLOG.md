...
| Motion compensation reachable from the product decoder root | **EVIDENCE-BACKED GREEN / OWNER W-SWAP-O5** | `h264_decode_core` now instantiates `h264_dpb_one_ref` (`u_product_dpb_ref`) and `h264_inter_mc_part` (`u_product_p16_mc`), pulling `h264_inter_mc_16x16`, `h264_luma_qpel_block_16x16`, `h264_chroma_epel_block_8x8`, `h264_luma_ref_tap_addr` and `h264_ref_clamp` under the product root. `check_rtl_module_instantiations.py --root h264_decode_core` reports `RTL_MODULE_INSTANTIATION_OK rtl_modules=68 reachable=21 bench_only=18 root=h264_decode_core` with all eight REQUIRED modules REACHABLE. The green is **red/green-proved**: `tests/unit/test_h264_decode_core_mc_reachability_redgreen.py` cuts each of the 10 product instantiation sites in turn, requires rc=1 with `REQUIRED_RTL_MODULE_UNREACHABLE`, restores and requires rc=0 (8/8). Plain `emu`-rooted reachability remains masked by `decode_stub` and must not be used for this claim. |
| P16x16 motion-compensated prediction correctness at frame scope | **EVIDENCE-BACKED GREEN / OWNER W-SWAP-O5** | `tests/unit/test_h264_decode_core_full_frame_mc_rtl_sim.sh` drives **1170/1170** macroblocks of a real 624x480 frame through the product core as P16x16 inter MBs and checks **449280/449280** predicted samples against an independent qpel/epel model evaluated on the reference picture: `OK h264_decode_core full-frame MC: 1170/1170 P16x16 macroblocks predicted exactly; samples Y=299520 U=74880 V=74880 total=449280; reads=705510 (all addresses exact); luma_qpel_phases=16/16 chroma_epel_phases=64/64 sub_pel_luma_samples=280064 edge_clamped_mbs=96 differ_from_colocated=230469/299520 pred_range=0..197 cycles=1162986`. Red-checked by `H264_DECODE_CORE_FAULT_PERTURB_MV` and `H264_DECODE_CORE_FAULT_DROP_PRED`. This gate found a real defect the 2-3 MB gates missed: `h264_dpb_one_ref` retired only one of two in-flight reads in `PH_DRAIN`, losing the final chroma-V window sample; `PH_DRAIN2` now retires both. **Scope limit (measured, not overstated):** this proves MC prediction arithmetic + reference addressing + DPB writeback inside the product core. It does **not** prove end-to-end P-slice decode: MVs and reference pictures are supplied by the testbench, not parsed from a bitstream, and zero frames have still been decoded and displayed by the FPGA. |
