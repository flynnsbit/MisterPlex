...
| P3-3p | P-slice inter prediction / motion compensation | **RTL PRIMITIVES + SIM DONE / HARDWARE QUEUE READY** | W-REL: `docs/phase3-inter-rtl.md`; product RTL `h264_inter_pred.sv` listed in `files.qip`; Verilator `test_p3_inter_rtl_sim.sh` in `make unit` compares real RTL against `inter_mc_v1.json` and red-checks bad interpolation rounding. `decode_stub.sv` now instantiates the inter block in the shipped artifact and paints top-row MB1 as four green/red bands for MV/luma/chroma/fetch (aggregate signature `0x56`). Server-side PMS XML now delivers Baseline/L3.0: `profile_idc=66`, CAVLC, `max_num_ref_frames=1`, `i=6 p=294 b=0`, coded 624×480/display 618×480. Exact-flag x264 proxy with `partitions=none` measured P MBs: intra 4.8%, P16×16 17.6%, sub-MB 0%, skip 77.7%; RTL rung intentionally supports P_Skip + P_L0_16x16 only. Modules cover MV median predictor/MVD add/P_Skip zero rule, luma quarter-pel six-tap interpolation, chroma eighth-pel bilinear, and 9×9 reference fetch/edge clamp. One 624×480 YUV420 ref+current=898,560 B; +present=1,347,840 B; 25 fps YUV path ~40 MB/s before overhead (~50–70 MB/s planning), comfortable for 80 MHz DDR/YUV, not SDRAM. |
