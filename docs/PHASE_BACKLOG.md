...
| P3-3l2 | Inv quant + IDCT first 4×4 | **RTL SIM PREP DONE / HW BLOCKED** | W-REL: `h264_iq_idct_4x4.sv` + `test_p3_idct_rtl_sim` in `make unit` compare all 16 MB0 block0 dequant/post-IDCT/recon samples against `mb0_luma_v1.json` (`y00=73`, mean=62, coeff_csum=0x14); red perturb fails at `dequant[1]`. Hard-block until res_csum=0x14 stable ≥2 on **non-DIAG product** RBF. Lab **LOADED `94bbfe43`**; **H-gate-rcsum6 IN_PROGRESS/PENDING** — **do not unblock 3l2** until hard product sticky 0x14. **BUILD_OK+DEPLOY ≠ 3l2 UNBLOCK**. |
