...
| **Goldens** | no current default; each run must set `VISUAL_GOLDEN` whose `*_visual_golden_v1.json` source RBF md5 matches the loaded core |
...
**Capture/provenance integrity:** compare exit codes are distinct: `1` = real visual mismatch, `3` = stale frame,
`4` = V4L2/FFmpeg corrupted buffer/data, `5` = absent device, `6` = busy device, `8` = loaded RBF md5 does not
match the visual golden/source RBF. Capture-rig or wrong-core failure is never accepted as either green or
known-red core evidence.
...
- The old `57674f2e` rollback golden is retired as a default. It is still a valid record of that exact RBF md5,
  but rollback predates the current YUV420 frame-store contract; using it against a current I420 pipeline now
  fails provenance before pixel grading. The 624×480 fixture/golden are retained only as a future target and are
  not the default because that unproven pairing false-reded on known-good `57674f2e`; full-frame/default active-region
  compare also false-reded due reload-dependent pixels outside the stable 320×240 ROI.
...
- [x] Historical: promoted the proven 320×240 vector + hardware-captured `57674f2e` golden before the YUV420
  pipeline mismatch was identified.
- [x] Retire the `57674f2e` rollback golden as a default and require visual-golden RBF md5 provenance (`rc=8`
  on mismatch/missing identity) before grading pixels.
...
| P3-3p | P-slice inter prediction / motion compensation | **P SYNTAX + MV PRIMITIVES INTEGRATED / FULL-FRAME STILL RED** | W-REL: `docs/phase3-inter-rtl.md`; product RTL `h264_inter_pred.sv` and `h264_p_slice_modes.sv` listed in `files.qip`. `test_p3_inter_rtl_sim.sh` compares real RTL against `inter_mc_v1.json` (`mv_cases=6`, `partition_cases=10`, `frame_mv_cases=9090`) and red-checks bad interpolation rounding plus bad partition MV. `test_h264_p_slice_modes_rtl_sim.sh` covers P_Skip, P_L0_16x16, P_L0_16x8, P_L0_8x16, P_8x8/P_8x8ref0, sub-MB 8x8/8x4/4x8/4x4, intra-in-P and unsupported mode classification; red-check swaps 16x8 and fails. `slice_hdr_parser.sv` now handles non-IDR ref-marking/ref-idx bits before QP and parses P `mb_skip_run` + first P MB type; shared multi-NAL raw `nalu=15 slice=11 idle_between_vcl=1 recon_sig_3b_cycles=0 p_first_mb_seen=11 p_first_modes=8/2/1 p_first_bad=0`. Integrated `test_p3_inter_stream_path_rtl_sim.sh` still proves the product `stream_path` + parsers + `decode_stub` + inter RTL handoff and bad-pixel red-check. Full-frame per-plane gate remains honest RED at frame 0 (`Y exact=189/76800 mae=82.082448`, `U exact=51/76800 mae=84.779232`, `V exact=29/76800 mae=84.160716`); parsed P syntax is classified but not yet driving reference fetch/MC/reconstruction. Next step consumes shared full P-MB goldens/ref-store latency and wires P_Skip/partition outputs into the MC datapath without weakening the full-frame comparator. |
