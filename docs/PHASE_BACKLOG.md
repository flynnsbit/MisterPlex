...
| P3-3m | Inter prediction scope + host goldens | **BASELINE SCOPE/MODEL DONE / RTL TODO** | W-REL: `docs/phase3-inter-prediction.md`; checked-in P16×16-only Baseline vector `plex_inter_p16_baseline_320x240_12f.264` (27653 B, md5 `fe5ba815…`), `pframe1_mb_v1.json`, `frame_mae_v1.csv`, and `test_p3_inter_pred_vectors` in `make unit`. `main` now requests PMS 480p H.264 Baseline Level 3.0 (`640x480`, 2500 kbps), making scope tractable: P/CAVLC only, no B/CABAC/weighted/interlace. Must still probe delivered stream and fail closed on B/CABAC/non-Baseline/too many refs. Level 3.0 DPB max at 640x480 is 6 refs = 2.76 MB YUV420; one-ref target is much cheaper. DDR3/YUV420 reference store required; BRAM/SDRAM not viable. |
