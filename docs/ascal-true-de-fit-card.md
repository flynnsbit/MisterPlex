...
**RBF provenance binding (named deploy capability):** every produced RBF must carry `scripts/rbf_provenance.py` manifest (sidecar `Plex.rbf.provenance.json` + `release_artifacts/rbf-manifests/<md5>.json`) with **rbf_md5 + git_commit + git_dirty + resolved QIP file list**. `build_rbf_remote.sh` emits after copy-back; `deploy_plex_core.sh` **refuses** (exit **8**) when the RBF has no matching manifest (`DEPLOY_PROVENANCE=require` default; `warn`/`off` are loud non-passes). One command for “what commit built the on-device core?”: `make rbf-what-built DEVICE=1` or `MD5=dfebf2bf…`. Historical G-VID1 seed binds md5 `dfebf2bf…` → commit `0139f2c5` with **no** `ddr_frame_store` in QIP — the exact stale-bitstream defect class. **Proves source binding only — not that the fit is correct.**  

**Fabric build-id stamp (RTL):** `rtl/plex_rbf_build_id.sv` instantiated as `u_rbf_build_id` in `Plex.sv`, listed in `files.qip`. Params from `scripts/gen_rbf_build_id_vh.py` → `plex_rbf_build_id_params.vh` (COMMIT_PREFIX + dirty + qip_count) run before remote fit. Prefit reachability requires `plex_rbf_build_id` **REACHABLE**. **Post-fit stage** (`make post-fit-hierarchy FIT_RPT=…`, also after `build_rbf_remote` copy-back): entity must appear in the fit hierarchy with ≥8 registers — that is the only mechanical proof the stamp **survived fitting**. Soft-skip 77 ≠ pass if FIT_RPT absent. Sim: `tests/unit/test_plex_rbf_build_id_rtl_sim.sh` GREEN + FAULT_ZERO_STAMP RED twin.  
