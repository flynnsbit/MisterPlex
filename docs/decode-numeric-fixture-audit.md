...
| Real-slice chroma residual recon/writeback + geometry | Always-on 8f derived I420 scoreboard now requires nonzero chroma residual, U/V-distinct chroma residuals, right **and** bottom chroma clamp reads, and scores 3 P16 MBs exact through pred+residual+clip writeback. | Swap U/V residual on zero-MV residual feed (`H264_DECODE_CORE_FAULT_SWAP_CHROMA_RESIDUAL`); swap U/V chroma reads. | `sample 256 plane=U ... got=151 want=132 pred=142 residual=-10`; `read_ordinal 20736 got_addr=0x5f6a8 want_addr=0x4d228` | Sound |
| Real-slice deblock gap (enabled gold) | Companion 8f slice decoded with loop filter **enabled**; product pre-deblock writeback scored against it without redefining residual from the enabled pair. | Point enabled path at disabled fixture (instrument vacuity). | `DEBLOCK_GAP_SURVIVAL ... mb_exact=0/3 sample_match=...`; vacuous path: `disabled and enabled slices are identical` / `enabled-gold survival vacuous` | Sound — gap is large (temporal cascade); not a product deblock PASS |
| I-frame0 pure filter gap (no cascade) | Bitstream frame 0 decoded skip_loop_filter vs default; locks Y mism=80, chroma exact, mb_exact=1167/1170, three edge MBs. | disabled-as-enabled identical buffers. | `DEBLOCK_IFRAME_GAP ... y_mism=80/299520 u_mism=0 v_mism=0 y_max_abs=1 mb_exact=1167/1170 mismatch_mbs=[[0,20],[0,21],[38,20]]`; vacuous: `identical (gap vacuous)` | Sound — separates pure edge filter delta from P-slice cascade |
...
- **In-loop deblock is not product-wired on the P16 recon path.** Evidence split: (1) I-frame0 pure filter gap is tiny (`y_mism=80`, chroma exact); (2) cascaded 8f P-slice scoreboard vs enabled gold is large (`mb_exact=0/3`, `sample_pct≈57` on those MBs; broader grid ≈32%). Closing needs stream-derived bS/QP + neighbor-aware luma edges before DPB commit; residual-oracle MB scoreboard cannot match cascaded enabled gold alone.
