...
- **In-loop deblock is not product-wired on the P16 recon path.** The enabled companion fixture measures the gap (MB-exact ~0%; sample survival ~30% on scoreboard MBs due to filtered-ref cascade). Closing requires full-frame/neighbor-aware edge filtering before DPB commit — isolated residual-oracle MB scoreboard cannot match enabled gold alone.
