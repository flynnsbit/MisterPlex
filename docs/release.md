...
**Readback finding (source-proven, 2026-07-29):** there is **no** FPGA→ARM reconstructed I420 path today. Evidence: `ddr_frame_store.sv` header states ARM writes the banks; FPGA `DDRAM_WE` sites write mailboxes only (PLXF/PLXD/PLXS/…); ARM API is `sendYuv420pFrameDdr`/`publishDdrFrame` (write) + mailbox readers — `FpgaSpi::tryCaptureReconI420` fails closed with `kNoReconReadbackReason`. DPB recon (`h264_dpb.sv` `mem_*`) is internal, not an HPS capture ABI. Reading present banks would return the ARM's own last write (silently wrong). Until RTL exports a dedicated recon plane + ready doorbell, hybrid stays **host-only composite with loud reclass** (`used_host_fallback`). `product_recon_ok` is never invented on the host. Daily-driver conf leaves `HYBRID_PRESENT=0`.

**Bandwidth:** paper arithmetic at 320×240 I420 is 115 200 B/frame (~2.88 MB/s @ 25 fps) vs ~28.87 MB/s DPB budget — plausible on paper. **Measured hybrid composite fps / per-frame DDR copy+flush+doorbell on device = UNKNOWN** (no deploy in this lane; `lastDdrTiming` covers ARM write-present only). Do not claim a frame rate without that measurement.
