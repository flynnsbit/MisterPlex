...
**Recon export ABI (P3-3l5, 2026-07-29):** FPGA→ARM I420 uses a **dedicated** window — `mailbox_abi::kReconExportPhysBase` (`0x30200000`), dual bank stride `0x40000`, ready/seq mailbox **PLXO** `@ 0x3007F130` magic `PLXO`. **Never** the present banks at `0x30000000` (reading those returns ARM's own last write). RTL: `h264_recon_export.sv` (new file) taps DPB current-frame byte writes from `decode_stub`, steals the stream-side DDR slot only when bitstream is idle. ARM: `FpgaSpi::tryCaptureReconI420` double-reads PLXO and fails closed on `ready=0` / `torn=1` / magic mismatch; on success memcpy from the published bank. Hybrid still loud-reclassifies (`used_host_fallback`) when capture fails. `product_recon_ok` is never invented on the host. Daily-driver conf leaves `HYBRID_PRESENT=0`.
