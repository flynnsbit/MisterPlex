...
```text
RTL_MODULE_INSTANTIATION_OK rtl_modules=68 default_reachable=41 nondefault_config_reachable=7 bench_only=20 config_reachable=41
```
...
DECODE_REAL_INTRA_ON_ONLY_COUNT 4
...
DECODE_REAL_INTRA_ON_ONLY h264_intra_nb_ctx
...
Conclusion from the numbers only: `h264_intra_nb_ctx`, `h264_decode_top`,
`h264_intra4x4_pred`, and `h264_intra16x16_pred` do become reachable together
when `DECODE_REAL_INTRA=1`; they are absent from the default product graph.
...
DEFAULT_OFF_DEFINE_REACHABLE_MODULE h264_intra_nb_ctx defines=DECODE_REAL_INTRA=1
