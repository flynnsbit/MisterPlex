...
module ddr_bitstream_reader #(
	...
	// (8192 by default), independently of VCL RBSP and this leaf's ring capacity.
	...
	input  wire        decoder_fault_valid,
	input  wire [31:0] decoder_fault_seq,
	// Observed CTRL reset or local flush, held through real retirement.
	...
	reg [3:0] state;
	reg [1:0] mode;
	localparam int POLL_CYCLES = `PLEX_PERF_SYS_CYCLES(1 << POLL_DIV_BITS);
	localparam int POLL_COUNT_BITS = POLL_CYCLES > 1 ? $clog2(POLL_CYCLES) : 1;
	reg [POLL_COUNT_BITS-1:0] poll_div;
	reg [63:0] beat_q;
	reg [2:0] byte_idx;
	reg [3:0] beat_left;
	...
	wire decoder_failed = ENABLE_AU_PROTOCOL && decoder_fault_valid;
	...
	wire want_poll = enable && (poll_div == 0);
	...
	assign reset_pending = flush_pending || state == ST_EPOCH;
	...
	always @(posedge clk) begin
		...
		if (reset) begin
			...
			poll_div <= poll_div == POLL_CYCLES-1 ? '0 : poll_div + 1'd1;
			if (flush)
				flush_pending <= 1'b1;
			...
			// A local flush cancels speculative bytes/metadata, not an accepted
			// DDR transaction. Retire that transaction before issuing a NEW CTRL
			// read; a poll accepted before flush is not the cancellation baseline.
			if ((flush || flush_pending) && state != ST_EPOCH) begin
				out_valid <= 0;
				out_last <= 0;
				au_valid <= 0;
				bit_in_valid <= 0;
				...
				if (state == ST_COMMAND) begin
					if (bus_done) begin
						state <= ST_CANCEL;
						if (tx_kind == TX_POLL && poll_return_state == ST_CANCEL &&
						    bus_result[31:0] == MAGIC_CTRL) begin
							have_ctrl <= 1;
				...
				end else if (state != ST_CANCEL)
					state <= ST_CANCEL;
				else if (bus_idle && !cmd_valid && !flush)
					issue(TX_POLL, 0, CTRL_W, 0);
			end else case (state)
				ST_IDLE: if (bus_idle && (enable || flush_pending || flush_ack_pending)) begin
				...
				ST_EPOCH: if (fence_idle && bus_idle && !cmd_valid && !flush) begin
					...
					publish_pending <= 1;
					publish_step <= 0;
					// A stopped source cannot suppress its cancellation ACK.
					flush_ack_pending <= flush_pending;
					flush_pending <= 0;
					reset_parser();
					state <= ST_IDLE;
				end
				...
				ST_HEADER: begin
					state <= ST_IDLE;
					if (fatal_sticky || decoder_failed)
						beat_left <= 0;
					else if (!header_common_ok)
						fail_record();
				...
				ST_METADATA: begin
					if (fatal_sticky || decoder_failed)
						state <= ST_IDLE;
					else if (hdr64(0) == 64'h8000000000000000 ||
					    hdr32(8) == 0 || hdr[11][7] ||
					    hdr32(12) == 0 || hdr[15][7] ||
				...
				ST_META_WAIT: if (fatal_sticky || overrun_sticky || decoder_failed)
					state <= ST_IDLE;
				...
				ST_FENCE: if (fatal_sticky || overrun_sticky || decoder_failed)
					state <= ST_IDLE;
			...
			endcase
			// Halt this session without releasing its nonce/epoch or cancelling
			// the bus FSM. Accepted DDR commands still retire normally; CTRL
			// reset remains the only path that acknowledges a fresh identity.
			if (decoder_failed &&
			    !flush && !flush_pending && state != ST_EPOCH) begin
				out_valid <= 0;
				out_last <= 0;
				au_valid <= 0;
				bit_in_valid <= 0;
				...
				active <= 0;
				paused <= 0;
				if (!fatal_sticky) begin
					fatal_sticky <= 1'b1;
					last_bad_seq <= decoder_fault_seq;
					publish_pending <= 1'b1;
				end
			end
		end
