...
module colorbars #(
	parameter int CLOCK_DIV = `PLEX_PERF_SYS_MULT,
	parameter int CLOCK_DEN = `PLEX_PERF_SYS_DEN
) (
	input  wire        clk,
	input  wire        reset,
	...
	assign hc_out = hc;
	assign vc_out = vc;
	localparam int PIX_DIV_W = CLOCK_DIV > 1 ? $clog2(2 * CLOCK_DIV + 1) : 1;
	reg [PIX_DIV_W-1:0] pixel_div;
	wire [PIX_DIV_W-1:0] pixel_period =
		PIX_DIV_W'(scandouble ? CLOCK_DIV : 2 * CLOCK_DIV);
	wire [PIX_DIV_W-1:0] pixel_last =
		PIX_DIV_W'((scandouble ? CLOCK_DIV : 2 * CLOCK_DIV) - CLOCK_DEN);
	...
	always @(posedge clk) begin
		if (CLOCK_DIV == 1) begin
			...
			ce_pix <= pixel_div >= pixel_last;
			// Rephase an over-range remainder when scandoubling shortens the period.
			pixel_div <= pixel_div >= pixel_period ? '0 :
			             pixel_div >= pixel_last ? pixel_div - pixel_last :
			             pixel_div + PIX_DIV_W'(CLOCK_DEN);
		end
