...
generate if (PERSISTENT_MASTER) begin : persistent
	// This mode requires a master whose offered command and burst ownership
	// survive reset. Quiesce new offers, not a stalled offer or a started burst.
	reg isolated = 1'b1;
	reg held_read = 1'b0;
	reg [BURSTCOUNT_WIDTH-1:0] write_left = 0;
	wire allow_command = (!rst_req_sync && !isolated) || held_read || (write_left != 0);
	always_ff @(posedge clk) begin
		isolated <= rst_req_sync;
		held_read <= read_master && waitrequest_master;
		if (write_master) begin
			if (write_left == 0)
				write_left <= burstcount_slave - (waitrequest_master ? 1'd0 : 1'd1);
			else if (!waitrequest_master)
				write_left <= write_left - 1'd1;
		end
	end
	assign burstcount_master = burstcount_slave;
	assign address_master = address_slave;
	assign read_master = read_slave && allow_command;
	assign write_master = write_slave && allow_command;
	assign byteenable_master = byteenable_slave;
	assign waitrequest_slave = waitrequest_master || !allow_command;
...
// Return traffic is independent of command isolation and backpressure.
