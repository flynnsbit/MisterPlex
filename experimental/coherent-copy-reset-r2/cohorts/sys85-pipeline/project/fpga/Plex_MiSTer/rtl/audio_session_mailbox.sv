...
module audio_session_mailbox #(
	...
	parameter POLL_CYCLES = `PLEX_PERF_SYS_CYCLES(1024)
) (
	input wire clk,
	input wire reset,
	...
	localparam IDLE=0, READ_FIRST=1, WAIT_FIRST=2, READ_BODY=3, WAIT_BODY=4,
	           ...
	           CHECK_FIELDS=12, CHECK_SNAPSHOT=13;
	reg [3:0] state = IDLE;
	...
	reg fields_valid = 0, command_changed = 0;
	(* preserve *) reg [447:0] snapshot_capture = 0;
	(* async_reg = "true", preserve *) reg ack_s1 = 0, ack_s2 = 0, snap_s1 = 0, snap_s2 = 0;
	...
	always @(posedge clk) begin
		...
		if (ENABLE) begin
			...
			WAIT_BODY: begin end
			CHECK_FIELDS: begin
				fields_valid <= request_fields_valid;
				command_changed <= !have_command || requested_command != last_command;
				state <= CHECK;
			end
			CHECK: begin
				// Recheck current identity at dispatch, after any video
				// reset/session transition during the preceding validation.
				if (!reset && fields_valid && command_changed &&
				    (live_identity || reset_recovery_identity)) begin
			...
			WAIT_SNAPSHOT: if (snap_s2 == audio_snapshot_toggle) begin
				snapshot_capture <= audio_snapshot_data;
				state <= CHECK_SNAPSHOT;
			end
			CHECK_SNAPSHOT: begin
				publication_data <= {snapshot_capture[447:40],
				                     AUDIO_ABI_VERSION[7:0], AUDIO_STATUS_MAGIC};
				publication <= (publication == 32'hffffffff) ? 32'd1 : publication + 1'b1;
				clock_epoch <= snapshot_capture[127:64];
				clock_nonce <= snapshot_capture[191:128];
				consumer_identity_valid <= !reset && snapshot_capture[44] &&
				                           snapshot_capture[127:64] != 0 &&
				                           snapshot_capture[191:128] != 0;
				samples_consumed <= snapshot_capture[319:256];
				clock_valid <= !reset && session_active &&
				               session_epoch != 0 && probe_nonce != 0 &&
				               snapshot_capture[44] && snapshot_capture[40] &&
				               snapshot_capture[63:56] == 0 &&
				               snapshot_capture[255:192] != 0 &&
				               snapshot_capture[127:64] == session_epoch &&
				               snapshot_capture[191:128] == probe_nonce &&
				               snapshot_capture[383:320] == session_epoch &&
				               snapshot_capture[447:384] == probe_nonce;
				clock_active <= snapshot_capture[40];
				clock_paused <= snapshot_capture[41];
				consumer_quiescent <= snapshot_capture[44] &&
				                       !snapshot_capture[40] &&
				                       !snapshot_capture[42] &&
				                       !snapshot_capture[43];
				state <= INVALIDATE;
