...
`endif
            ...
            for (ci = 0; ci < 16; ci = ci + 1) begin
                if ((level_pipe_valid[0] && level_pipe_idx == 5'(ci)) ||
                    (level_pipe_valid[1] && ci >= 1 && level_pipe_idx == 5'(ci-1)) ||
                    (level_pipe_valid[2] && ci >= 2 && level_pipe_idx == 5'(ci-2)))
                    level_dbg[ci] <=
                        (level_pipe_value[0] & {16{level_pipe_idx == 5'(ci)}}) |
                        (level_pipe_value[1] & {16{ci >= 1 && level_pipe_idx == 5'(ci-1)}}) |
                        (level_pipe_value[2] & {16{ci >= 2 && level_pipe_idx == 5'(ci-2)}});
            end
`ifdef CAVLC_CYCLE_PROBE
                ...
                if (idx >= tc_r - 5'd1 || zeros_left == 4'd0) begin
                    ...
                    if (!found_a) st <= ST_FAIL;
                        ...
                        if (idx + 5'd1 < tc_r - 5'd1 && zl_a != 4'd0 && bpos_a < bit_len) begin
                            ...
                            found_b = decoded_run_b[9] && bit_len - bpos_a >= rblen_b;
