...
static uint64_t accepted_tick = 0, paired_blocks = 0;
static uint64_t candidate_service = 0, baseline_service = 0;
static std::array<uint64_t,4> placement_checks{};
static std::ofstream cycle_trace;

static void tick(Vh264_cavlc_residual_tb_top& dut) {
    const bool accepted = dut.start && !dut.reset && !dut.busy;
    dut.clk = 0;
    dut.eval();
    dut.clk = 1;
    dut.eval();
    ++cycle_ticks;
    if (accepted) accepted_tick=cycle_ticks;
    if (dut.done && !dut.reset) {
        bool same=(dut.baseline_done || dut.baseline_done_seen) &&
            dut.ok==dut.baseline_ok && dut.bit_offset_end==dut.baseline_end &&
            dut.total_coeff==dut.baseline_tc && dut.trailing_ones==dut.baseline_t1 &&
            dut.total_zeros==dut.baseline_zeros;
        for(int i=0;i<16;++i)
            same=same && dut.coeff[i]==dut.baseline_coeff[i] &&
                dut.level_dbg[i]==dut.baseline_levels[i] && dut.run_dbg[i]==dut.baseline_runs[i];
        if(!same) {
            std::cerr<<"FAIL V12/V13 completion equivalence ok="<<unsigned(dut.ok)<<"/"
                     <<unsigned(dut.baseline_ok)<<" bit_end="<<dut.bit_offset_end<<"/"
                     <<dut.baseline_end<<"\n";
            std::exit(1);
        }
        ++paired_blocks;
        candidate_service+=cycle_ticks-accepted_tick;
        baseline_service+=dut.baseline_cycles;
    }
    ...
    if (dut.placement_stage) ++placement_checks[dut.placement_stage];
}
...
static void check_fast_tables(Vh264_cavlc_residual_tb_top& dut) {
    uint64_t checks = 0;
    auto check_lookup = [&](unsigned got, unsigned expected, const char* kind) {
        if (got != expected) {
            std::cerr << "FAIL fast " << kind << " row=" << unsigned(dut.probe_fast_row)
                      << " window=" << dut.probe_window << " got=" << got
                      << " expected=" << expected << "\n";
            std::exit(1);
        }
        ++checks;
    ...
    for (unsigned row = 0; row < 8; ++row) {
        dut.probe_fast_row = row;
        for (unsigned word = 0; word < 65536; ++word) {
            unsigned expected = 0;
            if (row <= 4) {
                for (unsigned symbol = 0; symbol < (row == 4 ? 20 : 68); ++symbol) {
                    unsigned len = row == 4 ? chroma_dc_len[symbol] : coeff_token_len[row][symbol];
                    unsigned bits = row == 4 ? chroma_dc_bits[symbol] : coeff_token_bits[row][symbol];
                    if (len && word >> (16-len) == bits)
                        expected = ((0x80 | symbol) << 5) | len;
                }
            }
            dut.probe_window = word << 16;
            dut.eval();
            check_lookup(dut.probe_fast_token, expected, "token");
        }
    }
    for (unsigned row = 0; row < 64; ++row) {
        dut.probe_fast_row = row;
        const unsigned tc = row & 31;
        for (unsigned word = 0; word < 512; ++word) {
            unsigned expected = 0;
            if (tc && tc < (row & 32 ? 4u : 16u)) {
                for (unsigned zeros = 0; zeros <= (row & 32 ? 4u : 16u)-tc; ++zeros) {
                    unsigned len = row & 32 ? chroma_tz_len[tc-1][zeros] : total_zeros_len[tc-1][zeros];
                    unsigned bits = row & 32 ? chroma_tz_bits[tc-1][zeros] : total_zeros_bits[tc-1][zeros];
                    if (len && word >> (9-len) == bits)
                        expected = ((16 | zeros) << 5) | len;
                }
            }
            dut.probe_window = word << 23;
            dut.eval();
            check_lookup(dut.probe_fast_zeros, expected, "zeros");
        }
    }
    for (unsigned row = 0; row < 16; ++row) {
        ...
        for (unsigned word = 0; word < 2048; ++word) {
            unsigned expected = 0;
            if (row && row <= 7) {
                for (unsigned run = 0; run < (row == 7 ? 15 : row+1); ++run) {
                    unsigned len = run_len[row-1][run], bits = run_bits[row-1][run];
                    if (len && word >> (11-len) == bits)
                        expected = ((16 | run) << 5) | len;
                }
            }
            dut.probe_window = word << 21;
            dut.eval();
            check_lookup(dut.probe_fast_run, expected, "run");
        }
    }
    std::cout << "CAVLC fast lookup exhaustive PASS checks=" << checks << "\n";
}
#endif

static void check_window_and_level_pipeline(Vh264_cavlc_residual_tb_top& dut) {
    uint64_t windows=0, masks=0, longs=0;
    auto window_check=[&](unsigned pos,unsigned limit) {
        dut.probe_position=pos; dut.probe_limit=limit; dut.eval();
        for(unsigned bit=0;bit<96;++bit) {
            const unsigned p=pos+bit;
            const unsigned want=p<limit && p<1024 ? (dut.rbsp[p/8]>>(7-p%8))&1 : 0;
            const unsigned got=(dut.probe_aligned_window[(95-bit)/32]>>((95-bit)%32))&1;
            if(got!=want) {
                std::cerr<<"FAIL word/align/EOF window pos="<<pos<<" limit="<<limit<<" bit="<<bit<<"\n";
                std::exit(1);
            }
        }
        ++windows;
    };
    uint32_t random=0x42564c43;
    for(unsigned pattern=0;pattern<2;++pattern) {
        for(int i=0;i<128;++i) {
            random=random*1664525u+1013904223u;
            dut.rbsp[i]=pattern ? uint8_t(random>>24) : 0xff;
        }
        for(unsigned pos=0;pos<=1025;++pos)
            for(unsigned limit=0;limit<=1025;++limit) window_check(pos,limit);
    }
    for(int i=0;i<128;++i) dut.rbsp[i]=0;
    for(unsigned source=0;source<1024;++source) {
        dut.rbsp[source/8]=1u<<(7-source%8);
        for(unsigned pos=0;pos<=1024;++pos) window_check(pos,1024);
        dut.rbsp[source/8]=0;
    }
    const unsigned lanes=CAVLC_TEST_LANES<2 ? 1 : CAVLC_TEST_LANES>=3 ? 3 : 2;
    for(unsigned valid=0;valid<8;++valid) for(unsigned idx=0;idx<32;++idx)
        for(unsigned dst=0;dst<16;++dst) for(unsigned basis=0;basis<3;++basis) {
            dut.probe_mask_valid=valid; dut.probe_mask_idx=idx; dut.probe_mask_dst=dst;
            dut.probe_window=basis==0 ? 0xffff : basis==1 ? 0xffff0000 : 0;
            dut.probe_level_code=basis==2 ? 0xffff : 0;
            dut.eval();
            bool enable=false;
            unsigned masked=0,old_data=0;
            for(unsigned lane=0;lane<3;++lane) {
                const bool address=dst>=lane && idx==dst-lane;
                const bool take=lane<lanes && (valid&(1u<<lane)) && address;
                enable|=take;
                if(basis==lane && address) old_data=0xffff;
                if(basis==lane && take) masked=0xffff;
            }
            if(dut.probe_mask_data!=masked || (enable && dut.probe_mask_data!=old_data)) {
                std::cerr<<"FAIL valid-qualified coefficient D-mask equivalence\n"; std::exit(1);
            }
            ++masks;
        }
    for(unsigned pfx=0;pfx<32;++pfx) for(unsigned sl=0;sl<8;++sl)
        for(unsigned first=0;first<2;++first) for(unsigned t1=0;t1<4;++t1) {
            uint32_t base=(pfx<15 ? pfx : 15)<<sl;
            if(pfx>=15 && sl==0) base+=15;
            if(pfx>=16) base+=(1u<<(pfx-3))-4096;
            if(first && t1<3) base+=2;
            const unsigned suffix_bits=pfx==14 && sl==0 ? 4 : pfx<15 ? sl : pfx-3;
            const uint32_t maximum=(1u<<suffix_bits)-1;
            std::vector<uint32_t> suffixes={0,1,2,maximum,maximum?maximum-1:0,
                                          65533u-base,65534u-base,65535u-base,65536u-base,0xffffffffu};
            for(unsigned i=0;i<8;++i) {
                random=random*1664525u+1013904223u;
                suffixes.push_back(random&maximum);
            }
            for(auto suffix:suffixes) {
                dut.probe_long_prefix=pfx; dut.probe_suffix_length=sl;
                dut.probe_level_code=suffix; dut.probe_first=first; dut.probe_t1=t1;
                dut.eval();
                if(dut.probe_long_code!=uint32_t(base+suffix)) {
                    std::cerr<<"FAIL long escape32-bit arithmetic pfx="<<pfx<<" sl="<<sl<<"\n";
                    std::exit(1);
                }
                ++longs;
            }
        }
    std::cout<<"CAVLC cache/mask/long equivalence PASS windows="<<windows
             <<" mask_cases="<<masks<<" long_cases="<<longs<<"\n";
}

static void check_raw_completion_equivalence(Vh264_cavlc_residual_tb_top& dut) {
    uint32_t random=0x2657079f;
    const unsigned maxima[]={0,4,15,16,17,31};
    for(unsigned test=0;test<2048;++test) {
        for(int i=0;i<128;++i) {
            random=random*1664525u+1013904223u; dut.rbsp[i]=random>>24;
        }
        dut.coeff_token_table=test%8; dut.max_coeff=maxima[(test/8)%6];
        dut.bit_len=(test*71)%1026; dut.bit_offset_start=(test*37)%1026;
        dut.start=1; tick(dut); dut.start=0;
        int guard=1000;
        while(!dut.done && --guard) tick(dut);
        if(!guard) { std::cerr<<"FAIL random raw equivalence timeout\n"; std::exit(1); }
        tick(dut);
    }
    std::cout<<"CAVLC raw valid/invalid/available-bit completion equivalence PASS cases=2048\n";
}
...
int main(int argc, char** argv) {
    ...
    dut.reset = 0;
#ifdef CAVLC_FAST_VLC_TEST
    check_fast_tables(dut);
#endif
#ifndef CAVLC_TIMING_BASELINE
    check_timing_helpers(dut);
    check_inline_helpers(dut);
    check_window_and_level_pipeline(dut);
    check_raw_completion_equivalence(dut);
#endif
    validate_tables(dut);
    ...
    int placement_cases = 0;
    for (int maxc : {4,15,16}) {
        for (int tc=1;tc<=maxc;++tc) {
            for (int first=0;first<=maxc-tc;++first) {
                std::array<int,16> c{};
                for (int k=0;k<tc;++k) c[first+k]=k%2 ? -32768 : 32767;
                run_case(dut,"placement-signed16-prefix-boundaries",c,maxc==4?4:0,maxc);
                ++placement_cases;
            }
        }
        for (int low=0;low<maxc;++low) {
            for (int high=low+1;high<maxc;++high) {
                std::array<int,16> c{};
                c[low]=-32768; c[high]=32767;
                run_case(dut,"placement-two-islands",c,maxc==4?4:3,maxc);
                ++placement_cases;
            }
        }
    }
    ...
    if (!placement_checks[1] || !placement_checks[2] || !placement_checks[3]) {
        std::cerr << "FAIL placement register stages were not exercised\n";
        ++failures;
    }
    std::cout << "CAVLC staged placement PASS corner_cases=" << placement_cases
              << " partial4_checks=" << placement_checks[1]
              << " prefix16_checks=" << placement_checks[2]
              << " scatter_checks=" << placement_checks[3] << "\n";
    if (cycle_trace.is_open()) {
        cycle_trace.close();
        if (!cycle_trace) {
    ...
    std::cout << "H264 CAVLC residual Verilator PASS: prefix-free tables checked; roundtrip_cases="
              ...
              << " paired_blocks="<<paired_blocks<<" candidate_service_cycles="<<candidate_service
