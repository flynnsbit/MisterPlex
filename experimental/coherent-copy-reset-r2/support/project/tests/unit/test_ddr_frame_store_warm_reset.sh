...
case "${1:-}" in
  ""|--positive-only|--protocol-only|--osd-epoch-batch) ;;
  *) echo "usage: $0 [--positive-only|--protocol-only|--osd-epoch-batch]" >&2; exit 2 ;;
esac
set +e
...
VERILATOR_BUILD_ROOT="${MPX_WARM_RESET_BUILD_ROOT:-$ROOT/build/verilator}"
BUILD="$VERILATOR_BUILD_ROOT/ddr_frame_store_warm_reset"
FAULT_BUILD="$VERILATOR_BUILD_ROOT/ddr_frame_store_warm_reset_fault"
SCHED_FAULT_BUILD="$VERILATOR_BUILD_ROOT/ddr_frame_store_warm_reset_sched_fault"
FORMAT_FAULT_BUILD="$VERILATOR_BUILD_ROOT/ddr_frame_store_warm_reset_format_fault"
UV_FAULT_BUILD="$VERILATOR_BUILD_ROOT/ddr_frame_store_warm_reset_uv_fault"
CHROMA_VERTICAL_FAULT_BUILD="$VERILATOR_BUILD_ROOT/ddr_frame_store_warm_reset_chroma_vertical_fault"
CHROMA_STRIDE_FAULT_BUILD="$VERILATOR_BUILD_ROOT/ddr_frame_store_warm_reset_chroma_stride_fault"
REFERENCE_BUILD="$VERILATOR_BUILD_ROOT/ddr_frame_store_warm_reset_rejected_r2"
...
run_rejected_reference() {
  if [[ -z "${MPX_REJECTED_R2_RTL:-}" ]]; then
    echo "RTL SIM: no rejected-R2 source supplied; cross-revision negatives were not run." >&2
    return 0
  fi
  printf 'f081ea3a3b1211b441b5786870e1d356b549aa324451be26616e76e879e57dad  %s\n' "$MPX_REJECTED_R2_RTL" |
    sha256sum --check --status
  mkdir -p "$REFERENCE_BUILD"
  "$RUN_VERILATOR" --cc --exe --build \
    --Mdir "$REFERENCE_BUILD" \
    --top-module ddr_frame_store_warm_reset_tb -GSTALE_DOORBELL_FALLBACK_POLLS=256 \
    +define+DDR_FRAME_STORE_TEST_REFERENCE_R2 -Wno-fatal -Wno-WIDTHEXPAND -Wno-WIDTHTRUNC -Wno-SELRANGE -Wno-UNSIGNED \
    -CFLAGS "-std=c++17 -O2" \
    "$ROOT/tests/rtl/ddr_frame_store_warm_reset_tb_top.sv" \
    "$MPX_REJECTED_R2_RTL" "${RTL_SOURCES[@]:1}" \
    "$ROOT/tests/rtl/ddr_frame_store_warm_reset_tb.cpp"
  for selector in picture-cdc picture-replacement; do
    set +e
    REFERENCE_OUT="$("$REFERENCE_BUILD/Vddr_frame_store_warm_reset_tb" "--$selector" 2>&1)"
    REFERENCE_RC=$?
    set -e
    printf '%s\n' "$REFERENCE_OUT"
    if [[ "$REFERENCE_RC" -eq 0 ]]; then
      echo "FAIL DDR protocol reference: immutable R2 unexpectedly passed $selector" >&2
      exit 1
    fi
    if [[ "$selector" == picture-cdc ]]; then
      expected="ownership CDC cleared an active cache"
    else
      expected="accepted replacement lost at queue retirement"
    fi
    if ! grep -Fq "$expected" <<<"$REFERENCE_OUT"; then
      echo "FAIL DDR protocol reference: missing expected $selector failure" >&2
      exit 1
    fi
    echo "OK DDR protocol reference: immutable R2 failed $selector as expected"
  done
}
...
"$RUN_VERILATOR" --cc --exe --build \
  --Mdir "$BUILD" \
  --top-module ddr_frame_store_warm_reset_tb -GSTALE_DOORBELL_FALLBACK_POLLS=256 \
  +define+OSD_RESET_EPOCH_CANDIDATE -Wno-fatal -Wno-WIDTHEXPAND -Wno-WIDTHTRUNC -Wno-SELRANGE -Wno-UNSIGNED \
  -CFLAGS "-std=c++17 -O2 -DOSD_RESET_EPOCH_CANDIDATE" \
  "$ROOT/tests/rtl/ddr_frame_store_warm_reset_tb_top.sv" \
  "${RTL_SOURCES[@]}" \
  "$ROOT/tests/rtl/ddr_frame_store_warm_reset_tb.cpp"
if [[ "${1:-}" == "--protocol-only" ]]; then
  "$BUILD/Vddr_frame_store_warm_reset_tb" --picture-cdc
  "$BUILD/Vddr_frame_store_warm_reset_tb" --picture-replacement
  run_rejected_reference
  exit 0
fi
"$BUILD/Vddr_frame_store_warm_reset_tb"
if [[ "${1:-}" == "--osd-epoch-batch" ]]; then
  "$BUILD/Vddr_frame_store_warm_reset_tb" --osd-reset-epoch 0
  "$BUILD/Vddr_frame_store_warm_reset_tb" --osd-reset-epoch 1
  : "${MPX_OSD_BASELINE_RTL:?immutable ba0 leaf required for the OSD epoch negative}"
  printf '95b0cf66b8f520ebda3203b4bf0f58ce8739a663ec9d2f70bfc7ad3ca06eb363  %s\n' "$MPX_OSD_BASELINE_RTL" |
    sha256sum --check --status
  OSD_REFERENCE_BUILD="$VERILATOR_BUILD_ROOT/ddr_frame_store_osd_ba0"
  mkdir -p "$OSD_REFERENCE_BUILD"
  "$RUN_VERILATOR" --cc --exe --build \
    --Mdir "$OSD_REFERENCE_BUILD" \
    --top-module ddr_frame_store_warm_reset_tb -GSTALE_DOORBELL_FALLBACK_POLLS=256 \
    -Wno-fatal -Wno-WIDTHEXPAND -Wno-WIDTHTRUNC -Wno-SELRANGE -Wno-UNSIGNED \
    -CFLAGS "-std=c++17 -O2" \
    "$ROOT/tests/rtl/ddr_frame_store_warm_reset_tb_top.sv" \
    "$MPX_OSD_BASELINE_RTL" "${RTL_SOURCES[@]:1}" \
    "$ROOT/tests/rtl/ddr_frame_store_warm_reset_tb.cpp"
  "$OSD_REFERENCE_BUILD/Vddr_frame_store_warm_reset_tb" --osd-reset-epoch 0 \
    >"$OSD_REFERENCE_BUILD/phase0.log" 2>&1
  cat "$OSD_REFERENCE_BUILD/phase0.log"
  set +e
  "$OSD_REFERENCE_BUILD/Vddr_frame_store_warm_reset_tb" --osd-reset-epoch 1 \
    >"$OSD_REFERENCE_BUILD/phase1.log" 2>&1
  OSD_REFERENCE_RC=$?
  set -e
  cat "$OSD_REFERENCE_BUILD/phase1.log"
  if [[ "$OSD_REFERENCE_RC" -ne 1 ]] ||
      ! grep -Fq "OSD epoch: stable final status missing after stopped-DDR reset/restart" \
        "$OSD_REFERENCE_BUILD/phase1.log"; then
    echo "FAIL OSD epoch reference: immutable ba0 did not reproduce final-status loss" >&2
    exit 1
  fi
  echo "OK OSD epoch reference: immutable ba0 phase0 control passed; phase1 lost stable final status"
  exit 0
fi
if [[ "${1:-}" == "--positive-only" ]]; then
  exit 0
fi
