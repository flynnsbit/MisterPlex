...
CLOCK_DEFINES=()
COPY_DEFINES=()
case "${NATIVE_COPY_PIPELINE:-0}" in
  0) ;;
  1) COPY_DEFINES=(-DPLEX_NATIVE_COPY_PIPELINE=1) ;;
  *) echo "NATIVE_COPY_PIPELINE must be 0 or 1" >&2; exit 2 ;;
esac
BASE_HZ=720000000
DDR_PERIOD=8
case "${CLOCK_PROFILE:?Select control20, sys85 or sys120 explicitly}" in
  control20) SYS_MHZ=20 ;;
  sys85) SYS_MHZ=85; BASE_HZ=3060000000; DDR_PERIOD=34; CLOCK_DEFINES=(-DPLEX_CLK_SYS_85=1) ;;
  sys120) SYS_MHZ=120; CLOCK_DEFINES=(-DPLEX_CLK_SYS_120=1) ;;
  *) printf 'Unsupported clock profile: %s\n' "$CLOCK_PROFILE" >&2; exit 2 ;;
esac
SYS_PERIOD=$((BASE_HZ / (SYS_MHZ * 1000000)))
...
case "$MODE" in
  ...
  joint-seek)
    STATIC_IDR=0; FILTER=1; TIMING=0; CASES=(feature-textured-pair)
    SEEK_STAGES=4 # This all-I16 fixture cannot exercise the fifth I4-read selector.
    EXTRA=(128 truncated ingress-seek) ;;
  full240-joint)
...
BUILD="$ROOT/build/verilator/clock_${CLOCK_PROFILE}_${LIMIT}_${STATIC_IDR}_${FILTER}_${INJECT}_${TIMING}_${FRAMES}_${ACTIVE_FAULT}_planes${PLANES}_seek${SEEK_STAGES}_copy${NATIVE_COPY_PIPELINE:-0}"
mkdir -p "$BUILD/compiler-scratch"
export TMPDIR="$BUILD/compiler-scratch"
exec 9>"$BUILD/build.lock"
flock 9
printf '{"profile":"%s","SYS_MHz":%s,"DDR_MHz":90,"base_Hz":%s,"SYS_period":%s,"DDR_period":%s,"rising_phase_ticks":0,"event_budget_seconds":1,"SYNC_MB_PLANES":%s}\n' \
  "$CLOCK_PROFILE" "$SYS_MHZ" "$BASE_HZ" "$SYS_PERIOD" "$DDR_PERIOD" "$PLANES" > "$BUILD/clock-binding.json"
SOURCES=()
...
bash "$ROOT/scripts/run_verilator.sh" --cc --exe --build --assert -j 1 \
  ...
  -CFLAGS "-std=c++17 -O2 -DSIM_SYS_MHZ=$SYS_MHZ -DFULL_AU=1 -DFULL_AU_SEEK_STAGES=$SEEK_STAGES -DFULL_AU_FRAMES=$FRAMES -DFULL_AU_ORIGINAL_TIMING=$TIMING -DFULL_AU_JOINT_FILTER=$JOINT -DFULL_AU_RUNTIME_GEOMETRY=1 -DFULL_AU_NATIVE_BEAM=1 -DFULL_AU_SCANDOUBLE=1 -I$ROOT/tests/frozen-host" \
