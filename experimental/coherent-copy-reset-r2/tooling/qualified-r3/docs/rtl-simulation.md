...
Hierarchy profile authority comes from the captured QSF and `inputs.json`, never
the working directory's QSF. The driver supplies both explicitly and writes
`fit-hierarchy.json`, binding the selected profile, exact QSF/manifest hashes,
input-map digest, checker/config/report/log hashes, resource policies, observed
rows and exit status. Publish that receipt, not an independently reconstructed
profile or stale console summary. Existing baseline and FPGA320 floors remain
100000 block bits and 81920 block bits/96 M10Ks respectively.

For report-only calls, the checker recognizes captured inputs beside the fit
report. It prefers `project/Plex.qsf`; after normal local/remote project cleanup,
it can read the exact `Plex.qsf` member of `inputs.tar` **without extracting it**,
checking both the archive hash and QSF hash against the captured manifest.
Unreadable, mismatched, ambiguous or malformed authority is refused, not
replaced with a working-tree default. An explicitly requested QSF never falls
back to another file/archive. Legacy report-only callers with no captured-input
context retain baseline behavior, labeled `legacy-default`; they cannot publish
a captured result receipt.

For a new host-only evaluation of an existing retained slot:

```bash
python3 scripts/check_quartus_fit_hierarchy.py --fit-rpt "$slot/Plex.fit.rpt" \
  --map-rpt "$slot/Plex.map.rpt" --log "$slot/compile.log" \
  --qsf "$slot/project/Plex.qsf" --input-manifest "$slot/inputs.json" \
  --result-json NEW_OUTPUT/hierarchy.json
```

The destination must be new; an existing receipt is never overwritten. A
hierarchy pass is not a timing pass, fitted-database validation, or deployment
approval.

`scripts/run_instrumented_sta.sh ANALYSIS_WORK` is a separately packaged
analysis payload, **not** a grant or ownership/container launcher. A separately
authorized exclusive analysis owner must provide an empty `ANALYSIS_WORK/output`
and a bound `ANALYSIS_WORK/reporter` prepared/verified with the frozen driver's
`prepare_reporting`/`verify_reporter`, and run it from the corresponding fitted
project in the pinned SDK. Its runtime inputs include `sdk-image.id`,
`observation.id`, `timing.tcl`, the SDK producer/parser and their captured
reporter dependencies. SDK/reporter nonzero exits keep their exit receipts;
diagnostic grep no-match is normal, while matches or grep/file/tool errors fail.
Normal completion requires matching before/after RBF hashes. The collector and
both timing/exclusion gates remain separate mandatory checks. Neither a new
payload nor host mock qualification changes an old failed shell outcome or
supplies a missing historical hash receipt.
...
New builds require the v4 timing sidecar (extended evidence v2). Alongside the global top-ten reports,
...
The failed v2 file-only observation format is rejected, not silently upgraded.
Zero-logic, top-level and self-feedback paths are valid: selection never removes
them. The native report header's minimum remains authoritative.
...
collection/source evidence refuses rather than weakening the gate. Observer
callbacks never replace or call the original setter and always return normally.
Their failures are latched and journaled separately, preserving native setter
codes, results and return options even when Quartus catches an HDL error.
Paired setter/read-sdc events, trace-installation/removal checks, a per-transaction
identity, and a final completion marker are mandatory. A failed evidence sink
cannot create a successful observation. The outer reporter fails after `read_sdc`,
not from inside a setter's enter/leave callback.

The R5 source candidate uses `misterplex.timing-reporter.v5` and
`misterplex.timing-observer.v2`. `Plex.execution-events.tsv` is an independent
native-command witness, not a copy of the primary journal: separate callbacks
observe the exact resolved origins of the five exclusion setters and
`read_sdc`. Every occurrence has a contiguous ID, balanced completion, original
command/result and source frame. The witness, primary journal, command inventory,
completion counters and both sets of native-log markers must agree in order.
Trace mutations and observed-command renames during `read_sdc` latch failure,
even if the traces are restored before the final inventory check.

Revision2 recognizes Tcl's accepted nonempty `add`/`remove` and `execution`
prefixes, including mixed spellings and namespace-qualified calls. Literal
prefix comparison does not interpret glob characters. Read-only `info` prefixes
and unrelated variable/command trace operations retain their native behavior.
The original revision's literal-only guard is source-admission blocked: removing
both setter callbacks with `trace r e`, executing a native constraint and
restoring them with `trace a e` could leave two mutually consistent incomplete
journals. Restored registrations alone are not proof of continuous observation.
Existing host regressions execute that omission, check actual native/primary/
witness counters and require both host gates to refuse. They also reject an
actual failed observation summary when a previously valid fixture manifest is
cached; no witness or failure count is synthesized for that adversarial case.
This source correction requires independent re-review and later real-SDK
validation; it does not retroactively qualify the blocked revision's evidence.

The `step_*` completion fields count these native-command occurrences, including
`read_sdc`; they do **not** count Tcl interpreter steps. No `enterstep`,
`leavestep`, source-command trace, setter replacement or constraint replay is
used. Interpreter-step tracing counts imported-command forwarding and can
perturb source `-errorline` metadata. Both native LEAVE callbacks return the
native value with a normal callback return code, preserving it for subsequent
trace observers as well as for the original caller.

Static source sites are possible literal commands, not an expected execution
count. Inactive branches need not execute; loops, procedures and repeated source
inclusion retain every runtime occurrence. Inline, multiline, bracket-nested and
literal namespace-qualified commands bind to a unique file/line/column and
hash-bound complete command. Only Tcl channel newline and backslash-newline
folding is allowed, never whitespace folding inside words. Ambiguous identical
commands on the same line, computed command heads and unsupported source frames
refuse rather than inventing a column or guessing provenance. Unsupported
inactive lexical candidates remain visible to the static audit but cannot bind
an execution. The SDC inventory includes configured roots and additional frozen
SDC files identified by actual executing frames; it is not a claim that every
listed file or branch executed. The exclusion gate rederives the same witness
and checks every occurrence against the supplied frozen sources without
deduplicating it.

The host exclusion CLI reports setter occurrences separately from file-SDC
literal sites. Its validated observation supplies total/file-SDC/HDL occurrence
counts and possible/unique-executed site counts; inactive sites are the difference
between those two site counts, not missing execution evidence. Static policy
inventory is never subtracted from runtime occurrences. Repeated, inline and
inactive commands still undergo the same provenance and policy checks.
This host-only summary can read an older capture without changing its reporter,
input or compiler identity; it is not a new SDK observation or qualification.

R4 observer-v1 traces lack this independent witness and are explicitly rejected;
they cannot be upgraded by filling in missing records. The original R4 tool and
failed evidence remain historical inputs. This is an
**R5_OFFLINE_SOURCE_CANDIDATE_UNQUALIFIED**: host fixtures are not pinned
Quartus17/SDK noninterference, actual error-propagation or physical qualification.
The unchanged SDK-side Python3.5.3 source collector and HDL helper remain separate
from the modern host-only collector. All four hierarchy/exclusion/RBF-ban runtime
policy files remain required. A fresh independently authorized real-SDK
same-netlist comparison is still necessary; no R5 result qualifies the 85MHz RBF.
...
For a callback without an SDC file frame, `hdl_pending` is **not** attribution.
The collector requires its original Tcl command, the native Quartus
`332164/332165/332166` HDL execution context, and exactly one matching literal
`altera_attribute` / `SDC_STATEMENT` in frozen source bytes. It also requires
consistent native `12021/12023` compiler definition file/line and `12128`
elaboration evidence, bound to the retained compile-log hash. An unused matching
source file or the **instantiating** file on a `12128` line is not definition
provenance. Missing or duplicate project definition evidence refuses.
The accepted record
has origin `hdl`, entity, real HDL file/line, source/attribute/statement hashes,
and native log location; no fictitious `.sdc` location or entity whitelist is used.
Resolved decoder-exclusion checks still apply to HDL constraints. Only literal
Verilog/SystemVerilog attributes are currently attributable. Unsupported syntax,
missing frames, ambiguous matches and HDL from an unpinned external SDK library
fail closed **after the real setter has run**.

SDK HDL has a separate, explicit proof method: the native Analysis & Synthesis
**Source Files Read** table must select the source actually used in the netlist.
The immutable reporter's Python3.5-compatible SDK collector reads those
dependencies inside the host-verified pinned image before analysis, recording
SDK-relative paths, file hashes/modes/lengths and literal definition/attribute
coordinates. It never exports SDK source text. Single-string Verilog
concatenations (`{"literal"}`) are supported; computed attributes are not.
The collector rejects missing/duplicate dependencies, malformed or incomplete
native tables, source escape, and ambiguous project/library definitions. The
host audit joins the derived literal statement to the **original** Tcl frame
and native HDL execution messages; there is no module-name whitelist.
Unknown/compiler-generated origins without supported source evidence still
fail. A source-file-table record is not represented as a `12023` definition.

The SDK catalog is runtime output from the hash-bound producer in the pinned
image, not a cryptographic signature or an offline reconstruction of vendor
bytes. Retain its native report and producer/image identities with the final
manifest. Both gates rederive its evidence and reject any post-collection
changes. SDK collection failure or unexpected diagnostics stops the runner.
SDK-origin attribution does not relax decoder exclusions or negative timing.
Host mocks using a real SDK-derived catalog establish source/gate behavior,
**not actual netlist noninterference**.

Normal settled transactions still delete their working `project` directory.
For a separately authorized **local-container** transaction, opt in explicitly:

```bash
MISTERPLEX_ALLOW_LOCAL_FIT=1 scripts/build_rbf.sh --backend local-container \
  NEW_SLOT PROJECT_PATH --retain-project
```

The wrapper forwards the flag unchanged. It keeps the exact local project,
inputs and database on **success and failure**, without moving or publishing
them. `analysis-project.json` records the actual absolute path, last transaction
stage, input/source/image identity, tool/container identity, stage exits, every
retained file's SHA-256/length/mode and the database-file subset. Missing
directories are `missing`; partial/failed compilation is
`incomplete-or-unvalidated`; successful compilation with database files remains
`retained-unvalidated`. **No status claims a complete, loadable fitted netlist.**
Only a separately authorized successful native database load can establish that.
Inventory I/O or identity failure is explicit and never deletes the retained
tree. Unresolved ownership keeps the existing leases and project intact; its
record is `ownership-unresolved`, with no racing inventory of an active project.
Inventory traversal uses descriptor-relative directory enumeration, stat and
no-follow opens; nested enumeration/stat/read errors propagate rather than
silently omitting a subtree. Only verified initial absence is `missing`.
An inventory failure sets `inventory-incomplete` and fails an otherwise
successful transaction. If the build was already failing, that original error
retains precedence and the additional retention failure is reported separately.

This flag is local-only. The remote wrapper still forwards arguments, but the
driver rejects `--retain-project` for a remote backend before dispatch; remote
cleanup/transfer behavior is unchanged. Neither the SDK nor the database is
automatically uploaded or packaged. Use a **new copy** for authorized later STA.
After independent review/publication, same-netlist uninstrumented/instrumented
qualification is PENDING a separately granted V13 transaction that retains its
new database. The absent V12 database is not to be recreated by an extra fit.
Retention itself grants no Quartus operation, promotion or deployment.

The immutable reporter bundle includes its Python collector, observer verifier,
shared STA parser, frozen SDC/HDL source catalog and observation identity, all in
its exact hash map. Local and standalone SSH workers execute that collector in an
isolated, bytecode-disabled Python process, avoiding mutable imports and caches.
The catalog is bound to the full frozen input-file-map digest. Collection itself
requires complete witnessed native exclusion execution, not static-site
cardinality and not just the later exclusion gate.
The runner retains `reporter.log` and refuses swallowed Quartus errors or a
missing completion marker even with reporter exit0. The collector and both gates
independently reject missing, legacy, incomplete or error-tainted observation;
host collection failures persist in `collector-failure.json` and cannot be
cleared by retrying the same transaction. Timing and effective
...
Targeted host tests exercise the production Tcl in an empty child interpreter
using a host `tclsh`, **never a Quartus executable**:

```bash
PYTHONDONTWRITEBYTECODE=1 MISTERPLEX_HOST_TCL=/path/to/tclsh8.6 \
  PYTHONPATH=scripts:tests/unit python3 -B -m unittest \
    test_timing_observer.ObserverPolicy test_quartus_timing.DetailedTiming \
    test_rbf_build.BuildPolicy.test_tcl_observer_and_standalone_bound_collector \
    test_rbf_build.BuildPolicy.test_reporter_runner_preserves_compile_failure_and_rbf_bytes
```

`ObserverPolicy` uses the shared snapshot fixture with IPC disabled. General
`BuildPolicy` tests still allocate their private test semaphores by default:
do not run those selectors under an offline no-IPC grant without an explicitly
IPC-free fixture configuration. Actual SDK catalog selectors are optional and
remain skipped unless separately supplied authorized pinned inputs.

All netlist APIs in that fixture are mocks. It does not open a real timing
netlist, run a fit, or establish hardware acceptance. Set `MISTERPLEX_TEST_WORK`
to a writable project-local directory when testing a sealed source tree. The
sanitized V12 failure fixture retains the original record's SHA-256 and selected
diagnostics; generated HDL/mock RBFs are synthetic test data, not product changes.
