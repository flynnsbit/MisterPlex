...
namespace eval mpx_timing {
    ...
    variable events 0
    variable journal ""
    variable step_journal ""
    variable step_events 0
    variable step_enters 0
    variable step_leaves 0
    variable step_pending {}
    variable witness_read_depth 0
...
proc mpx_timing::step_event {operation id kind code command origin file line raw result} {
    variable step_journal
    variable step_events
    set values [list $step_events $operation $id $kind $code [hex $command] \
                    $origin $file $line [hex $raw] [hex $result]]
    incr step_events
    row $step_journal $values
    flush $step_journal
    puts "MPX_EXECUTION [hex [join $values "\t"]]"
    flush stdout
}
...
proc mpx_timing::status {state} {
    ...
    variable events
    variable step_events
    variable step_enters
    variable step_leaves
    variable step_pending
    set channel [open [file join $directory observer.summary] w]
    set code [catch {
        fconfigure $channel -encoding utf-8 -translation lf
        puts $channel "schema=misterplex.timing-observer.v2"
        ...
        puts $channel "step_events=$step_events"
        puts $channel "step_enters=$step_enters"
        puts $channel "step_leaves=$step_leaves"
        puts $channel "step_pending=[llength $step_pending]"
    } message options]
...
proc mpx_timing::exception_trace {kind command args} {
    ...
    # Tcl passes the current interpreter result to the next LEAVE trace.
    # Return the native value normally so another observer cannot see our "".
    if {[lindex $args end] eq "leave"} {return [lindex $args 1]}
    return
}

proc mpx_timing::observe_step {kind command arguments} {
    variable step_enters
    variable step_leaves
    variable step_pending
    variable source_names
    variable source_table
    variable witness_read_depth
    set operation [lindex $arguments end]
    if {$operation eq "enter"} {
        if {$step_enters >= 2048} {error "Execution step inventory exceeds limit"}
        set id $step_enters
        incr step_enters
        lappend step_pending [list $id $kind $command unknown "" 0 ""]
        if {$kind eq "read_sdc"} {
            incr witness_read_depth
            set origin read_sdc
            set file ""
            set line 0
            set raw ""
        } else {
            if {!$witness_read_depth} {error "Setter execution outside witnessed read_sdc"}
            lassign [source_location $kind] origin file line raw
        }
        if {$origin eq "sdc"} {
            if {![dict exists $source_names $file]} {
                row $source_table [list $file]
                dict set source_names $file 1
            }
        }
        lset step_pending end [list $id $kind $command $origin $file $line $raw]
        step_event enter $id $kind 0 $command $origin $file $line $raw ""
    } elseif {$operation eq "leave"} {
        if {![llength $step_pending]} {error "Unpaired execution step completion"}
        lassign [lindex $step_pending end] id entered_kind entered_command origin file line raw
        set step_pending [lrange $step_pending 0 end-1]
        if {$entered_kind ne $kind || $entered_command ne $command} {
            error "Execution step completion changed command identity"
        }
        incr step_leaves
        set code [lindex $arguments 0]
        step_event leave $id $kind $code $command $origin $file $line $raw [lindex $arguments 1]
        if {$kind eq "read_sdc"} {incr witness_read_depth -1}
        if {$code != 0} {failure original-step "$kind returned code $code"}
    } else {
        error "Unsupported execution step operation"
    }
}

proc mpx_timing::execution_trace {kind command args} {
    variable callback_depth
    variable witness_read_depth
    # A separate trace on the actual native command, not an interpreter step
    # trace (which also counts import forwarding and perturbs source errorline).
    set saved {}
    foreach name {::errorInfo ::errorCode} {
        if {[info exists $name]} {dict set saved $name [set $name]}
    }
    set code [catch {
        if {$kind eq "trace_guard"} {
            set operation [lindex $command 1]
            set trace_type [lindex $command 2]
            # Tcl accepts unique nonempty prefixes; match literal prefixes, not globs.
            if {$witness_read_depth && $operation ne "" && $trace_type ne "" &&
                ([string first $operation add] == 0 || [string first $operation remove] == 0) &&
                [string first $trace_type execution] == 0} {
                error "Execution trace mutation during witnessed read_sdc"
            }
        } elseif {$kind eq "rename_guard"} {
            if {$witness_read_depth} {
                foreach name [lrange $command 1 2] {
                    if {[namespace tail $name] in {set_clock_groups set_false_path set_max_delay
                        set_min_delay set_multicycle_path read_sdc trace rename}} {
                        error "Observed command rename during witnessed read_sdc"
                    }
                }
            }
        } else {
            if {$callback_depth} {error "Reentrant execution witness callback"}
            observe_step $kind $command $args
        }
    } message]
    if {$code} {failure execution-witness $message}
    foreach name {::errorInfo ::errorCode} {
        if {[dict exists $saved $name]} {
            set $name [dict get $saved $name]
        } else {
...
proc mpx_timing::inventory_tables {} {
    ...
    set source_table [open_table Plex.sdc-sources.tsv {file}]
    foreach filename [get_default_sdc_file_names] {
        set name [relative_source $filename]
        if {[dict exists $source_names $name]} {error "Duplicate configured SDC source"}
        row $source_table [list $name]
        dict set source_names $name 1
    }
...
proc mpx_timing::begin_inventory {output_dir} {
    ...
    variable traces
    if {[catch {
        foreach name {observer.summary observer.complete Plex.observer-events.tsv Plex.execution-events.tsv} {
    ...
    foreach kind {set_clock_groups set_false_path set_max_delay set_min_delay set_multicycle_path read_sdc} {
        if {[catch {
            ...
            lappend traces [list $kind $origin $callback {enter leave}]
            set callback [list ::mpx_timing::execution_trace $kind]
            trace add execution $origin {enter leave} $callback
            lappend traces [list $kind $origin $callback {enter leave}]
        } message]} {failure install-trace "$kind: $message"}
    }
    foreach kind {trace rename} {
        if {[catch {
            set origin [namespace origin $kind]
            set callback [list ::mpx_timing::execution_trace "${kind}_guard"]
            trace add execution $origin enter $callback
            lappend traces [list $kind $origin $callback enter]
        } message]} {failure install-continuity-guard $message}
    }
}
...
proc mpx_timing::end_inventory {} {
    ...
    set finished 1
    foreach item $traces {
        lassign $item kind origin callback operations
        if {[catch {
            if {[namespace origin $kind] ne $origin ||
                [lsearch -exact [trace info execution $origin] [list $operations $callback]] < 0} {
    ...
    if {[llength $pending] || $commands != $completions || $read_depth ||
        ...
        [llength $step_pending] || $step_enters != $step_leaves} {
    ...
    if {[catch {step_event end -1 observer 0 "" "" "" 0 "" ""} message]} {
        failure end-step-event $message
    }
    close_tables
...
set result [catch {
    ...
    foreach kind {timing-nodes sdc-sources exception-commands exception-arguments exception-objects observer-events execution-events} {
    ...
    puts $summary "audit_files=[expr {7 + $corner_count * 5}]"
    ...
    puts $summary "misterplex.timing-observer.v2:$mpx_timing::observation_id"
    close $summary
} message]
