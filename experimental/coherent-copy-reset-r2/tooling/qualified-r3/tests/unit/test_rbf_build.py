...
def fake_execution_steps(identity, commands=()):
    """Hand-authored fixture data only, never a replacement for runtime evidence."""
    rows = [(0, "begin", -1, "observer", 0, identity.encode().hex(), "", "", 0, "", "")]
    rows.append((1, "enter", 0, "read_sdc", 0, b"read_sdc".hex(), "read_sdc", "", 0, "", ""))
    for index, (kind, command, file, line, raw) in enumerate(commands, 1):
        for event, result in (("enter", ""), ("leave", "original setter result")):
            rows.append((len(rows), event, index, kind, 0, command.encode().hex(),
                         "sdc", file, line, raw, result.encode().hex()))
    rows.append((len(rows), "leave", 0, "read_sdc", 0, b"read_sdc".hex(), "read_sdc", "", 0, "", ""))
    rows.append((len(rows), "end", -1, "observer", 0, "", "", "", 0, "", ""))
    lines = ["\t".join(map(str, row)) for row in rows]
    return {
        "table": "sequence\tevent\tid\tkind\tcode\tcommand_hex\torigin\tfile\tline\traw_hex\tresult_hex\n" +
                 "\n".join(lines) + "\n",
        "log": "".join("MPX_EXECUTION " + line.encode().hex() + "\n" for line in lines),
        "summary": f"step_events={len(rows)}\nstep_enters={len(commands) + 1}\n"
                   f"step_leaves={len(commands) + 1}\nstep_pending=0\n",
    }
...
def write_fake_extended(timing):
    ...
    (timing / "observer.summary").write_text(
        ...
        "failures=0\nenters=0\nleaves=0\nread_enters=1\nread_leaves=1\npending=0\nevents=4\n" +
        steps["summary"])
    (timing / "observer.complete").write_text(f"misterplex.timing-observer.v2:{identity}\n")
...
class BuildPolicy(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.semaphores = []
        self.ipc_patches = []
        for attribute in ("CONTROLLER_KEY", "EXECUTION_KEY") if self.requires_ipc else ():
            while True:
    ...
    def test_reporter_is_readonly_sidecar_and_covers_clocks_and_corners(self):
        ...
        self.assertEqual(provenance["schema"], "misterplex.timing-reporter.v5")
    ...
    def test_reporter_runner_preserves_compile_failure_and_rbf_bytes(self):
        ...
        reporter.write_text(
            ...
            "if case.name != 'missing-observer': pathlib.Path(sys.argv[3],'observer.complete').write_text('misterplex.timing-observer.v2:'+identity+'\\n')\n"
    ...
    def test_instrumented_runner_preserves_statuses_and_normal_after_hash(self):
        ...
        sta_program = (
            ...
            " (out/'observer.complete').write_text('misterplex.timing-observer.v2:'+identity+'\\n')\n"
