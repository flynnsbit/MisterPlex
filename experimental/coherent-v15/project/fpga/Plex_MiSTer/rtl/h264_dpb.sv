	...
	always @(posedge clk) begin
		...
		if (reset) begin
			...
			if (read_accept) begin
				// Accepted raster context replaces division/modulo in the
				// address-to-RAM-enable cone. Backpressure holds both contexts.
				if (last_issue) begin
					issue_col <= 0; issue_row <= 0;
...
module h264_chroma_epel_block_8x8 #(
	...
	wire [15:0] sum = ax*ay*a + {1'b0,fx}*ay*b +
		ax*{1'b0,fy}*c + {1'b0,fx}*{1'b0,fy}*d + 16'd32;
	assign sample_valid=!reset && busy;
	assign sample_index=index;
	assign sample_value=packed_reference ? a : sum[13:6];
	always @(posedge clk) begin
		done <= 0;
		if (reset) begin busy<=0; index<=0; fx<=0; fy<=0; packed_reference<=0; end
		else if (start && !busy) begin
			busy<=1; index<=0; fx<=frac_x; fy<=frac_y;
			packed_reference<=SYNC_REFERENCE && packed_copy;
		end
		else if (busy) begin
			pred[index] <= sample_value;
