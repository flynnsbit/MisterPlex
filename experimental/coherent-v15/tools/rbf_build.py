...
def prepare_reporting(work, meta):
    ...
    write_json(directory / "provenance.json", {
        "schema": "misterplex.timing-reporter.v1",
...
def verify_reporter(work, meta):
    ...
    if set(provenance["files"]) != {"run.sh", "timing.tcl"}:
...
def verify_timing_reports(work, meta, expected_reporter):
    ...
    write_json(directory / "manifest.json", {"schema": "misterplex.timing-paths.v1",
                                           "reporter": provenance, "rbf_sha256": after, "reports": records,
                                           "timing_acceptance": "NOT_GRANTED"})
...
def validate(root, work, reference):
    ...
    run([sys.executable, root / "scripts/check_quartus_timing.py", "--sta-rpt", work / "Plex.sta.rpt",
         "--paths-dir", work / "timing"])
    ...
    run([sys.executable, root / "scripts/check_timing_exclusions.py",
         "--sta-rpt", work / "Plex.sta.rpt", *sdcs])
