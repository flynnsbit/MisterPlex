#!/usr/bin/env python3
"""Run an existing functional regression under the existing shared guards."""
import ctypes
import datetime
import fcntl
import hashlib
...
def validation_map():
    ...
    return {str(p.relative_to(project)): sha(p)
            for name in ("tests", "host", "scripts")
            for p in sorted((project / name).rglob("*")) if p.is_file()}

def secondary_cohorts():
    cohorts = {}
    for project in sorted((OWN / "baselines").glob("*/project")):
        tree = project / "fpga/Plex_MiSTer"
        sources = {str(p.relative_to(tree)): sha(p)
                   for p in sorted(tree.rglob("*")) if p.is_file()}
        validation = {str(p.relative_to(project)): sha(p)
                      for name in ("tests", "scripts")
                      for p in sorted((project / name).rglob("*")) if p.is_file()}
        cohorts[project.parent.name] = {
            "source_map": sources, "validation_map": validation,
...
def main():
    ...
    before = source_map()
    validation_before = validation_map()
    secondary_before = secondary_cohorts()
    receipt = {
        ...
        "source_map": before,
        "validation_map": validation_before,
        "secondary_cohorts": secondary_before,
        "guard_wrapper_sha256": sha(Path(__file__)),
        "source_sha256": hashlib.sha256(json.dumps(before, sort_keys=True).encode()).hexdigest(),
        "base_source_sha256": "4f09a4176a6b954463e0d6b7484c3d421efdb686421bcf67f7bf254bf34604ef",
    ...
    try:
        ...
        if (OWNER not in resource["permitted_owners"] or
                resource["state"] != "functional-exclusive-acquisition-required"):
            raise RuntimeError("this owner is not permitted to acquire the shared functional slot")
        receipt["registry_sha256"] = sha(registry_path)
        preflight_path = ROOT / "build/resource-preflight.lock"
        preflight_lock = preflight_path.open("r")
        fcntl.flock(preflight_lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        preflight_stat = os.fstat(preflight_lock.fileno())
        receipt["primary_preflight_lock"] = {
            "path": str(preflight_path), "inode": preflight_stat.st_ino,
            "device": preflight_stat.st_dev, "acquired_at": now(),
        }
        save()
        ...
        if validation_map() != validation_before:
            raise RuntimeError("bench/oracle/fixture bytes changed under validation")
        if secondary_cohorts() != secondary_before:
            raise RuntimeError("a frozen baseline cohort changed under validation")
