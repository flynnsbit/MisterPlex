...
#endif
static_assert(SIM_SYS_MHZ == 20 || SIM_SYS_MHZ == 120);
constexpr uint64_t kBaseHz = 720000000;
constexpr uint64_t kSysHz = uint64_t(SIM_SYS_MHZ) * 1000000;
constexpr uint64_t kSysPeriod = kBaseHz / kSysHz;
constexpr uint64_t kDdrPeriod = 8;
constexpr uint64_t kNativeDivisor = SIM_SYS_MHZ / 20;
...
struct Sim {
    struct FrameMetrics {
        uint64_t vcl = 0, promoted = 0, copy = 0, filter_start = 0, filter_done = 0;
    ...
    uint8_t native(uint32_t address) const {
        if (image == 4) {
            ...
            return uint8_t((offset % stride) * 73 + (offset / stride) * 37 + plane * 91);
        }
        ...
        if (ddr_rise) {
            ...
            if (!d.DDRAM_BUSY && d.DDRAM_RD) {
                ...
                for (unsigned i = 0; i < d.DDRAM_BURSTCNT; ++i)
                    responses.push_back({ddr_cycles + 3 + i, load(d.DDRAM_ADDR + i), d.DDRAM_ADDR + i});
            }
            ...
            if (FULL_AU_NATIVE_BEAM && !d.reset) {
                ...
                if (d.native_ce) {
                    if (previous_native_ce_tick)
                        check(time - previous_native_ce_tick == kNativePixelTicks,
...
static void decoded_access_units(Sim& s, const std::string& directory,
    ...
    if (s.ingress_seek) {
        const unsigned stages = (s.d.decoder_plane_state & 128) ? 5 : 3;
    ...
    metric_output << "frame,vcl_cycle,promotion_cycle,copy_accept_cycle,display_cycle,publication_retired_cycle,"
                     ...
                     "SYS_Hz,DDR_Hz,base_Hz,native_divisor,checker_retired_cycle,checker_delay_after_feedback\n";
    ...
    for (unsigned frame = 0; frame < FULL_AU_FRAMES; ++frame) {
        ...
        metric_output << ',' << kSysHz << ",90000000," << kBaseHz << ',' << kNativeDivisor
                      << ',' << s.time / kSysPeriod
                      << ',' << s.time / kSysPeriod - publication_retired_cycle << '\n';
        metric_output.flush();
        check(bool(metric_output), "cannot write measured SYS-cycle metrics");
        std::cout << "METRIC seq=" << frame << " vcl_to_promotion=" << metric.promoted - metric.vcl
