...
int main(int argc,char**argv) {
    ...
    try {
        ...
        const std::string hdc_cancel=std::getenv("P2_HDC_CANCEL")?std::getenv("P2_HDC_CANCEL"):"";
        const unsigned hdc_cancel_stage=std::getenv("P2_HDC_CANCEL_STAGE")?
            std::stoul(std::getenv("P2_HDC_CANCEL_STAGE")):0;
        if((!hdc_cancel.empty()&&hdc_cancel!="reset"&&hdc_cancel!="vcl")||
           hdc_cancel_stage>19||(!hdc_cancel.empty()&&(!chroma_cancel.empty()||!cancel_mode.empty())))
            throw std::runtime_error("invalid or overlapping Hadamard cancellation selector");
        bool hdc_canceled=false,hdc_outstanding=false;
        unsigned hdc_requests=0,hdc_retirements=0,hdc_cancellations=0;
        ...
        auto tick=[&] {
            ...
            d.clk=0;d.eval();
            if(d.reset||cancel_active) {
                chroma_outstanding=false;
                hdc_outstanding=false;
            }
            else {
                if(d.hdc_accept) {
                    if(hdc_outstanding)throw std::runtime_error("duplicate owned Hadamard request");
                    hdc_outstanding=true;++hdc_requests;
                }
                if(d.hdc_retire) {
                    if(!hdc_outstanding)throw std::runtime_error("unowned/stale Hadamard retirement");
                    hdc_outstanding=false;++hdc_retirements;
                }
                if(d.chroma_start) {
        ...
        for(unsigned n=0;n<12000000&&(d.frames_out==frame||(d.native_lease_enabled&&d.busy));++n) {
            if(!hdc_cancel.empty()&&!hdc_canceled&&(d.hdc_busy||d.hdc_done)&&
               d.hdc_stage==hdc_cancel_stage) {
                if(!hdc_outstanding||chroma_outstanding)
                    throw std::runtime_error("Hadamard cancellation missed the sole owned request");
                cancel_active=true;hdc_canceled=true;++hdc_cancellations;
                ...
                if(d.busy||d.hdc_busy||d.hdc_done||d.hdc_valid||d.hdc_accept||
                   d.native_valid||d.present_meta_valid||d.done||d.frames_out!=frame||
                   swaps||pixels||promotions||d.prediction_reference_valid||
                   (hdc_cancel=="vcl"&&d.error!=12))
                    throw std::runtime_error("canceled Hadamard request survived or published");
                std::cout<<"HADAMARD_CANCEL mode="<<hdc_cancel<<" stage="<<hdc_cancel_stage
                         <<" frames="<<d.frames_out<<" requests="<<hdc_requests
                         <<" retirements="<<hdc_retirements<<"\n";
        ...
        if(!hdc_cancel.empty()&&!hdc_canceled)
            throw std::runtime_error("owned Hadamard cancellation was not exercised");
        if(hdc_outstanding||hdc_requests!=hdc_retirements+hdc_cancellations)
            throw std::runtime_error("Hadamard request/retirement/cancellation conservation");
        std::cout<<"HADAMARD_TRANSACTIONS requests="<<hdc_requests<<" retired="<<hdc_retirements
                 <<" canceled="<<hdc_cancellations<<" core_cycles=19\n";
        return 0;
