    ...
    void tick() {
        ...
        if (sys_rise) {
            ...
            if (!FULL_AU) {
                const uint64_t cycle = time / kSysPeriod;
                if (ram_due) {
                    check(cycle - ram_responses.front().accepted >= 1, "native response violated d3 latency");
                ...
                if (ram || painter_request) {
                    ...
                    const unsigned delay = 1 + (ram_stress ? ((ram_random >> 8) % 6) : 0);
                    ...
                    if (painter_request) ++painter_accepted;
                        ...
                        if (ram_last_accept) {
                            ...
                            check(cycle - ram_last_accept >= 1, "native request violated d3 II1");
                        }
...
static void copy_contract(Sim& s) {
    ...
    for (unsigned stress = 0; stress < 2; ++stress) {
        for (unsigned height : {240u, 224u}) {
            ...
            std::cout << "COPY_CONTRACT height=" << height << " visible=" << (height == 224 ? 212 : 240)
                      ...
                      << " sys_hz=" << kSysHz << " latency_min=1 bytes_checked=115200\n";
        }
    }
    s.ram_stress = false;
    ...
    std::cout << "PASS copy contract d3 latency1/II1, delayed cancel/reset/error8, final readback/geometry errors, both banks, painter poisoning\n";
}
