...
//  One complete coded picture within 320x240; full-picture filtering is opt-in.
...
module h264_mb_ctrl #(
	...
	parameter bit SYNC_MB_PLANES = 1'b0,
	parameter bit EXPLICIT_READ_READY = 1'b0
)(
	input  wire        clk,
	input  wire        reset,
	...
	input  wire signed [4:0] slice_alpha_c0_offset, slice_beta_offset,
	...
	localparam int WAIT_MAX = `PLEX_PERF_SYS_CYCLES(4096) - 1;
	localparam int WAIT_W = $clog2(WAIT_MAX + 1);
...
`ifdef CAVLC_WINDOW_BYTES
	localparam int FIRST_CAVLC_WINDOW_BYTES = `CAVLC_WINDOW_BYTES;
`else
	localparam int FIRST_CAVLC_WINDOW_BYTES = 8;
`endif
	// synthesis translate_off
	initial begin
		if (!(FIRST_CAVLC_WINDOW_BYTES == 8 || FIRST_CAVLC_WINDOW_BYTES == 16 ||
		      FIRST_CAVLC_WINDOW_BYTES == 32 || FIRST_CAVLC_WINDOW_BYTES == 64 ||
		      FIRST_CAVLC_WINDOW_BYTES == 128))
			$error("unsupported initial CAVLC window");
	end
	...
	reg signed [4:0] lat_alpha_offset, lat_beta_offset;
	reg abort_issued;
	...
	wire picture_filter_on = ENABLE_FRAME_DEBLOCK && lat_filter_idc != 1;
	wire picture_filter_ok = lat_filter_idc == 1 ||
		(ENABLE_FRAME_DEBLOCK && lat_filter_idc <= 2 &&
		 lat_alpha_offset >= -5'sd12 && lat_alpha_offset <= 5'sd12 && !lat_alpha_offset[0] &&
		 lat_beta_offset >= -5'sd12 && lat_beta_offset <= 5'sd12 && !lat_beta_offset[0]);
	...
	reg [7:0] mb_v [0:63];
	reg [4:0] plane_i;
	reg [1:0] plane_neighbor_step;
	reg plane_initialized;
	wire [127:0] plane_word;
	reg [4:0] plane_read_address;
	wire [3:0] plane_sample_lane = filt_plane==0 ?
		{cmt_i[5:4],cmt_i[1:0]} : {cmt_i[4:3],cmt_i[1:0]};
	wire [7:0] plane_sample = plane_word[plane_sample_lane*8+:8];
	reg [55:0] bottom_u, bottom_v;
	...
	reg signed [18:0] chr_dc_hadamard [0:3];
	reg chr_dc_pending, chr_dc_cb;
	reg [5:0] chr_dc_qp;
	...
	reg [2:0] paint_step;
	wire read_ready = !EXPLICIT_READ_READY || dpb_mem_rready;
	reg [7:0] paint_y_sample, paint_u_sample, paint_v_sample;
	wire [7:0] paint_u_cached, paint_v_cached;
	...
	reg [15:0] paint_last_x, paint_last_y;
	...
	wire [7:0] paint_out_u = paint_step==5 ? paint_u_sample : paint_u_cached;
	wire [7:0] paint_out_v = paint_step==5 ? dpb_mem_rdata : paint_v_cached;
	...
	// At a row end x=width-crop_right-1, so x+321-visible_width
	// is exactly 320+crop_left. Avoid subtracting the crop widths again.
	wire [31:0] paint_read_col = paint_next && x==paint_last_x ?
		32'd320+{16'd0,present_crop_left} : {22'd0,x}+{31'd0,paint_next};
	wire [31:0] paint_raddr = paint_base +
		((paint_step==1 && paint_need_chroma) || paint_step==2 ? 32'd76800+paint_chroma_addr :
		 paint_step==3 || paint_step==4 ? 32'd96000+paint_chroma_addr :
		 {14'd0,paint_y320}+paint_read_col);
	wire        paint_on    = (phase == ST_PAINT);
	wire [9:0] paint_cache_next_x = paint_next ?
		(x == paint_last_x ? present_crop_left[9:0] : x + 10'd1) : x;
	wire [7:0] paint_cache_raddr = paint_cache_next_x[8:1];
	wire paint_u_cache_write = !reset && paint_on && !paint_reset_hold &&
		!paint_swap_hold && paint_step == 3 && dpb_mem_rvalid;
	wire paint_v_cache_write = !reset && paint_on && !paint_reset_hold &&
		!paint_swap_hold && paint_step == 5 && dpb_mem_rvalid;
	wire [7:0] paint_u_ram_data, paint_v_ram_data;
	reg paint_u_forward, paint_v_forward;
	reg [7:0] paint_u_forward_data, paint_v_forward_data;
	reg [159:0] paint_cache_valid;
	line_buf_ram #(.WIDTH(160), .AW(8), .DATA_W(8)) paint_u_cache (
		.wr_clk(clk), .wr_en(paint_u_cache_write), .wr_addr(x[8:1]), .wr_data(dpb_mem_rdata),
		.rd_clk(clk), .rd_addr(paint_cache_raddr), .rd_data(paint_u_ram_data)
	);
	line_buf_ram #(.WIDTH(160), .AW(8), .DATA_W(8)) paint_v_cache (
		.wr_clk(clk), .wr_en(paint_v_cache_write), .wr_addr(x[8:1]), .wr_data(dpb_mem_rdata),
		.rd_clk(clk), .rd_addr(paint_cache_raddr), .rd_data(paint_v_ram_data)
	);
	assign paint_u_cached = paint_u_forward ? paint_u_forward_data : paint_u_ram_data;
	assign paint_v_cached = paint_v_forward ? paint_v_forward_data : paint_v_ram_data;
	// Request the next pixel's chroma alongside its Y read. Explicit forwarding
	// defines same-address read/write behavior without resetting payload RAM.
	always @(posedge clk) begin
		if (reset) begin
			paint_u_forward <= 0; paint_v_forward <= 0;
			paint_cache_valid <= '0;
		end else begin
			paint_u_forward <= paint_u_cache_write && x[8:1] == paint_cache_raddr;
			paint_v_forward <= paint_v_cache_write && x[8:1] == paint_cache_raddr;
			if (paint_u_cache_write) paint_u_forward_data <= dpb_mem_rdata;
			if (paint_v_cache_write) paint_v_forward_data <= dpb_mem_rdata;
			if (dpb_frame_start || (paint_next && x == paint_last_x && y[0]))
				paint_cache_valid <= '0;
			else if (paint_v_cache_write)
				paint_cache_valid[x[8:1]] <= 1'b1;
		end
		// synthesis translate_off
		if (!reset && paint_response && paint_step != 5)
			assert(paint_cache_valid[x[8:1]])
				else $fatal(1, "RGB drain used an unfilled chroma-cache generation");
		// synthesis translate_on
	end
	...
	wire frame_filter_busy, frame_filter_done, frame_filter_error, frame_filter_meta_ready;
	wire frame_filter_we, frame_filter_rd;
	wire [31:0] frame_filter_waddr, frame_filter_raddr;
	wire [7:0] frame_filter_wdata;
	reg frame_filter_start;
	// Cancel on the detecting edge too: a pending DPB promotion must not race
	// the registered transition to ST_STOP on an overlapping VCL.
	wire frame_abort = (phase == ST_STOP && !abort_issued) || (busy && vcl_pulse);
	wire filter_owns_memory = ENABLE_FRAME_DEBLOCK &&
		(phase == ST_FRAME_DEBLOCK || frame_filter_busy);
	wire final_mb_sample = filt_plane == 2 && cmt_i == 63;
	wire commit_meta_ready = !picture_filter_on || !final_mb_sample || frame_filter_meta_ready;
	wire commit_ready = dpb_write_ready && commit_meta_ready;
	assign dpb_mem_we    = filter_owns_memory ? frame_filter_we : dpb_we;
	wire dpb_mem_wready = !filter_owns_memory;
	wire dpb_mem_waccept = dpb_we && dpb_mem_wready;
	assign dpb_mem_waddr = filter_owns_memory ? frame_filter_waddr : dpb_waddr;
	assign dpb_mem_wdata = filter_owns_memory ? frame_filter_wdata : dpb_wdata;
	// ST_PAINT shares the DPB read port and reads the promoted reference bank.
	assign dpb_mem_rd    = filter_owns_memory ? frame_filter_rd :
	                      paint_on ? (!paint_reset_hold && !paint_swap_hold &&
	                                  (paint_step==0 || paint_step==2 || paint_step==4 || paint_next ||
	                                   (paint_step==1 && paint_need_chroma && dpb_mem_rvalid) ||
	                                   (paint_step==3 && dpb_mem_rvalid))) :
	                                 (STATIC_IDR_ONLY ? 1'b0 : dpb_rd);
	assign dpb_mem_raddr = filter_owns_memory ? frame_filter_raddr :
	                      paint_on ? paint_raddr : (STATIC_IDR_ONLY ? 32'd0 : dpb_raddr);
	...
	wire pel_copy = !STATIC_IDR_ONLY && (pdec_mode == 3'd0) && (mv_xw == 16'sd0) && (mv_yw == 16'sd0)
	                && (is_pskip || is_p16);
	wire        store_v   = (phase == ST_COMMIT) && commit_meta_ready;
	...
	wire        store_uv  = 1'b1;
	wire [7:0]  store_s   = SYNC_MB_PLANES ? ((filt_plane==0 || store_uv) ? plane_sample : 8'd128) :
	                       (filt_plane == 2'd0) ? mb_pix[cmt_i] :
	...
	h264_dpb_one_ref #(
		...
		.frame_done(dpb_frm_done && decode_error == 0 && !vcl_pulse),
		.frame_error(dpb_frame_error), .frame_promoted(dpb_frame_promoted),
		.mem_wready(dpb_mem_wready), .mem_wdrained(1'b1),
		.mem_rready(!filter_owns_memory && !paint_on && read_ready),
		...
		.mem_rvalid(!STATIC_IDR_ONLY && !filter_owns_memory && !paint_on && dpb_mem_rvalid),
	...
	genvar b;
	generate if (ENABLE_FRAME_DEBLOCK) begin: g_frame_deblock
		wire [15:0] nonzero;
		for (b=0; b<16; b=b+1) begin: g_nonzero
			assign nonzero[b] = tc_cur[b] != 0;
		end
		// The same-clock BRAM accepts every issued byte at its clock edge.
		// There is no posted write queue; all prior accepted writes are drained.
		h264_deblock_frame #(.STORAGE_W(320), .STORAGE_H(240)) u_filter (
			.clk(clk), .reset(reset),
			.frame_begin(dpb_frame_start), .frame_base(dpb_cur_base),
			.frame_width(present_coded_width), .frame_height(present_coded_height),
			.start(frame_filter_start), .frame_abort(frame_abort),
			.busy(frame_filter_busy), .done(frame_filter_done), .error(frame_filter_error),
			.meta_valid(picture_filter_on && store_v && dpb_write_ready && final_mb_sample),
			.meta_ready(frame_filter_meta_ready), .meta_mb(mb_index),
			.meta_mode(is_intra_mb ? (is_i16 ? 3'd3 : 3'd2) : is_pskip ? 3'd1 : 3'd0),
			.meta_qp(lat_qp), .meta_luma_nonzero(nonzero),
			// Neighbour-MV state changes during luma commit. Use the MV that
			// actually fetched this MB, not the recomputed live predictor.
			.meta_mvx(is_intra_mb ? 16'sd0 : mv_kick_x),
			.meta_mvy(is_intra_mb ? 16'sd0 : mv_kick_y),
			.meta_reference(16'd0), .meta_slice(16'd0),
			.meta_chroma_offset(lat_chroma_offset[4:0]),
			.meta_alpha_offset(lat_alpha_offset), .meta_beta_offset(lat_beta_offset),
			.meta_disable_idc(lat_filter_idc),
			.mem_rd(frame_filter_rd), .mem_rready(filter_owns_memory && read_ready),
			.mem_raddr(frame_filter_raddr),
			.mem_rvalid(filter_owns_memory && dpb_mem_rvalid), .mem_rdata(dpb_mem_rdata),
			.mem_we(frame_filter_we), .mem_wready(filter_owns_memory),
			.mem_waddr(frame_filter_waddr), .mem_wdata(frame_filter_wdata),
			.mem_wdrained(1'b1)
		);
	end else begin: g_no_frame_deblock
		assign frame_filter_busy=1'b0, frame_filter_done=1'b0, frame_filter_error=1'b0;
		assign frame_filter_meta_ready=1'b1, frame_filter_we=1'b0, frame_filter_rd=1'b0;
		assign frame_filter_waddr=32'd0, frame_filter_raddr=32'd0, frame_filter_wdata=8'd0;
	end endgenerate
	reg  mc_start;
	wire mc_done;
	wire mc_y_valid, mc_c_valid, mc_c_v;
	wire [7:0] mc_y_index, mc_y_sample, mc_c_sample;
	wire [5:0] mc_c_index;
	wire mc_reset = reset || (SYNC_MB_PLANES && frame_abort);
	wire mc_stream_owner = SYNC_MB_PLANES && !reset && !frame_abort &&
		phase==ST_DPB_WAIT && mc_hold;
	wire mc_y_take = mc_stream_owner && mc_y_valid;
	wire mc_c_take = mc_stream_owner && mc_c_valid;
	reg [23:0] mc_y_prefix, mc_c_prefix;
	always @(posedge clk) begin
		if (mc_y_take && mc_y_index[1:0]!=3)
			mc_y_prefix[mc_y_index[1:0]*8+:8]<=mc_y_sample;
		if (mc_c_take && mc_c_index[1:0]!=3)
			mc_c_prefix[mc_c_index[1:0]*8+:8]<=mc_c_sample;
	end
	genvar mc_index;
	// Only the legacy parallel MC ports are tied off. The selected RAM path
	// consumes the real DPB response stream and starts after fetch retirement.
	generate
	...
	if (!STATIC_IDR_ONLY) begin: g_inter_mc
	h264_inter_mc_16x16 #(.SYNC_REFERENCE(1'b1)) u_mc (
		.clk(clk), .reset(mc_reset), .start(mc_start),
		.luma_ref_win(mc_luma),
		.chroma_u_ref_win(mc_u),
		.chroma_v_ref_win(mc_v),
		.packed_copy(mc_int_copy),
		.luma_write(dpb_luma_wv), .luma_write_index(dpb_luma_wi), .luma_write_data(dpb_luma_ws),
		.chroma_u_write(dpb_cu_wv), .chroma_v_write(dpb_cv_wv),
		.chroma_write_index(dpb_chroma_wi), .chroma_write_data(dpb_chroma_ws),
		.luma_frac_x(dpb_lfx), .luma_frac_y(dpb_lfy),
		.chroma_frac_x(dpb_cfx), .chroma_frac_y(dpb_cfy),
		.pred_y(mc_pred_y), .pred_u(mc_pred_u), .pred_v(mc_pred_v),
		.pred_y_valid(mc_y_valid), .pred_y_index(mc_y_index), .pred_y_sample(mc_y_sample),
		.pred_c_valid(mc_c_valid), .pred_c_v(mc_c_v),
		.pred_c_index(mc_c_index), .pred_c_sample(mc_c_sample),
		.done(mc_done)
	);
	end else begin: g_no_inter_mc
		assign mc_done=1'b0;
		assign mc_y_valid=0, mc_c_valid=0, mc_c_v=0;
		assign mc_y_index=0, mc_c_index=0, mc_y_sample=0, mc_c_sample=0;
	...
	function automatic signed [31:0] chroma_dc_scale(
		...
		begin
			...
			endcase
			v=value*factor;
			chroma_dc_scale=(v<<<h264_qp_div6(q))>>>1;
		end
	endfunction
	...
	wire [7:0] top_wdata = SYNC_MB_PLANES ? plane_sample : mb_pix[{4'd15, cmt_i[3:0]}];
	...
	function automatic [4:0] plane_block_address(input [1:0] p, input [7:0] index);
		plane_block_address = p==0 ? {1'b0,index[7:6],index[3:2]} :
			(p==1 ? 5'd16 : 5'd20)+{3'd0,index[5],index[2]};
	endfunction
	wire plane_initialize = phase==ST_MB_SETUP && mb_index<mb_count;
	wire plane_reconstruct = phase==ST_RECON_BLK && recon_done;
	wire [4:0] plane_write_address = plane_initialize ? plane_i :
		ly_i>=16 ? ly_i : {1'b0,blk_y,blk_x};
	wire [127:0] plane_recon_word;
	wire [127:0] plane_write_word = plane_initialize ? {16{8'd128}} :
		plane_recon_word;
	genvar plane_lane;
	generate for (plane_lane=0;plane_lane<16;plane_lane=plane_lane+1) begin: g_plane_word
		assign plane_recon_word[plane_lane*8+:8]=recon_blk[plane_lane];
	end endgenerate
	wire [7:0] commit_next_index = !commit_ready ? cmt_i :
		(filt_plane==0 && cmt_i==255) || (filt_plane!=0 && cmt_i==63) ? 8'd0 : cmt_i+8'd1;
	wire [1:0] commit_next_plane = commit_ready &&
		((filt_plane==0 && cmt_i==255) || (filt_plane!=0 && cmt_i==63)) ?
		(filt_plane==2 ? 2'd0 : filt_plane+2'd1) : filt_plane;
	always @* begin
		plane_read_address=0;
		if (phase==ST_COMMIT)
			plane_read_address=plane_block_address(commit_next_plane,commit_next_index);
		else if (phase==ST_FILL && ly_i<24) begin
			if (ly_i>=16) plane_read_address=ly_i;
			else if (!is_intra_mb) plane_read_address={1'b0,scan};
			else if (is_intra_mb && !is_i16 && scan[1:0]!=0)
				plane_read_address={1'b0,scan[3:2],scan[1:0]-2'd1};
		end else if (phase==ST_PRED) begin
			if (ly_i>=16) plane_read_address=ly_i;
			else if (!is_intra_mb) plane_read_address={1'b0,blk_y,blk_x};
			else if (is_intra_mb && !is_i16 && !pred_nb) begin
				case (plane_neighbor_step)
				0: plane_read_address=blk_y!=0 ? {1'b0,blk_y-2'd1,blk_x} : 5'd0;
				1: plane_read_address=blk_y!=0 ?
					{1'b0,blk_y-2'd1,blk_x<3 ? blk_x+2'd1 : blk_x} : 5'd0;
				default: plane_read_address=blk_y!=0 && blk_x!=0 ?
					{1'b0,blk_y-2'd1,blk_x-2'd1} : 5'd0;
				endcase
			end
		end
	end
	genvar plane_row;
	generate if (SYNC_MB_PLANES) begin: g_plane_ram
		reg read_chroma;
		wire [127:0] y_word, c_word;
		wire block_write=!reset && (plane_initialize || plane_reconstruct);
		wire y_packet=mc_y_take && mc_y_index[1:0]==3;
		wire c_packet=mc_c_take && mc_c_index[1:0]==3;
		always @(posedge clk) read_chroma<=plane_read_address[4];
		assign plane_word=read_chroma ? c_word : y_word;
		// Y and U/V outputs can coincide. Separate row banks accept both
		// streams without a write conflict or an extra MC/copy clock.
		for (plane_row=0;plane_row<4;plane_row=plane_row+1) begin: g_row
			line_buf_ram #(.WIDTH(16), .AW(4), .DATA_W(32)) y_memory (
				.wr_clk(clk),
				.wr_en((block_write && !plane_write_address[4]) ||
					(y_packet && mc_y_index[5:4]==plane_row)),
				.wr_addr(block_write ? plane_write_address[3:0] : {mc_y_index[7:6],mc_y_index[3:2]}),
				.wr_data(block_write ? plane_write_word[plane_row*32+:32] : {mc_y_sample,mc_y_prefix}),
				.rd_clk(clk), .rd_addr(plane_read_address[3:0]), .rd_data(y_word[plane_row*32+:32])
			);
			line_buf_ram #(.WIDTH(8), .AW(3), .DATA_W(32)) c_memory (
				.wr_clk(clk),
				.wr_en((block_write && plane_write_address[4]) ||
					(c_packet && mc_c_index[4:3]==plane_row)),
				.wr_addr(block_write ? plane_write_address[2:0] : {mc_c_v,mc_c_index[5],mc_c_index[2]}),
				.wr_data(block_write ? plane_write_word[plane_row*32+:32] : {mc_c_sample,mc_c_prefix}),
				.rd_clk(clk), .rd_addr(plane_read_address[2:0]), .rd_data(c_word[plane_row*32+:32])
			);
		end
	end else begin: g_parallel_planes
		assign plane_word=0;
	end endgenerate
	always @(posedge clk) begin
		if (reset || frame_abort || (phase==ST_IDLE && vcl_pulse)) plane_initialized<=0;
		else if (plane_initialize) plane_initialized<=plane_i==23;
		if (!reset && phase==ST_COMMIT && commit_ready && cmt_i>=56 && cmt_i<63) begin
			if (filt_plane==1) bottom_u[cmt_i[2:0]*8+:8]<=plane_sample;
			if (filt_plane==2) bottom_v[cmt_i[2:0]*8+:8]<=plane_sample;
		end
	end
	// synthesis translate_off
	reg [4:0] plane_read_tag;
	reg [8:0] mc_stream_y_count;
	reg [7:0] mc_stream_c_count;
	always @(posedge clk) begin
		plane_read_tag<=plane_read_address;
		if (reset || frame_abort || mc_start) begin
			mc_stream_y_count<=0;
			mc_stream_c_count<=0;
		end else begin
			if (mc_y_take) mc_stream_y_count<=mc_stream_y_count+1'b1;
			if (mc_c_take) mc_stream_c_count<=mc_stream_c_count+1'b1;
		end
		if (mc_y_take)
			assert (mc_stream_y_count<256 && mc_y_index==mc_stream_y_count)
				else $error("MC luma workspace stream lost order or generation");
		if (mc_c_take)
			assert (mc_stream_c_count<128 && mc_c_index==mc_stream_c_count[5:0] &&
				mc_c_v==mc_stream_c_count[6])
				else $error("MC chroma workspace stream lost order or generation");
		if (mc_stream_owner && mc_done)
			assert (mc_stream_y_count==256 && mc_stream_c_count==128)
				else $error("MC completion preceded accepted workspace writes");
		if (!reset && SYNC_MB_PLANES)
			assert (!((plane_initialize || plane_reconstruct) && (mc_y_take || mc_c_take)))
				else $error("overlapping macroblock and MC RAM writers");
		if (!reset && SYNC_MB_PLANES && phase==ST_COMMIT) begin
			assert (plane_initialized) else $error("uninitialized macroblock plane publication");
			assert (plane_read_tag==plane_block_address(filt_plane,cmt_i))
				else $error("macroblock commit consumed an unowned RAM response");
			assert (plane_sample == (filt_plane==0 ? mb_pix[cmt_i] :
				filt_plane==1 ? mb_u[cmt_i[5:0]] : mb_v[cmt_i[5:0]]))
				else $error("scheduled macroblock plane differs from parallel datapath");
		end
		if (!reset && SYNC_MB_PLANES && phase==ST_PRED && !is_intra_mb)
			assert (plane_initialized && plane_read_tag==(ly_i>=16 ? ly_i : {1'b0,blk_y,blk_x}))
				else $error("inter prediction consumed an unowned RAM response");
	end
	// synthesis translate_on

	// One packed block per word: reset-free payload ports, with uncoded-block
	// validity kept outside RAM. ST_FILL primes the whole signed16 record.
	...
	wire [255:0] coeff_word, coeff_word_q;
	line_buf_ram #(.WIDTH(24), .AW(5), .DATA_W(256)) coeff_ram (
		.wr_clk(clk), .wr_en(coeff_store), .wr_addr(coeff_store_addr), .wr_data(coeff_word),
		.rd_clk(clk), .rd_addr(ly_i<24 ? ly_i : 5'd0), .rd_data(coeff_word_q)
	);
	genvar coeff_bank;
	generate for (coeff_bank=0;coeff_bank<16;coeff_bank=coeff_bank+1) begin: g_coeff_bank
		assign coeff_word[coeff_bank*16+:16] = cav_coeff[coeff_bank];
		assign recon_cavlc_coeff[coeff_bank] = recon_from_mb ?
			(coeff_read_valid ? $signed(coeff_word_q[coeff_bank*16+:16]) : 16'sd0) :
			cavlc_coeff[coeff_bank];
	end endgenerate
	always @(posedge clk) begin
	...
	// Each complete bottom row is committed as one word on its final accepted
	// sample. NLOAD0 primes RAM; NLOAD1 owns/latches it for the whole MB, so
	// overwriting this column cannot corrupt the next MB's top-left sample.
	wire [63:0] chroma_top_u_word, chroma_top_v_word, chroma_top_u_q, chroma_top_v_q;
	reg [63:0] chroma_above_u, chroma_above_v;
	wire chroma_top_store = !reset && phase==ST_COMMIT && dpb_write_ready && cmt_i==63;
	line_buf_ram #(.WIDTH(CHR_MB_W), .AW(5), .DATA_W(64)) chroma_top_u_ram (
		.wr_clk(clk), .wr_en(chroma_top_store && filt_plane==1),
		.wr_addr(lat_mb_x[4:0]), .wr_data(chroma_top_u_word),
		.rd_clk(clk), .rd_addr(lat_mb_x[4:0]), .rd_data(chroma_top_u_q)
	);
	line_buf_ram #(.WIDTH(CHR_MB_W), .AW(5), .DATA_W(64)) chroma_top_v_ram (
		.wr_clk(clk), .wr_en(chroma_top_store && filt_plane==2),
		.wr_addr(lat_mb_x[4:0]), .wr_data(chroma_top_v_word),
		.rd_clk(clk), .rd_addr(lat_mb_x[4:0]), .rd_data(chroma_top_v_q)
	);
	always @(posedge clk) begin
		if (!reset && phase==ST_NLOAD && nload_i==1) begin
			chroma_above_u<=chroma_top_u_q;
			chroma_above_v<=chroma_top_v_q;
		end
	end
	genvar chroma_lane;
	generate for (chroma_lane=0;chroma_lane<8;chroma_lane=chroma_lane+1) begin: g_chroma_top
		if (chroma_lane==7) begin: g_last
			assign chroma_top_u_word[chroma_lane*8+:8]=SYNC_MB_PLANES ? plane_sample : mb_u[63];
			assign chroma_top_v_word[chroma_lane*8+:8]=SYNC_MB_PLANES ? plane_sample : mb_v[63];
		end else begin: g_saved
			assign chroma_top_u_word[chroma_lane*8+:8]=SYNC_MB_PLANES ? bottom_u[chroma_lane*8+:8] : mb_u[56+chroma_lane];
			assign chroma_top_v_word[chroma_lane*8+:8]=SYNC_MB_PLANES ? bottom_v[chroma_lane*8+:8] : mb_v[56+chroma_lane];
		end
		assign chr_above_u[chroma_lane]=has_above ? chroma_above_u[chroma_lane*8+:8] : 8'd128;
		assign chr_above_v[chroma_lane]=has_above ? chroma_above_v[chroma_lane*8+:8] : 8'd128;
	end endgenerate
	...
	always @(posedge clk) begin
		...
		if (reset) begin
			paint_last_x <= 0; paint_last_y <= 0;
			phase           <= ST_IDLE;
			plane_i <= 0;
			plane_neighbor_step <= 0;
			decode_error    <= 0;
			recon_use_dc    <= 0;
			recon_from_mb   <= 0;
			recon_dc_value  <= 0;
			...
			if (chr_dc_pending) begin
				for (coeff_i=0;coeff_i<4;coeff_i=coeff_i+1) begin
					if (chr_dc_cb)
						chr_dc_u[coeff_i] <= chroma_dc_scale(chr_dc_hadamard[coeff_i], chr_dc_qp);
					else
						chr_dc_v[coeff_i] <= chroma_dc_scale(chr_dc_hadamard[coeff_i], chr_dc_qp);
				end
			end
			case (phase)
			ST_IDLE: begin
			...
			ST_WAIT_MB0: begin
				...
				if (slice_error || (rbsp_done && wait_cnt==0)) begin
					...
					lat_alpha_offset <= slice_alpha_c0_offset;
					lat_beta_offset <= slice_beta_offset;
			...
			ST_WAIT_RBSP: begin
				...
				if (slice_error) begin
					...
					if (!picture_filter_ok || first_mb != 0 ||
			...
			ST_MB_SETUP: begin
				if (mb_index >= mb_count) begin
					...
					// synopsys translate_on
				end else if (SYNC_MB_PLANES && plane_i<23) begin
					plane_i<=plane_i+5'd1;
				end else begin
					plane_i<=0;
					lat_mb_x  <= next_mb_x;
					lat_mb_y  <= next_mb_y;
			...
			ST_RES_DEC: begin
				if (!STATIC_IDR_ONLY && is_pskip) begin
					...
					win_i         <= 7'd0;
					win_count     <= 8'(FIRST_CAVLC_WINDOW_BYTES);
					win_hold      <= 2'd0;
			...
			ST_PRED: begin
				if (ly_i >= 16) begin
					...
					// left, above, above-right, then top-left.
					case (plane_neighbor_step)
					0: for (coeff_i=0;coeff_i<4;coeff_i=coeff_i+1)
						left4p[coeff_i]<=blk_x==0 ? left16[{blk_y,coeff_i[1:0]}] :
							plane_word[(coeff_i*4+3)*8+:8];
					1: for (coeff_i=0;coeff_i<4;coeff_i=coeff_i+1) begin
						above4[coeff_i]<=blk_y==0 ? above16[{blk_x,coeff_i[1:0]}] :
							plane_word[(12+coeff_i)*8+:8];
						above4[coeff_i+4]<=blk_y==0 ?
							(blk_x==3 && lat_constrained_intra &&
							 (lat_mb_x+1>=grid_w || (!STATIC_IDR_ONLY && mv_row_v[lat_mb_x+1])) ?
							 above16[15] : above16[blk_x*4+coeff_i+4]) : plane_word[120+:8];
					end
					2: if (blk_y!=0 && blk_x<3 && blk_scan({blk_y-2'd1,blk_x+2'd1})<ly_i)
						for (coeff_i=0;coeff_i<4;coeff_i=coeff_i+1)
							above4[coeff_i+4]<=plane_word[(12+coeff_i)*8+:8];
					3: begin
						i4_tl <= (blk_x==0 && blk_y==0) ? top_left_pix :
							blk_x==0 ? left16[{blk_y-2'd1,2'd3}] :
							blk_y==0 ? above16[{blk_x-2'd1,2'd3}] : plane_word[120+:8];
						pred_nb<=1;
					end
					endcase
					plane_neighbor_step<=plane_neighbor_step+2'd1;
					...
					pred_nb <= 1'b0;
					// Packed zero-MV and filtered MC now retire through the
					// same prediction output; no parallel reference-window mux.
					for (coeff_i = 0; coeff_i < 16; coeff_i = coeff_i + 1)
						pred[coeff_i] <= SYNC_MB_PLANES ? plane_word[coeff_i*8+:8] :
							mc_pred_y[{blk_y, coeff_i[3:2], blk_x, coeff_i[1:0]}];
					recon_start <= 1'b1;
					phase       <= ST_KICK_BLK;
				end
			end
			...
			ST_DB_LD: if (STATIC_IDR_ONLY || SYNC_MB_PLANES) begin
				decode_error<=8'd1;
				phase<=ST_STOP;
			end else begin
			...
			ST_DB_WR: if (STATIC_IDR_ONLY || SYNC_MB_PLANES) begin
				decode_error<=8'd1;
				phase<=ST_STOP;
			end else begin
				for (di = 0; di < 4; di = di + 1) begin
			...
			ST_COMMIT: if (commit_ready) begin
				// synopsys translate_off
				...
				if (filt_plane == 2'd0) begin
					filt_idx <= cmt_i;
					filt_s   <= SYNC_MB_PLANES ? plane_sample : mb_pix[cmt_i];
					if (cmt_i[3:0] == 4'd15)
						left_col[cmt_i[7:4]] <= SYNC_MB_PLANES ? plane_sample : mb_pix[{cmt_i[7:4], 4'd15}];
					// top_line write is the M10K posedge port (top_we/top_waddr/top_wdata).
				end else begin
					if (filt_plane==1) begin
						if (cmt_i[2:0]==7) chr_left_u[cmt_i[5:3]]<=SYNC_MB_PLANES ? plane_sample : mb_u[cmt_i[5:0]];
					end else begin
						if (cmt_i[2:0]==7) chr_left_v[cmt_i[5:3]]<=SYNC_MB_PLANES ? plane_sample : mb_v[cmt_i[5:0]];
					end
					filt_idx <= {2'd0, cmt_i[5:0]};
					if (store_uv && (filt_plane == 2'd1))
						filt_s <= SYNC_MB_PLANES ? plane_sample : mb_u[cmt_i[5:0]];
					else if (store_uv && (filt_plane == 2'd2))
						filt_s <= SYNC_MB_PLANES ? plane_sample : mb_v[cmt_i[5:0]];
					else
						filt_s <= 8'd128;
				end
			...
			ST_DPB_WAIT: if (STATIC_IDR_ONLY) begin
				...
				if (dpb_fetch_err && !dpb_kick) begin
					mc_start <= 1'b0;
					decode_error <= 8'd9;
					phase <= ST_STOP;
				end else if (dpb_fetch_done && !dpb_kick) begin
					// The last response must reach RAM before start. Packed
					// zero-MV reads bypass interpolation, not RAM retirement.
					if (!mc_hold) begin
						mc_hold  <= 1'b1;
						mc_start <= 1'b1;
					end else if (mc_done) begin
					mc_hold <= 1'b0;
					// synopsys translate_off
					if ((mb_index <= 16'd2) || (mb_index == 16'd40) || (mb_index == 16'd41) || (mb_index == 16'd61))
						$display("SKIP_MC mb=%0d xy=%0d,%0d origin=%0d,%0d pred00=%0d pack00=%0d pack_2_10=%0d pel_copy=%0d after_db=%0d pskip=%0d p16=%0d",
						         ...
						         mc_pred_y[0], mc_pred_y[0], mc_pred_y[10*16+2],
					...
					for (pi = 0; pi < 64; pi = pi + 1) begin
						mb_u[pi] <= mc_pred_u[pi];
						mb_v[pi] <= mc_pred_v[pi];
					end
			...
			ST_STOP: begin
				...
				// synopsys translate_on
				if (abort_issued && !frame_filter_busy && !dpb_fetch_busy) begin
					vcl_lock   <= 1'b0;
					phase      <= ST_IDLE;
					busy       <= 1'b0;
					done       <= 1'b0;
			...
			ST_PAINT: begin
				...
				if (paint_reset_hold) begin
					...
					paint_hold<=0;
					case(paint_step)
					0:if(read_ready)paint_step<=1;
					1:if(dpb_mem_rvalid)begin
						paint_y_sample<=dpb_mem_rdata;
						if(paint_need_chroma)paint_step<=read_ready ? 3 : 2;
					...
					2:if(read_ready)paint_step<=3;
					3:if(dpb_mem_rvalid)begin
						paint_u_sample<=dpb_mem_rdata;
						paint_step<=read_ready ? 5 : 4;
					end
					4:if(read_ready)paint_step<=5;
			...
			ST_PUBLISH: begin
				if(dpb_frame_promoted) begin
					paint_last_x<=present_coded_width-present_crop_right-1'b1;
					paint_last_y<=present_coded_height-present_crop_bottom-1'b1;
					native_complete<=1;
			...
			ST_TAIL: if (br_syn_done) begin
				if (br_syn_ok && br_ue == (16'd1 << (tail_bits-1'b1))) begin
					if (picture_filter_on) begin
						frame_filter_start<=1;
						phase<=ST_FRAME_DEBLOCK;
					end else begin
						frm_done_hold<=1;
						phase<=ST_PUBLISH;
					end
			...
			ST_FRAME_DEBLOCK: begin
				if (frame_filter_error) begin decode_error<=8'd21; phase<=ST_STOP; end
				else if (frame_filter_done) begin
					frm_done_hold<=1;
					phase<=ST_PUBLISH;
				end
			end
			...
			endcase
			if (busy && vcl_pulse) begin
				decode_error<=8'd12; phase<=ST_STOP;
				abort_issued<=1;
				frame_filter_start<=0;
				dpb_frm_done<=0; frm_done_hold<=0;
				native_picture_valid<=0; present_meta_valid<=0;
				paint_pixel_valid<=0; wr_reset_ptr<=0; swap_req<=0;
			end
			else if (busy && dpb_opened && !dpb_frame_start && !dpb_idr_start &&
			         phase!=ST_STOP && dpb_frame_error) begin
				decode_error<=8'd13; phase<=ST_STOP;
			end else if (busy && dpb_opened && !dpb_frame_start &&
			             phase!=ST_STOP && frame_filter_error) begin
				decode_error<=8'd21; phase<=ST_STOP;
			end
		end
	end
