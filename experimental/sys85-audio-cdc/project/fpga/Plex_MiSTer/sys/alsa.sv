...
always @(posedge clk) begin
	...
	(* async_reg = "true", preserve *) reg ctrl_s1 = 0, ctrl_s2 = 0;
	(* async_reg = "true", preserve *) reg snapshot_s1 = 0, snapshot_s2 = 0;
	reg ctrl_ack = 0, snapshot_ack = 0;
	...
	always @(posedge clk) begin
		...
		// Reset-time rejection uses the same destination capture as every
		// other command; no raw bundled payload bypasses this register.
		if (!pending && ctrl_s2 != ctrl_ack) begin
		...
		if (reset || reset_draining) begin
			...
			if (pending && (reset || cmd_opcode != 8'd4)) begin
				ack_epoch <= cmd_epoch;
				ack_nonce <= cmd_nonce;
				ack_token <= cmd_token;
				ack_opcode <= cmd_opcode;
