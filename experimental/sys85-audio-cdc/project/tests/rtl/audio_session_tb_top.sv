module audio_session_tb_top #(
	parameter AUDIO_CLOCK_RATE = 48000 * 128,
	parameter TEST_CDC_DATA_DELAY = 0
) (
	...
	output wire [216:0] command_bus,
	output wire [447:0] snapshot_bus,
	output wire observed_snapshot_ack,
	output wire [3:0] mailbox_state,
	...
	reg [216:0] delayed_command = 0;
	reg [447:0] delayed_snapshot = 0;
	always @(posedge clk_audio) delayed_command <= ctrl_data;
	always @(posedge clk_sys) delayed_snapshot <= snapshot_data;
	assign command_bus = ctrl_data;
	assign snapshot_bus = snapshot_data;
	assign observed_snapshot_ack = snapshot_ack;
	assign mailbox_state = mailbox.state;
	...
	audio_session_mailbox #(.ENABLE(1), .POLL_CYCLES(7)) mailbox (
		...
		.audio_snapshot_data(TEST_CDC_DATA_DELAY ? delayed_snapshot : snapshot_data), .clock_epoch(), .clock_nonce(),
	...
	alsa #(.CLK_RATE(AUDIO_CLOCK_RATE), .SESSION_CONTROL(1)) dut (
		...
		.audio_ctrl_data(TEST_CDC_DATA_DELAY ? delayed_command : ctrl_data), .audio_ctrl_ack_toggle(ctrl_ack),
		.audio_snapshot_toggle(snapshot_toggle), .audio_snapshot_ack_toggle(snapshot_ack),
