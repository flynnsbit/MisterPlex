...
void testCaps() {
    ...
    require(result.abi_version == abi::kFpgaVideoAbiVersion &&
            ...
            result.max_width == TEST_MAX_WIDTH && result.max_height == TEST_MAX_HEIGHT,
            "RTL capability fields differ from host contract/default parameters");
    constexpr bool supportsVideo = AU_MODE &&
        ((TEST_VIDEO_FEATURES & ring::kRequiredVideoFeatures) == ring::kRequiredVideoFeatures);
    require(result.features == (AU_MODE ? TEST_VIDEO_FEATURES : 0) &&
            result.build_id == (AU_MODE ? TEST_VIDEO_BUILD_ID : 0u) &&
            result.supportsVideo() == supportsVideo,
            "physical capability publication or host admission differs from selected profile");
    s.run(500);
    ...
    std::cout << "PASS caps ABI addresses/magics/order, nonce, profile flags; host video admission="
              << supportsVideo << "\n";
}
...
void testRejected() {
    ...
    reject("undefined event10", [](Bytes& b) { b[4] = 10; });
...
void testLocalFlushCancellation() {
    const Bytes encoded(96, 0x55);
    for (unsigned stage = 0; stage < 5; ++stage) {
        Sim s;
        s.boot(ring::kCountMask - 140); s.probe(); s.begin();
        s.dut.decoder_idle = 0;
        s.dut.sink_read = 0;
        const Bytes pending = accessUnit(encoded);
        if (stage == 0) {
            s.append(Bytes(pending.begin(), pending.begin() + 13));
            s.until([&] { return s.dut.fpga_read_count == s.wr; },
                    "local-cancel partial-header setup failed");
        } else if (stage == 1) {
            s.dut.au_ready = 0;
            s.append(pending);
            s.until([&] { return s.dut.au_valid; },
                    "local-cancel held-metadata setup failed");
        } else if (stage == 2) {
            s.append(pending);
            s.until([&] { return s.accepted.size() == 8 && s.dut.out_valid && s.dut.out_full; },
                    "local-cancel held-byte setup failed");
        } else if (stage == 3) {
            s.append(Bytes(pending.begin(), pending.begin() + 67));
            s.until([&] { return s.accepted.size() == 3; },
                    "local-cancel fragmented-payload setup failed");
        } else {
            s.append(record(ring::Event::Pause, session)); s.ack();
            s.append(Bytes(pending.begin(), pending.begin() + 40));
            s.run(1000);
            require(s.accepted.empty(), "paused local-cancel setup consumed queued AU");
        }
        const size_t beforeMetadata = s.metadata.size();
        // These newly published bytes have not appeared in a reader CTRL
        // response. Cancellation must use a fresh producer sample.
        s.append(Bytes(19, 0x77));
        const uint32_t cancelledThrough = s.wr;
        const unsigned beforeFlush = s.flushes;
        s.dut.flush = 1;
        s.tick();
        s.dut.flush = 0;
        s.until([&] { return s.dut.reset_pending && s.flushes > beforeFlush; },
                "local flush failed to cancel blocked parser stage " + std::to_string(stage),
                5000);
        s.run(200);
        require(!s.dut.out_valid && !s.dut.au_valid && s.dut.reset_pending &&
                s.dut.video_session_id == session && s.dut.video_nonce == nonce &&
                s.publishedRead() != cancelledThrough,
                "local flush emitted cancelled data or bypassed the consumer fence");
        s.until([&] { return s.dut.transport_quiescent; },
                "local flush abandoned an accepted DDR command");
        s.dut.sink_read = 1;
        s.dut.au_ready = 1;
        s.dut.decoder_idle = 1;
        s.until([&] {
            ring::Status status;
            return s.ddr.readStatus(status) && !status.active &&
                   status.reset_epoch == s.epoch &&
                   s.publishedRead() == cancelledThrough && !s.dut.reset_pending;
        }, "local cancellation did not acknowledge the freshly sampled producer");
        require(s.dut.video_nonce == 0 && s.dut.video_session_id == 0 &&
                s.drained.empty() && !s.fatal(),
                "local cancellation retained stale identity or replayed discarded bytes");
        const uint64_t freshSession = session + 100 + stage;
        s.probe(nonce + 700 + stage); s.begin(true, freshSession);
        const Bytes fresh{0, 0, 1, 0x65, 0x88, 0x80};
        s.append(accessUnit(fresh, 0, freshSession));
        s.append(record(ring::Event::Drain, freshSession));
        s.until([&] { return s.drained.size() == fresh.size(); },
                "fresh AU after local cancellation did not recover");
        s.ack();
        require(s.drained == fresh && s.metadata.size() == beforeMetadata + 1 &&
                s.metadata.back().session_id == freshSession &&
                int64_t(s.metadata.back().pts) == -9001,
                "seek epoch leaked old bytes/metadata or changed original PTS");
    }
    std::cout << "PASS local flush: partial header/held metadata/held byte/fragmented payload/"
                 "paused lookahead -> fresh CTRL, actual retirement, fresh Probe/Begin/PTS\n";
}

void testLocalFlushOwnedCommands() {
    {
        Sim s;
        s.boot(); s.probe(); s.begin();
        s.until([&] { return s.pending.valid && s.pending.delay > 4; },
                "no accepted CTRL read for local cancellation");
        const unsigned reads = s.reads;
        s.pending.delay = 100;
        s.append(Bytes(19, 0x77));
        s.dut.decoder_idle = 0;
        s.dut.enable = 0;
        s.dut.flush = 1; s.tick(); s.dut.flush = 0;
        s.run(25);
        require(s.pending.valid && s.reads == reads && s.dut.reset_pending &&
                !s.dut.transport_quiescent && s.dut.video_nonce == nonce,
                "local cancellation lost an accepted old-CTRL response");
        s.run(200);
        require(!s.pending.valid && s.reads == reads + 1 &&
                s.dut.transport_quiescent && s.dut.reset_pending &&
                s.publishedRead() != s.wr,
                "local cancellation reused a pre-flush CTRL sample or bypassed retirement");
        s.dut.decoder_idle = 1;
        s.until([&] { return s.ddr.resetAcknowledged(s.wr, s.epoch); },
                "disabled source suppressed local cancellation ACK");
        s.run(200);
        require(!s.dut.DDRAM_RD && !s.dut.DDRAM_WE && !s.pending.valid &&
                !s.dut.active && s.dut.video_nonce == 0,
                "disabled source did not stop after final cancellation telemetry");
        s.dut.enable = 1;
        s.probe(nonce + 800); s.begin();
    }
    for (unsigned published : {2u, 8u}) {
        Sim s;
        s.boot();
        s.append(record(ring::Event::Probe, nonce, abi::kFpgaVideoLayoutId));
        s.until([&] { return s.capsWrites.size() == published; },
                "no partial capability publication for local cancellation");
        s.forceBusy = true;
        s.until([&] { return s.dut.DDRAM_WE; },
                "no BUSY-held capability write for local cancellation");
        const auto address = s.dut.DDRAM_ADDR;
        const auto data = s.dut.DDRAM_DIN;
        s.dut.decoder_idle = 0;
        s.dut.enable = 0;
        s.dut.flush = 1; s.tick(); s.dut.flush = 0;
        s.run(40);
        require(s.dut.DDRAM_WE && s.dut.DDRAM_ADDR == address &&
                s.dut.DDRAM_DIN == data && s.capsWrites.size() == published &&
                !s.dut.transport_quiescent && s.dut.video_nonce == 0,
                "local flush abandoned/changed an owned capability write");
        s.forceBusy = false;
        s.run(300);
        require(s.capsWrites.size() == published + 1 &&
                s.dut.video_nonce == 0 && s.dut.transport_quiescent &&
                s.dut.reset_pending && s.publishedRead() != s.wr,
                "cancelled Probe continued publication or bypassed the actual consumer fence");
        s.dut.decoder_idle = 1;
        s.until([&] { return s.ddr.resetAcknowledged(s.wr, s.epoch); },
                "cancelled Probe did not acknowledge with source disabled");
        s.run(300);
        require(s.capsWrites.size() == published + 1 && s.dut.video_nonce == 0,
                "retired cancellation resurrected an incomplete capability publication");
        ring::VideoCapabilities caps;
        const uint64_t fresh = nonce + 900 + published;
        require(!s.ddr.capabilitiesFor(fresh, caps), "cancelled DDR residue qualified a fresh nonce");
        s.dut.enable = 1;
        s.probe(fresh); s.begin();
    }
    std::cout << "PASS local cancellation: accepted old CTRL response, BUSY-held MPC word/final commit, "
                 "exactly-once retirement and source-disabled ACK\n";
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        testTelemetry();
#if AU_MODE
        testLocalFlushCancellation();
        testLocalFlushOwnedCommands();
