...
void motionTests(Sim& s) {
    ...
    static constexpr int edge[]={-32768,-32767,-1,0,1,32766,32767};
    unsigned cases=0;
    for(unsigned ai=0;ai<7;++ai)for(unsigned bi=0;bi<7;++bi)for(unsigned ci=0;ci<7;++ci)
    for(unsigned mask=0;mask<256;++mask)for(unsigned skip=0;skip<2;++skip) {
        std::array<int,4> x={edge[ai],edge[bi],edge[ci],edge[(ai+ci+1)%7]}, y{};
        for(unsigned i=0;i<4;++i)y[i]=int16_t(-x[i]);
        std::array<bool,4> available{}, inter{};
        for(unsigned i=0;i<4;++i) {
            available[i]=(mask>>i)&1;
            inter[i]=available[i]&&!((mask>>(i+4))&1);
        }
        t.avail_a=available[0];t.avail_b=available[1];t.avail_c=available[2];t.avail_d=available[3];
        t.intra_a=(mask>>4)&1;t.intra_b=(mask>>5)&1;t.intra_c=(mask>>6)&1;t.intra_d=(mask>>7)&1;
        t.mv_a_x=x[0];t.mv_b_x=x[1];t.mv_c_x=x[2];t.mv_d_x=x[3];
        t.mv_a_y=y[0];t.mv_b_y=y[1];t.mv_c_y=y[2];t.mv_d_y=y[3];
        const int dx=edge[(ai+bi)%7],dy=edge[(bi+ci)%7];
        t.mvd_x=uint16_t(dx);t.mvd_y=uint16_t(dy);t.p_skip=skip;t.eval();
        const unsigned c=available[2]?2:3;
        const std::array<bool,3> match={inter[0],inter[1],inter[c]};
        const unsigned matches=match[0]+match[1]+match[2];
        const bool zero=skip&&(!available[0]||!available[1]||
            (inter[0]&&x[0]==0&&y[0]==0)||(inter[1]&&x[1]==0&&y[1]==0));
        auto prediction=[&](const std::array<int,4>& values) {
            if(zero||matches==0)return 0;
            std::array<int,3> neighbors={match[0]?values[0]:0,
                match[1]?values[1]:0,match[2]?values[c]:0};
            if(matches==1)return match[0]?neighbors[0]:match[1]?neighbors[1]:neighbors[2];
            std::sort(neighbors.begin(),neighbors.end());
            return neighbors[1];
        };
        const int px=prediction(x),py=prediction(y);
        check(bool(t.skip_zero)==zero&&int16_t(t.mv_pred_x)==px&&int16_t(t.mv_pred_y)==py&&
              t.mv_x==uint16_t(px+(skip?0:dx))&&t.mv_y==uint16_t(py+(skip?0:dy)),
              "full signed16 MV/availability/intra/C-to-D/MVD boundary mismatch");
        ++cases;
    }
    std::cout<<"OK full signed16 MV cases="<<cases
             <<" availability/intra masks=256 pskip/P16/C-fallback/MVD-wrap verified\n";
}
...
void pipelinedFilterTests() {
    Sim s;
    auto& t=s.t;
    unsigned qpCases=0,edges=0;
    for(int p=0;p<64;++p)for(int q=0;q<64;++q)for(int off=-16;off<16;++off) {
        t.qp_p=p;t.qp_q=q;t.chroma_qp_index_offset=off&31;
        t.is_chroma=1;t.eval();
        check(t.qp_avg==(cqp(p,off)+cqp(q,off)+1)/2,"signed8 QP sum lost full-port chroma semantics");
        t.is_chroma=0;t.eval();
        check(t.qp_avg==(p+q+1)/2,"luma QP averaging changed");
        ++qpCases;
    }
    uint32_t random=0x145754;
    auto byte=[&] {random=random*1664525u+1013904223u;return unsigned(random>>24);};
    auto expected=[&] {
        std::array<unsigned,24> e{};
        for(unsigned i=0;i<4;++i) {
            e[i]=t.p2_out[i];e[4+i]=t.p1_out[i];e[8+i]=t.p0_out[i];
            e[12+i]=t.q0_out[i];e[16+i]=t.q1_out[i];e[20+i]=t.q2_out[i];
        }
        return e;
    };
    auto complete=[&](const std::array<unsigned,24>& e) {
        t.filter_start=1;s.tick();t.filter_start=0;
        check(t.filter_busy&&!t.filter_done,"sample pipeline did not own accepted start");
        for(unsigned stage=1;stage<3;++stage) {
            t.is_chroma=!t.is_chroma;t.qp_p=byte()&63;t.qp_q=byte()&63;
            t.alpha_off=byte()&31;t.beta_off=byte()&31;t.disable_all=!t.disable_all;
            for(unsigned i=0;i<4;++i) {
                t.p3_in[i]=byte();t.p2_in[i]=byte();t.p1_in[i]=byte();t.p0_in[i]=byte();
                t.q0_in[i]=byte();t.q1_in[i]=byte();t.q2_in[i]=byte();t.q3_in[i]=byte();
            }
            t.filter_start=1;s.tick();
            check(bool(t.filter_done)==(stage==2)&&bool(t.filter_busy)==(stage!=2),
                  "sample pipeline has an early/late/duplicate completion");
        }
        t.filter_start=0;
        for(unsigned i=0;i<4;++i)
            check(t.pipe_p2[i]==e[i]&&t.pipe_p1[i]==e[4+i]&&t.pipe_p0[i]==e[8+i]&&
                  t.pipe_q0[i]==e[12+i]&&t.pipe_q1[i]==e[16+i]&&t.pipe_q2[i]==e[20+i],
                  "sample pipeline changed original signed14 edge result or sampled live input");
    };
    for(int qp=0;qp<52;++qp)for(int off=-12;off<=12;off+=2)
      for(unsigned strength=0;strength<5;++strength)for(unsigned chroma=0;chroma<2;++chroma)
       for(unsigned pattern=0;pattern<8;++pattern) {
        t.is_chroma=chroma;t.qp_p=t.qp_q=qp;t.chroma_qp_index_offset=0;
        t.alpha_off=off&31;t.beta_off=(-off)&31;
        t.disable_all=0;t.slice_boundary_blocked=0;t.p_ref=t.q_ref=0;
        t.p_intra=strength>=3;t.q_intra=0;t.mb_boundary=strength==4;
        t.p_nonzero=strength==2;t.q_nonzero=0;
        t.p_mvx=t.p_mvy=t.q_mvy=0;t.q_mvx=strength==1?4:0;
        for(unsigned lane=0;lane<4;++lane) {
            std::array<unsigned,8> v{};
            for(unsigned tap=0;tap<8;++tap) {
                if(pattern==0)v[tap]=0;
                else if(pattern==1)v[tap]=255;
                else if(pattern==2||pattern==3)
                    v[tap]=clip((pattern==2?3:250)+int(tap)*3-9+int(lane));
                else if(pattern==4)v[tap]=byte();
                else if(pattern==5)v[tap]=((tap+lane)&1)?255:0;
                else if(pattern==6)v[tap]=clip(100+int(tap)*4-12-int(lane));
                else v[tap]=tap==0?0:tap==7?255:98+tap+lane;
            }
            t.p3_in[lane]=v[0];t.p2_in[lane]=v[1];t.p1_in[lane]=v[2];t.p0_in[lane]=v[3];
            t.q0_in[lane]=v[4];t.q1_in[lane]=v[5];t.q2_in[lane]=v[6];t.q3_in[lane]=v[7];
        }
        t.eval();check(t.bs==strength,"sample matrix did not select intended bS");
        complete(expected());++edges;
    }
    for(unsigned cut=0;cut<3;++cut) {
        t.filter_start=1;s.tick();t.filter_start=0;
        for(unsigned i=0;i<cut;++i)s.tick();
        t.filter_reset=1;s.tick();t.filter_reset=0;
        for(unsigned i=0;i<5;++i) {
            s.tick();check(!t.filter_busy&&!t.filter_done,"sample reset leaked stale completion");
        }
        t.eval();complete(expected());
    }
    std::cout<<"PASS staged sample filter edges="<<edges<<" lanes=4 cycles=3"
             <<" QP full-port pairs="<<qpCases
             <<" live-input/busy-start churn and3 reset/fresh-result fences\n";
}
