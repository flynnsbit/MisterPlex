...
from quartus_timing_observer import sdc_directives, unhex
...
def effective_exclusions(directory: Path, project: Path, sdcs: list[Path]) -> tuple[list[str], int, int]:
    ...
    for relative in evidence["active_sdc_files"]:
        ...
        expected.update({(relative, item.line_no, "set_" + item.kind): item.text
    ...
    if len(actual) != len(file_commands):
        raise ValueError("repeated exception execution at one source location needs explicit accounting")
    if actual != set(expected):
        raise ValueError(f"SDC execution coverage mismatch: missing={sorted(set(expected) - actual)}, "
                         f"unexpected={sorted(actual - set(expected))}")
    for item in file_commands:
        source = expected[item["file"], item["line"], item["kind"]]
        normal = lambda text: fingerprint(re.sub(r"\\\r?\n", " ", text))
        if normal(source) != normal(unhex(item["raw_hex"])):
            raise ValueError("original SDC command frame does not match its frozen source location")
