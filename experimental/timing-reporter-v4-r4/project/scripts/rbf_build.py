...
for entry in "$destination"/* "$destination"/.[!.]* "$destination"/..?*; do
    if [ -e "$entry" ] || [ -L "$entry" ]; then
        echo "REFUSED: timing destination is not empty" >&2
        exit 1
    fi
done
...
status_rc=$?
cat "$destination/reporter.log"
log_rc=$?
if [ "$report_rc" -ne 0 ]; then exit "$report_rc"; fi
if [ "$status_rc" -ne 0 ]; then exit "$status_rc"; fi
if [ "$log_rc" -ne 0 ]; then exit "$log_rc"; fi
identity=$(cat "$(dirname "$reporter")/observation.id") || exit 1
completion=$(cat "$destination/observer.complete") || exit 1
if [ "$completion" != "misterplex.timing-observer.v1:$identity" ]; then
    echo "REFUSED: timing observation did not complete" >&2
    exit 1
fi
grep -Ei '^[[:space:]]*(Error([[:space:]]*\\([0-9]+\\))?[[:space:]]*:|Critical Warning[[:space:]]*\\(332008\\)[[:space:]]*:|MPX_OBSERVER_FAILURE)|Read_sdc failed' "$destination/reporter.log"
diagnostic_rc=$?
if [ "$diagnostic_rc" -ne 1 ]; then
...
def prepare_reporting(work, meta):
    ...
    with tarfile.open(work / "inputs.tar") as archive:
        for member in archive.getmembers():
            suffix = Path(member.name).suffix.lower()
            if suffix not in {".v", ".sv", ".vhd", ".vhdl", ".sdc"}:
                continue
            if not member.isfile():
                raise RuntimeError("REFUSED: nonregular HDL catalog source")
            data = archive.extractfile(member).read()
            if digest(data) != meta["input_files"].get(member.name):
    ...
    files["constraint-sources.json"] = (json.dumps({
        ...
        "hdl": hdl_sources, "sdc": sdc_sources}, sort_keys=True, indent=2) + "\n").encode()
    if len(files["constraint-sources.json"]) > 16 * 1024**2:
        raise RuntimeError("REFUSED: constraint provenance catalog exceeds byte limit")
    files["observation.id"] = (uuid.uuid4().hex + "\n").encode()
...
def verify_reporter(work, meta):
    ...
    if {path.name for path in directory.iterdir()} != expected_files | {"provenance.json"}:
        raise RuntimeError("REFUSED: unlisted timing reporter execution files")
...
def verify_timing_reports(work, meta, expected_reporter):
    ...
    if directory.is_symlink() or not directory.is_dir():
        raise RuntimeError("REFUSED: missing/linked timing diagnostics; no safe failure sink")
    failure_path = directory / "collector-failure.json"
    if failure_path.exists() or failure_path.is_symlink():
        raise RuntimeError("REFUSED: persistent timing collector failure requires a new transaction")
    try:
        _verify_timing_reports(work, meta, expected_reporter)
    except Exception as exc:
        # Never turn a swallowed Quartus error or a missing observation into a
        # legacy success. The failure is sticky even if collection is retried.
        record = {"schema": "misterplex.timing-collector-failure.v1",
                  "stage": "verify-timing-reports", "input_sha256": meta["input_sha256"],
                  "error": f"{type(exc).__name__}: {exc}"[:16384]}
        try:
            with failure_path.open("x", encoding="utf-8") as stream:
                stream.write(json.dumps(record, sort_keys=True, indent=2) + "\n")
        except OSError as sink_error:
            raise RuntimeError(f"{exc}; collector failure evidence write failed: {sink_error}") from exc
        raise


def _verify_timing_reports(work, meta, expected_reporter):
    ...
    manifest = {"schema": "misterplex.timing-paths.v3", "reporter": provenance,
    ...
    if provenance["schema"] == "misterplex.timing-reporter.v4":
        ...
        try:
            result = run([sys.executable, "-I", "-B", "-c", collector, work / "reporter", directory],
...
def _compile_snapshot(work, meta, lease, docker):
    ...
    if (work / "timing").is_symlink():
        raise RuntimeError("REFUSED: linked timing destination")
    (work / "timing").mkdir(exist_ok=True)
    if any((work / "timing").iterdir()):
        raise RuntimeError("REFUSED: timing destination is not empty; use a new transaction")
    project = work / "project"
