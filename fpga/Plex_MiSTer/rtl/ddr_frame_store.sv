...
//
// PLXC core identity stamp is generated by scripts/gen_core_identity_stamp.py
// into core_identity_stamp.svh (git provenance). Compiles MUST pass
// -I.../fpga/Plex_MiSTer/rtl (Verilator TBs and Quartus). Never hand-edit the stamp.

`include "core_identity_stamp.svh"
...
module ddr_frame_store #(
	...
	// PLXC core identity — DOORBELL-relative (never hardcode 0x300xF130).
	parameter [31:0] IDENTITY_MAILBOX_PHYS = DOORBELL_PHYS + 32'h130,
	// Build stamp from generated core_identity_stamp.svh (git inject at build).
	parameter [27:0] CORE_IDENTITY_PROVENANCE = CORE_IDENTITY_PROVENANCE_LP,
	parameter bit    CORE_IDENTITY_CAP_DDR    = CORE_IDENTITY_CAP_DDR_LP[0],
	parameter bit    CORE_IDENTITY_CAP_SPI    = CORE_IDENTITY_CAP_SPI_LP[0],
	parameter [1:0]  CORE_IDENTITY_ABI        = CORE_IDENTITY_ABI_LP,
	...
	localparam [28:0] IDENTITY_MAILBOX_W = IDENTITY_MAILBOX_PHYS[31:3];
	...
	localparam [31:0] MAGIC_C = 32'h504C_5843; // PLXC core identity (stamp must match)
	// Pack once: [63:62] abi, [61] cap_spi, [60] cap_ddr, [59:32] provenance, [31:0] magic
	localparam [31:0] IDENTITY_HI = {
		CORE_IDENTITY_ABI,
		CORE_IDENTITY_CAP_SPI,
		CORE_IDENTITY_CAP_DDR,
		CORE_IDENTITY_PROVENANCE
	};
	...
	always @(posedge clk_ddr) begin
		if (reset_ddr) begin
			...
			if (!bank_mbox_valid || (bank_mbox_hb == 18'd0))
				bank_mbox_req <= 1'b1;
			// PLXC identity is constant; publish once then rare heartbeat refresh.
			id_mbox_hb <= id_mbox_hb + 18'd1;
			if (!id_mbox_valid || (id_mbox_hb == 18'd0))
				id_mbox_req <= 1'b1;
			...
			case (state_ddr)
				S_IDLE: begin
					...
					if (frame_mbox_req && (!frame_mbox_valid || poll_div[7:0] == 8'd224)
						...
						bank_mbox_hb <= 18'd0;
						state_ddr <= S_WRITE_WAIT;
					end else if (id_mbox_req && (!id_mbox_valid || poll_div[7:0] == 8'd96)
					    && !DDRAM_BUSY && !DDRAM_RD && !DDRAM_WE) begin
						// PLXC core identity — build stamp + DDR path capability.
						// DOORBELL-relative IDENTITY_MAILBOX_PHYS (product +0x130).
						// Constant payload; SPI cores never instantiate this module
						// so magic stays absent there (pair gate uses that).
						DDRAM_ADDR <= IDENTITY_MAILBOX_W;
						DDRAM_BURSTCNT <= 8'd1;
						DDRAM_DIN <= {IDENTITY_HI, MAGIC_C};
						DDRAM_WE <= 1'b1;
						id_mbox_valid <= 1'b1;
						id_mbox_req <= 1'b0;
						id_mbox_hb <= 18'd0;
