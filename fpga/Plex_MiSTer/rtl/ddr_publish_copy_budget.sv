...
module ddr_publish_copy_budget #(
	...
	output wire [31:0] prereg_t_fabric_vs_present_us, // PRE-REG contended copy
	...
	output wire        fabric_contended_beats_arm,
	output wire        fabric_ideal_fits_24,
	output wire        fabric_contended_fits_24,
	output wire        pl330_est_fits_24,
	output wire        pl330_beats_fabric_contended, // PL330 EST faster than contended fabric?
	...
	// Contended fabric copy PRE-REG: present R_req ≈ one direction → concurrent
	// R+W+R ≈ 1.5× ideal R+W duty → 3.840 * 1.5 = 5.760 ms. Device unverified.
	localparam int PR_T_FABRIC_VS_PRESENT_US = 5_760;
	...
	assign prereg_t_fabric_vs_present_us = 32'(PR_T_FABRIC_VS_PRESENT_US);
	...
	assign fabric_contended_beats_arm = (PR_T_FABRIC_VS_PRESENT_US < PR_T_COPY_ARM_US);
	assign fabric_ideal_fits_24 = (T_FAB_RW_US < T_BUDGET_24_US);
	assign fabric_contended_fits_24 = (PR_T_FABRIC_VS_PRESENT_US < T_BUDGET_24_US);
	assign pl330_est_fits_24 = (T_PL330_US < T_BUDGET_24_US);
	// Honest cross-check: at PREREG pins PL330 EST (9.216 ms) is SLOWER than
	// contended fabric PRE-REG (5.760 ms) but costs 0 M10K and avoids f2sdram
	// collision. Device may reverse this — both UNVERIFIED on silicon.
	assign pl330_beats_fabric_contended = (T_PL330_US < PR_T_FABRIC_VS_PRESENT_US);
