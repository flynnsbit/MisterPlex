...
module ddr_publish_engine #(
	...
	// bounce[0:MAX_BURST-1] needs $clog2(MAX_BURST) index bits (16 → 4, not 5).
	localparam int BURST_IDX_W = (MAX_BURST <= 1) ? 1 : $clog2(MAX_BURST);
	reg [BURST_IDX_W-1:0] burst_n, got, wr_i;
	reg        hold_present;
	...
	function automatic [BURST_IDX_W-1:0] nburst(input [31:0] left);
		begin
			if (left >= 32'(MAX_BURST))
				nburst = BURST_IDX_W'(MAX_BURST);
			else if (left == 32'd0)
				nburst = '0;
			else
				nburst = BURST_IDX_W'(left[BURST_IDX_W-1:0]);
		end
	endfunction
		...
		if (reset) begin
			...
			ST_RD_DATA: begin
				...
				if (DDRAM_DOUT_READY) begin
					...
					got <= got + BURST_IDX_W'(1);
					beats_rd <= beats_rd + 16'd1;
					src_w <= src_w + 1'b1;
					words_left <= words_left - 32'd1;
					if (got + BURST_IDX_W'(1) == burst_n) begin
						wr_i <= '0;
						wr_issued <= 1'b0;
						state <= ST_WR_ISSUE;
					end
			...
			ST_WR_ISSUE: begin
				...
				if (!wr_issued) begin
					...
					if (wr_i + BURST_IDX_W'(1) == burst_n)
						state <= ST_RD_ISSUE;
					else
						wr_i <= wr_i + BURST_IDX_W'(1);
				end
			end
