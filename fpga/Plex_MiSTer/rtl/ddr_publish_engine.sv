...
module ddr_publish_engine #(
	...
	// bounce[0:MAX_BURST-1] needs IDX_W index bits; burst_n holds 1..MAX_BURST (CNT_W).
	localparam int IDX_W = (MAX_BURST <= 1) ? 1 : $clog2(MAX_BURST);
	localparam int CNT_W = $clog2(MAX_BURST + 1);
	reg [CNT_W-1:0] burst_n;
	...
	function automatic [CNT_W-1:0] nburst(input [31:0] left);
		begin
			if (left >= 32'(MAX_BURST))
				nburst = CNT_W'(MAX_BURST);
			else if (left == 32'd0)
				nburst = '0;
			else
				nburst = CNT_W'(left[CNT_W-1:0]);
		end
	endfunction
		...
		if (reset) begin
			...
			ST_RD_DATA: begin
				...
				if (DDRAM_DOUT_READY) begin
					...
					// Widen got for compare: last beat when (got+1)==burst_n
					if ((CNT_W'(got) + CNT_W'(1)) == burst_n) begin
						wr_i <= '0;
						wr_issued <= 1'b0;
						state <= ST_WR_ISSUE;
					end
