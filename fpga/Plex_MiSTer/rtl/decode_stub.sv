...
module decode_stub #(
	...
	// CAVLC s16 → sat9 → dequant s9. MB0 place is already s9, so sat9 is a
	// no-op there (H264 Expert). Do not write a second cut; instantiate the
	// shipped h264_coeff_sat9.
	wire signed [15:0] cavlc_s16 [0:15];
	wire signed [8:0]  sat9_coeff [0:15];
	genvar sat_i;
	generate
		for (sat_i = 0; sat_i < 16; sat_i = sat_i + 1) begin : gen_sat9_in
			assign cavlc_s16[sat_i] = {{7{lat_coeff[sat_i][8]}}, lat_coeff[sat_i]};
		end
	endgenerate
	h264_coeff_sat9 u_h264_coeff_sat9 (
		.cavlc_coeff(cavlc_s16),
		.residual_coeff(sat9_coeff)
	);
	// One dequant lives in h264_recon (mb_ctrl). Stub does not take a second
	// 16-DSP copy. IDCT sees zeros; paint stays pred-128 diagnostic.
	genvar dq_i;
	generate
		for (dq_i = 0; dq_i < 16; dq_i = dq_i + 1) begin : gen_stub_dq0
			assign idct_dequant[dq_i] = 18'sd0;
		end
	endgenerate
