...
// Full filter (packed-window WIP) is archived at:
//   .scratch-archive/h264_deblock_mb_packed.sv
// That version hung Quartus 17.0.2 A&S for 35+ min with frozen qmsg after
// elaborating multiport packed-window gather (no Internal Error, no progress).
//
// This stub keeps the product hierarchy mappable so MC area (u_luma) and
// h264_dpb_ddr PRESENT can be MEASURED. Behaviour: store 384 reconstructed
// samples and re-emit them in plane order with NO filtering (identity).
// Neighbour strips are not re-emitted.
//
// OWNER: restore real in-loop filter once multiport window access is
// rewritten without pathological multiport/mux elaboration.
...
module h264_deblock_mb #(
	...
	// Silence unused ports (identity path ignores filter knobs / BS inputs).
	wire _u = slice_start | disable_deblocking | mb_is_intra |
	          ...
	          (MB_W == 0) | (MB_H == 0) | (FRAME_W == 0) | (FRAME_H == 0);
