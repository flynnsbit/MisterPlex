...
//   * I_16x16: DC block exported (i16_dc_level_*); AC max_coeff=15 to core
...
module h264_i_mb_feed #(
	...
	reg [7:0]  mb_type_r;
	reg        is_i16_r;
	reg        i16_dc_pending; // 1 until Intra16x16DCLevel has been parsed
	reg        slice_is_i_r;
	reg        mb_intra_r;
	reg        mb_skip_r;
	...
	function automatic res_step_coded;
		...
		begin
			if (step < STEP_LUMA_END) begin
				bi = step[3:0];
				// I16 AC uses CBP luma bits (DC is separate via i16_dc_pending).
	...
	always @(posedge clk) begin
		...
		if (reset) begin
			...
			ST_RES_REQ: begin
				...
				win_armed <= 1'b0;
				// I16 always has Intra16x16DCLevel before any AC 4x4.
				if (is_i16_r)
					i16_dc_pending <= 1'b1;
			...
			ST_RES_START: begin
				if (i16_dc_pending) begin
					// Force parse Intra16x16DCLevel before any luma AC step.
					...
					if (rel_bit16 >= 16'd400) begin
						...
						if (i16_dc_pending) begin
							// Intra16x16DCLevel: 16 coeffs, nC=0 (no left/up yet).
							cav_table <= 3'd0;
							cav_max <= 5'd16;
						end else if (res_step < STEP_LUMA_END) begin
							cav_table <= tok_table_luma;
							// I16 AC = 15 levels; I4x4 / inter luma = 16.
				...
				if (cav_done) begin
					if (!cav_ok) begin
						...
						if (i16_dc_pending) begin
							...
							// Stay on res_step=0 for first AC block next.
							st <= ST_RES_START;
						end else if (res_step < STEP_LUMA_END) begin
