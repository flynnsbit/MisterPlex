// Sequential 4x4 inverse scale + IDCT. Bit-exact vs flex+idct4x4 / FFmpeg.
// LevelScale: ((c*na)<<(qdiv+2)+2)>>2  ==  ((c*na*16)<<(qdiv+2)+32)>>6
// DSP-safe: qp_mod6/div6 LUTs + mul_norm + shl_amt mux (never qp%/ or x<<<qdiv).
// ~21 cycles/block. Residual held in o[] (registered during column pass).
...
module h264_iq_idct_seq (
	...
	function automatic [3:0] scan_of_raster;
		input [3:0] r;
		case (r)
		4'd0: scan_of_raster=4'd0; 4'd1: scan_of_raster=4'd1; 4'd2: scan_of_raster=4'd5;
		4'd3: scan_of_raster=4'd6; 4'd4: scan_of_raster=4'd2; 4'd5: scan_of_raster=4'd4;
		4'd6: scan_of_raster=4'd7; 4'd7: scan_of_raster=4'd12; 4'd8: scan_of_raster=4'd3;
		4'd9: scan_of_raster=4'd8; 4'd10: scan_of_raster=4'd11; 4'd11: scan_of_raster=4'd13;
		4'd12: scan_of_raster=4'd9; 4'd13: scan_of_raster=4'd10; 4'd14: scan_of_raster=4'd14;
	...
	// Variable-amount left shift via mux — never `v <<< qdiv` (DSP trap).
	function automatic signed [31:0] shl_amt;
		input signed [31:0] v;
		input [3:0] amt;
		case (amt)
		4'd0: shl_amt = v;
		4'd1: shl_amt = {v[30:0], 1'b0};
		4'd2: shl_amt = {v[29:0], 2'b0};
		4'd3: shl_amt = {v[28:0], 3'b0};
		4'd4: shl_amt = {v[27:0], 4'b0};
		4'd5: shl_amt = {v[26:0], 5'b0};
		4'd6: shl_amt = {v[25:0], 6'b0};
		4'd7: shl_amt = {v[24:0], 7'b0};
		4'd8: shl_amt = {v[23:0], 8'b0};
		4'd9: shl_amt = {v[22:0], 9'b0};
		default: shl_amt = {v[21:0], 10'b0};
		endcase
	endfunction
	...
	// Caller holds coeff/qp/max/skip/dc stable from start through done.
	...
	// FFmpeg LevelScale: ((c*na)<<(qdiv+2)+2)>>2
	wire signed [31:0] base = mul_norm({{16{cval[15]}}, cval}, na);
	wire signed [31:0] prod = shl_amt(base, qdiv + 4'd2);
	wire signed [28:0] scaled = sat29((prod + 32'sd2) >>> 2);
	wire signed [28:0] dq_val = (r == 4'd0)
		? (dc_override ? dc_value : (skip_dc ? 29'sd0 : scaled))
		: scaled;
	// IDCT +32 on raster position 0 only (before first butterfly).
