...
//
// Interaction: residual-add (sv-resadd) / real ref (sv-ref) are other lanes.
// This walker now also exports first-partition mvd_l0 se(v) (product fetch uses
// part_idx=0). Neighbour MVP + mv=mvp+mvd remain decode_stub / sv-mvd.
//
// FAULT_FORCE_ZERO_MVD: mutation twin — drop exported mvd to 0 (still mvd_valid).
//
...
module h264_p_mb_traverse #(
	...
	parameter bit FAULT_FORCE_ZERO_MVD = 1'b0     // mutation: zero exported mvd_l0
)(
	input  wire        clk,
	input  wire        reset,
	input  wire        clear,
	...
	// First partition mvd_l0 se(v) pair (7.3.5.1). P_Skip / intra → 0.
	// Multi-partition MBs still walk all pairs for bit alignment; only the
	// first pair is exported for product P_L0_16x16 / part0 fetch.
	output reg  signed [15:0] mvd_x,
	output reg  signed [15:0] mvd_y,
	output reg         mvd_valid,          // 1 when mvd ports hold a parsed pair
	...
	reg [7:0] mvd_pairs_total;   // pairs expected for this MB (first pair exported)
	reg signed [15:0] mvd_x_r, mvd_y_r;
	reg        mvd_valid_r;
	...
	task automatic emit_mb;
		...
		begin
			if (drop_this_mb) begin
				...
				// P_Skip / intra: mvd not in bitstream → 0 (MVP may still be nonzero).
				if (skipped || is_intra_r) begin
					mvd_x <= 16'sd0;
					mvd_y <= 16'sd0;
					mvd_valid <= 1'b1; // explicitly zero, not "unknown"
				end else if (FAULT_FORCE_ZERO_MVD) begin
					mvd_x <= 16'sd0;
					mvd_y <= 16'sd0;
					mvd_valid <= 1'b1;
				end else begin
			...
			if (mb_hold && !(mb_valid && mb_ready) && busy) begin
				...
				ST_TYPE: begin
					...
					if (ue_value <= 32'd4) begin
						...
						// Reset first-partition mvd capture for this MB.
						mvd_x_r <= 16'sd0;
						mvd_y_r <= 16'sd0;
						mvd_valid_r <= 1'b0;
						case (ue_value[2:0])
						3'd0: begin
							mvd_pairs_left <= 8'd1;
							mvd_pairs_total <= 8'd1;
							start_ue(ST_MVD_X);
						end
						3'd1: begin
							mvd_pairs_left <= 8'd2;
							mvd_pairs_total <= 8'd2;
							start_ue(ST_MVD_X);
						end
						3'd2: begin
							mvd_pairs_left <= 8'd2;
				...
				ST_SUB_UE: begin
					if (ue_value > 32'd3) begin
						...
						begin : sub_mvd_acc
							reg [7:0] next_pairs;
							next_pairs = mvd_pairs_left + sub_mb_mvd_pairs(ue_value);
							mvd_pairs_left <= next_pairs;
							if (sub_idx == 3'd3) begin
								mvd_pairs_total <= next_pairs;
								mvd_x_r <= 16'sd0;
								mvd_y_r <= 16'sd0;
								mvd_valid_r <= 1'b0;
								start_ue(ST_MVD_X);
				...
				ST_MVD_X: begin
					if (mvd_pairs_left == 8'd0)
						...
						// Capture X of the current pair; first pair is product export.
						mvd_x_tmp <= se16_from_ue(ue_value);
						start_ue(ST_MVD_Y);
					end
				end
				ST_MVD_Y: begin
					// ue_value holds MVD y; export first pair only.
					if (!mvd_valid_r) begin
						mvd_x_r <= mvd_x_tmp;
						mvd_y_r <= se16_from_ue(ue_value);
						mvd_valid_r <= 1'b1;
					end
