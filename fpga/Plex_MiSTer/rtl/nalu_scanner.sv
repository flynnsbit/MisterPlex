// Annex-B framing and EPB-stripped SPS/PPS/header/bounded VCL capture.
module nalu_scanner #(
	...
	output reg [RBSP_ADDR_W:0] sl_rbsp_len
);
	localparam [RBSP_ADDR_W:0] RBSP_MAX = {1'b1, {RBSP_ADDR_W{1'b0}}};
	localparam int ZERO_W = RBSP_ADDR_W < 16 ? 16 : RBSP_ADDR_W + 1;
	reg data_valid, pend_type, boundary_wait, au_end_seen;
	reg [1:0] cap_tgt;
	reg [5:0] cap_len;
	reg sl_idr_r, sl_done, vcl_flushed;
	reg [RBSP_ADDR_W:0] rbsp_len_r;
	reg [4:0] idle_c;
	// Zeros cannot be classified as payload until the following byte arrives.
	reg [ZERO_W-1:0] zero_count, flush_zeros;
	reg pending_valid;
		...
		if (reset) begin
			...
			if (flush_zeros != 0) begin
				...
				if (pend_type) begin
					...
					if (zero_count != {ZERO_W{1'b1}}) zero_count <= zero_count + 1'b1;
