	...
	always @(posedge clk_pix) begin
		if (reset_pix) begin
			...
			if (hold_busy) begin
				...
				// DOT_CRAWL_FIX: do NOT force black on gap/underrun. Black
				// inserts on the chevron silhouette (high contrast) read as
				// rolling edge dots on VGA/HDMI. Hold last RGB; still flag
				// underrun. PPC=1 experiment: crawl xor=0 but 720 path needs PPC=2.
				out_fstart <= 1'b0;
