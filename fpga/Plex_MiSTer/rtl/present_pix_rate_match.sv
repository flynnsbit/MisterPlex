...
module present_pix_rate_match #(
	...
	always @(posedge clk) begin
		if (reset) begin
			...
			fire_r <= 1'b0;
			// Fire when credit reaches one group and downstream can accept.
			// While backpressured at threshold, park acc (no unbounded growth)
			// so the next ready cycle fires immediately.
			if (acc + 32'(F_PIX_HZ) >= 32'(THRESH)) begin
				if (in_ready) begin
					acc    <= acc + 32'(F_PIX_HZ) - 32'(THRESH);
					fire_r <= 1'b1;
				end
				// else: hold acc (already >= THRESH - F_PIX after prior steps)
			end else begin
				acc <= acc + 32'(F_PIX_HZ);
			end
		end
