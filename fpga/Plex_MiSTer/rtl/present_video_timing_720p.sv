...
// present_beam_ppc parameterized from the same COMPACT totals (H=1650 V=750).
//
...
//   PRODUCT fabric COMPACT (PLL-legal 30 MHz — Quartus rejected 29.7):
//     f_pix = **30_000_000 Hz**; fps_eff = 30e6/(1650*750) = **24.242424… Hz**
//     (exact 24.000 at H1650×V750 would need 29.7 MHz — illegal on this integer-N PLL)
//   ALT exact-24: H=1600 V=750 @ 28.8 MHz (beam totals change required)
...
//   @20 MHz, 1650×750: fps_eff = 20e6/1_237_500 ≈ **16.16 Hz**  ← stills trap
//   @30 MHz, 1650×750: fps_eff = 30e6/1_237_500 ≈ **24.242 Hz** ← product
...
module present_video_timing_720p #(
	...
	output wire        cea_24_needs_faster_pix // 1 if CLK_PIX_HZ < product 30 MHz
);
	// COMPACT fabric 720p (same totals as VIC4 geometry; product rate is 30 MHz)
	...
	// Product target clk_pix (PLL-legal 30 MHz)
	localparam int PRODUCT_PIX_HZ = 30_000_000;
