...
module sdram_memtest #(
	...
	localparam [25:0] WORDS_16_MB = 26'd8_388_608;
	localparam [25:0] WORDS_32_MB = 26'd16_777_216;
	localparam [25:0] WORDS_64_MB = 26'd33_554_432;
	...
	always @(posedge clk) begin
		if (reset) begin
			...
			case (state)
				...
				ST_DET_W32: start_write(ADDR_32_MB, PAT32, ST_DET_R0);
				ST_DET_R0:  start_read(26'd0, 16'd0, 1'b0, ST_DET_R16);
				ST_DET_R16: begin
					det_r0 <= last_read;
					start_read(ADDR_16_MB, 16'd0, 1'b0, ST_DET_PICK);
				end

				ST_DET_PICK: begin
					det_r16 <= last_read;
					if (det_r0 == PAT0 && last_read == PAT16) begin
						size_code   <= SIZE_64_MB;
						limit_words <= WORDS_64_MB;
					end else if (last_read == PAT16) begin
						size_code   <= SIZE_32_MB;
						limit_words <= WORDS_32_MB;
					end else if (det_r0 == PAT32 || last_read == PAT32) begin
				...
				ST_W1_READ: begin
					if (ptr < limit_words) begin
						start_read(ptr, walk_one(ptr), 1'b1, ST_W1_READ);
						ptr <= ptr + 26'd1;
					end else begin
						ptr <= 26'd0;
						state_code <= 4'd4;
				...
				ST_W0_WRITE: begin
					if (ptr < limit_words) begin
						start_write(ptr, ~walk_one(ptr), ST_W0_WRITE);
						ptr <= ptr + 26'd1;
					end else begin
						ptr <= 26'd0;
						state <= ST_W0_READ;
				...
				ST_W0_READ: begin
					if (ptr < limit_words) begin
						start_read(ptr, ~walk_one(ptr), 1'b1, ST_W0_READ);
						ptr <= ptr + 26'd1;
					end else begin
						ptr <= 26'd0;
						state_code <= 4'd5;
				...
				ST_ADDR_READ: begin
					if (ptr < limit_words) begin
						start_read(ptr, addr_pattern(ptr), 1'b1, ST_ADDR_READ);
						ptr <= ptr + 26'd1;
					end else begin
						state <= ST_DONE;
					end
				...
				ST_OP_WAIT: begin
					if (sdram_ready) begin
						...
						if (op_check && (sdram_dout != op_expect) && (error_count != 16'hFFFF))
