...
module slice_hdr_parser (
	...
	// ── P macroblock prediction syntax (7.3.5.1 mb_pred / 7.3.5.2 sub_mb_pred)
	// Slot layout for the per-partition arrays below: for P_L0_16x16 slot 0,
	// for P_L0_L0_16x8 / P_L0_L0_8x16 slots 0..1, and for P_8x8 slot
	// (8x8 block * 4 + sub-partition), so a slot index always identifies the
	// same geometry regardless of macroblock type.
	output reg  [7:0]  first_sub_mb_types,      // 4 x 2-bit sub_mb_type, block 0 in [1:0]
	output reg         first_sub_mb_valid,
	output reg  [7:0]  first_mb_ref_idx_l0,     // 4 x 2-bit ref_idx_l0, mbPart 0 in [1:0]
	output reg  [15:0] first_mb_mvd_valid,      // bit per slot
	...
	reg [7:0]  intra_mbt;
	reg [5:0]  p_mbt;      // inter mb_type 0..4 currently being walked
	reg [2:0]  pred_blk;   // mbPart / 8x8 block index
	reg [2:0]  pred_sub;   // sub-partition index within an 8x8 block
	reg [2:0]  pred_ref_i; // ref_idx_l0 iterator
	reg signed [15:0] mvd_x_tmp;
	// Stable slot index for the partition currently being parsed.
	...
	// Table 9-4 Inter column. The Inter mapping is NOT the Intra mapping: using
	// the intra table on an inter macroblock silently decodes the wrong number
	// of residual blocks and desynchronises the rest of the slice.
	...
	// se(v) over the full 16-bit codeNum range. se_of() saturates at 8 bits,
	// which is fine for QP deltas but truncates motion vector differences.
	...
	// Number of sub-partitions carried by a P sub_mb_type (Table 7-18):
	// 0 = P_L0_8x8 (1), 1 = P_L0_8x4 (2), 2 = P_L0_4x8 (2), 3 = P_L0_4x4 (4).
	...
	localparam [5:0]
		...
		ST_P_MBT       = 6'd35,
		// P macroblock prediction syntax walk
		ST_SUBMBT      = 6'd36, // ue(sub_mb_type[]) x4
		ST_PRED_REF    = 6'd37, // dispatch: next ref_idx_l0 or move on to mvd
		ST_REFIDX_TE1  = 6'd38, // te(v) with cMax == 1: a single inverted flag
		ST_REFIDX_VAL  = 6'd39, // te(v) with cMax > 1: landed ue(v)
		ST_PRED_MVD    = 6'd40, // dispatch: next mvd_l0 pair or move on to cbp
		ST_MVD_X       = 6'd41,
		ST_MVD_Y       = 6'd42,
		ST_INTER_CBP   = 6'd43;
	...
	task automatic pred_clear;
		...
		begin
			...
			// ref_idx_l0 is inferred to 0 whenever it is not coded, which is
			// the normal case for this stream (one active reference), so the
			// cleared state is also the correct default.
	...
	always @(posedge clk) begin
		if (reset) begin
			...
			ST_REFIDX_L0: begin
				// num_ref_idx_l0_active_minus1 decides whether ref_idx_l0 is
				// coded at all in the macroblock layer, so it has to survive
				// the slice header instead of being discarded.
			...
			ST_P_MBT: begin
				...
				if (ue_val <= 16'd4) begin
					// P_L0_16x16 / P_L0_L0_16x8 / P_L0_L0_8x16 / P_8x8[ref0]
					intra_mbt <= 8'hFF;
					...
					// Intra macroblock inside a P slice: mb_type carries the
					// +5 offset of Table 7-13. Strip it and re-enter the same
					// intra syntax path an I slice uses, otherwise the parser
					// walks the wrong syntax and desynchronises the slice.
			...
			ST_SUBMBT: begin
				// ue(sub_mb_type[mbPartIdx]) x4, Table 7-18. Values above 3 are
				// not Baseline P sub types; stop rather than mis-walk.
			...
			ST_PRED_REF: begin
				// ref_idx_l0 is present only when more than one reference is
				// active, and never for P_8x8ref0 (mb_type 4). Consuming bits
				// that were never coded is the fastest way to desync a slice,
				// so the inferred-to-zero case must read nothing at all.
				if ((num_ref_idx_l0_active_minus1 == 8'd0) || (p_mbt == 6'd4) ||
					...
					// te(v) with cMax == 1 is a single flag, inverted.
					st <= ST_REFIDX_TE1;
				end else begin
			...
			ST_PRED_MVD: begin
				// One mvd_l0 pair per partition: 1 for 16x16, 2 for 16x8/8x16,
				// and for P_8x8 one per sub-partition of each 8x8 block.
				if (p_mbt >= 6'd3) begin
			...
			ST_MVD_Y: begin
				// Slot index: 8x8 block * 4 + sub-partition for P_8x8, plain
			...
			ST_INTER_CBP: begin
				// ue(coded_block_pattern) through the INTER column of Table 9-4.
				// Quartus rejects part-select on function-call results — stage through temp.
				cbp_me <= ue_val[5:0];
				begin : inter_cbp_map
					reg [5:0] cbp_mapped;
