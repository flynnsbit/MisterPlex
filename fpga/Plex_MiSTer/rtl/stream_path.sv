...
module stream_path #(
	...
	generate
		if (`DECODE_REAL_INTRA) begin : gen_real_intra_decode
			...
			reg real_ctx_mb_commit;
			reg [3:0] real_block_index;
			reg [7:0] real_mb_x;
			reg [7:0] real_mb_y;
			reg [15:0] real_first_mb_in_slice;
			...
			wire [7:0] real_nb_topleft;
			wire [7:0] real_nb_topright [0:3];
			wire real_mb_avail_left;
			wire real_mb_avail_top;
			wire real_mb_avail_topright;
			wire real_mb_avail_topleft;
			wire real_mb_recon_valid;
			wire [7:0] real_recon_y [0:255];
			wire [4:0] real_blocks_done;
			wire [7:0] real_ctx_recon_pixels [0:15];
			wire [7:0] real_ctx_recon_u [0:63];
			wire [7:0] real_ctx_recon_v [0:63];
			wire [7:0] real_ctx_above_unused [0:7];
			wire [7:0] real_ctx_left_unused [0:3];
			wire [7:0] real_ctx_top_left_unused;
			wire real_ctx_has_above_unused;
			wire real_ctx_has_left_unused;
			wire real_ctx_has_above_right_unused;
			wire [7:0] real_ctx_chroma_u_above_unused [0:7];
			wire [7:0] real_ctx_chroma_v_above_unused [0:7];
			wire [7:0] real_ctx_chroma_u_left_unused [0:7];
			wire [7:0] real_ctx_chroma_v_left_unused [0:7];
			wire [7:0] real_ctx_chroma_u_top_left_unused;
			wire [7:0] real_ctx_chroma_v_top_left_unused;
			wire real_ctx_has_chroma_above_unused;
			wire real_ctx_has_chroma_left_unused;
			wire [7:0] real_slice_mb_w = (sps_mb_w != 8'd0) ? sps_mb_w : 8'd39;
			...
			for (real_gi = 0; real_gi < 16; real_gi = real_gi + 1) begin : gen_real_defaults16
				...
				assign real_ctx_recon_pixels[real_gi] = 8'd128;
			end
			for (real_gi = 0; real_gi < 64; real_gi = real_gi + 1) begin : gen_real_chroma_defaults
				assign real_ctx_recon_u[real_gi] = 8'd128;
				assign real_ctx_recon_v[real_gi] = 8'd128;
			end

			// Intra prediction consumes PRE-deblock reconstructed neighbours.  This
			// context is intentionally separate from the DPB/reference POST-deblock tap.
			h264_intra_nb_ctx #(
				.MB_WIDTH_MAX(39),
				.MB_WIDTH_DEFAULT(39)
			) u_real_intra_nb_ctx (
				...
				.mb_x(real_mb_x),
				.mb_y(real_mb_y),
				.mb_width(real_slice_mb_w),
				.first_mb_in_slice(real_first_mb_in_slice),
				.mb_start(real_mb_start),
				.block_idx(real_block_index),
				.block_valid(1'b0),
				.recon_pixels(real_ctx_recon_pixels),
				.mb_commit(real_ctx_mb_commit),
				.recon_y_mb(real_recon_y),
				.recon_u_mb(real_ctx_recon_u),
				.recon_v_mb(real_ctx_recon_v),
				.above(real_ctx_above_unused),
				.left(real_ctx_left_unused),
				.top_left(real_ctx_top_left_unused),
				.has_above(real_ctx_has_above_unused),
				.has_left(real_ctx_has_left_unused),
				.has_above_right(real_ctx_has_above_right_unused),
				.mb_avail_left(real_mb_avail_left),
				.mb_avail_top(real_mb_avail_top),
				.mb_avail_topright(real_mb_avail_topright),
				.mb_avail_topleft(real_mb_avail_topleft),
				.nb_top(real_nb_top),
				.nb_left(real_nb_left),
				.nb_topleft(real_nb_topleft),
				.nb_topright(real_nb_topright),
				.chroma_u_above(real_ctx_chroma_u_above_unused),
				.chroma_v_above(real_ctx_chroma_v_above_unused),
				.chroma_u_left(real_ctx_chroma_u_left_unused),
				.chroma_v_left(real_ctx_chroma_v_left_unused),
				.chroma_u_top_left(real_ctx_chroma_u_top_left_unused),
				.chroma_v_top_left(real_ctx_chroma_v_top_left_unused),
				.has_chroma_above(real_ctx_has_chroma_above_unused),
				.has_chroma_left(real_ctx_has_chroma_left_unused)
			);
...
`endif
			...
			always @(posedge clk) begin
				...
				real_ctx_mb_commit <= real_mb_recon_valid;
				fs_wr_en <= 1'b0;
				fs_wr_reset <= 1'b0;
				fs_swap <= 1'b0;
				...
				if (reset | flush) begin
					...
					real_ctx_mb_commit <= 1'b0;
					real_mb_x <= 8'd0;
					real_mb_y <= 8'd0;
					real_first_mb_in_slice <= 16'd0;
					real_frames <= 16'd0;
					real_busy <= 1'b0;
					recon_sig <= 8'd0;
					recon_dbg <= 8'd0;
					...
					R_IDLE: begin
						if (residual_place_pulse) begin
							...
							real_mb_x <= (real_slice_mb_w != 8'd0) ? (sl_first % real_slice_mb_w) : 8'd0;
							real_mb_y <= (real_slice_mb_w != 8'd0) ? (sl_first / real_slice_mb_w) : 8'd0;
							real_first_mb_in_slice <= sl_first;
							real_mb_start <= 1'b1;
							real_pred_wait <= 2'd0;
							real_feed_idx <= 4'd0;
