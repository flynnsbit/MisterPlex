...
module stream_path #(
	...
	// AU scanning starts only after the final byte, so staging must hold the
	// entire negotiated encoded AU, independently of the VCL capture RAM.
	localparam int INPUT_FIFO_BYTES = ENABLE_AU_PROTOCOL ? RBSP_BYTES : 32768;
	...
	wire [RBSP_ADDR_W:0] sl_rbsp_len;
	wire scanner_idle, scanner_au_done;
	reg au_scan_complete;

	nalu_scanner #(
		.ENABLE_AU_END(ENABLE_AU_PROTOCOL), .RBSP_ADDR_W(RBSP_ADDR_W)
	) scan (
	...
	wire [RBSP_BIT_W-1:0] sl_bit_pos_hdr, sl_bit_pos_resid;
	...
	wire       place_pulse;
	slice_hdr_parser #(
		.BIT_W(RBSP_BIT_W), .LEGACY_DIAGNOSTIC(LEGACY_SLICE_DIAGNOSTIC)
	) slp (
		.clk(clk), .reset(pipeline_reset),
	...
	reg native_seen, native_copy_accepted;
	wire picture_fault = decoder_header_error || mb_decode_error != 0;
	reg picture_pending;
	...
	wire        dpb_read_accept = dpb_mem_rd && !pipeline_reset;
	assign pub_mem_ready = ENABLE_PICTURE_PUBLISH && !dpb_mem_rd && !pipeline_reset;
	wire pub_read = pub_mem_rd && pub_mem_ready && {14'd0, pub_mem_addr} < 32'(DPB_N);
	wire [17:0] picture_read_addr = pub_read ? pub_mem_addr : dpb_mem_raddr[17:0];
	wire picture_read_hit = pub_read || (dpb_read_accept && dpb_mem_raddr < 32'(DPB_N));
	// frame_ready admits the copy, not a bank release. Keep the native lease
	// until its real publisher/display fence and accepted RAM reads retire.
	assign mb_native_release = ENABLE_PICTURE_PUBLISH && native_copy_accepted &&
		!picture_pending && fs_writes_idle && !pub_read && !pub_mem_valid &&
	...
	always @(posedge clk) begin
		...
		if (picture_read_hit)
			picture_read_data <= dpb_pic[picture_read_addr];
	end
	always @(posedge clk) begin
		dpb_mem_rvalid <= dpb_read_accept;
		pub_mem_valid <= pub_read;
		...
		if (pipeline_reset) begin
			...
			if (mb_native_release) native_copy_accepted <= 1'b0;
			if (picture_valid && picture_ready) native_copy_accepted <= 1'b1;
	...
	// sl_rbsp_* = bounded VCL RBSP (64 KiB modern, 8 KiB legacy). sl_cap 48B
	...
	// Stage the bounded AU before scanning; only its explicit final-byte
	// marker can authorize EOF. One AU owns decoder metadata at a time.
	wire pipeline_quiet = bf_rd_empty && !bf_rd_en && !si_wr_en &&
	                      ...
	                      !mb_busy && !stub_busy_w &&
	                      (!ENABLE_PICTURE_PUBLISH || (!mb_native_valid && !native_copy_accepted)) &&
	                      !dpb_mem_we && !dpb_mem_rd && !dpb_mem_rvalid &&
	                      !picture_valid && !pub_read && !pub_mem_valid &&
	...
	assign au_ready = ENABLE_AU_PROTOCOL && ddr_selected && decoder_idle && !si_active;
	...
	always @(posedge clk) begin
		if (pipeline_reset) begin
			...
			if (current_au_valid && au_scan_complete && pipeline_drained) begin
				current_au_valid <= 1'b0;
				au_bytes_complete <= 1'b0;
			end
		end
