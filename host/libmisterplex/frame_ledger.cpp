...
void frameLedgerProcessStart(int64_t lifetimeFrames, int64_t lifetimePresents,
                             int64_t lifetimeDrops, int64_t lifetimePresentFails) {
    std::lock_guard<std::mutex> lk(g_mu);
    ...
    const int n = std::snprintf(
        ...
        "lifetime_drops=%lld lifetime_present_fails=%lld\n",
        ...
        static_cast<long long>(lifetimePresentFails));
    if (n > 0)
        appendLine(line, static_cast<size_t>(n));
    std::fprintf(stderr, "misterplexd: FRAME_LEDGER event=process_start path=%s pid=%d\n",
                 g_path.c_str(), static_cast<int>(::getpid()));
}
...
void frameLedgerSessionEnd(uint64_t sessionId, int64_t frames, int64_t presents, int64_t drops,
    ...
    if (!g_inited || g_path.empty())
        return;
    const int64_t residual = frameLedgerResidual(frames, presents, drops, presentFails);
    char ts[32];
    wallTs(ts, sizeof(ts));
    char line[768];
    const int n = std::snprintf(
        ...
        "present_fails=%lld residual=%lld closed=%d reason=%s\n",
        ...
        static_cast<long long>(drops), static_cast<long long>(presentFails),
        static_cast<long long>(residual), residual == 0 ? 1 : 0, reason ? reason : "?");
    if (n > 0)
        appendLine(line, static_cast<size_t>(n));
    std::fprintf(stderr,
                 "misterplexd: FRAME_LEDGER event=session_end session=%llu frames=%lld "
                 "presents=%lld drops=%lld present_fails=%lld residual=%lld closed=%d "
                 "reason=%s\n",
                 ...
                 static_cast<long long>(presentFails), static_cast<long long>(residual),
                 residual == 0 ? 1 : 0, reason ? reason : "?");
}
...
void frameLedgerProcessExit(int code, const char* why, int64_t lifetimeFrames,
    ...
    const int n = std::snprintf(
        ...
        "lifetime_presents=%lld lifetime_drops=%lld lifetime_present_fails=%lld\n",
    ...
    *out = FrameLedgerTotals{};
    ...
    while (std::getline(in, line)) {
        if (line.find("event=process_start") != std::string::npos)
            ...
            out->present_fails += grab("present_fails");
