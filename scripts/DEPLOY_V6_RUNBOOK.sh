#!/usr/bin/env bash
# Deploy runbook for arm-deploy-candidate v6 — execution, not composition.
#
# DEFAULT IS DRY-RUN. Real deploy only when:
#   PARENT_AUTHORIZE_DEPLOY=YES DEPLOY_EXECUTE=1 ./scripts/DEPLOY_V6_RUNBOOK.sh
#
...
#   PREV_BIN=.../backup/misterplexd.before-v6-ROLLBACK  (f230f95b)
...
EXPECT_ARM_MD5="${EXPECT_ARM_MD5:-87e8500aedfdea0cf9506572e7d88bd2}"
...
CAND_ARM="${CAND_ARM:-$ROOT/build/arm-deploy-v6-freeze/misterplexd}"
...
if [[ "$AUTH" != "YES" || "$EXECUTE" != "1" ]]; then
  ...
  log "  5) on any R1–R5: PREV_BIN=$ROLLBACK_BIN (expect md5 a56bbc3c…) restore (~5–15s)"
...
if [[ -f "$ROOT/build/arm/misterplexd" ]]; then
  cp -f "$ROOT/build/arm/misterplexd" "$ROOT/build/arm/misterplexd.pre-v6-local.$$"
fi
...
INSTALL_HELPER="$ROOT/build/v6_claim_install.sh"
...
TAG="before-v6-\$(date -u +%Y%m%dT%H%M%SZ)"
sshpass -p "\$PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=12 \
...
MISTER_CLAIM_AGENT=w-bounce2-v6 \
  "$ROOT/scripts/mister_soft_bounce.sh" claim \
  --reason "parent-authorised v6 daemon-only deploy (no conf) ARM ${EXPECT_ARM_MD5}" \
  -- "$INSTALL_HELPER"
claim_rc=$?
