...
Named capability: refuse the exclusive Quartus fit when a "critical" module is
either absent from files.qip or only appears in a pruned subtree (compiled then
stripped). Historical defect: h264_cavlc_residual was in files.qip but its only
parent h264_decode_skeleton was not — Quartus compiled CAVLC and pruned it to
zero logic while decode_stub sat in stream_path.
...
def run_gate(
    ...
    *,
    ...
    # Prove teeth: at least one critical must be non-reachable on known-hollow main
    # (caller may use --allow-green-critical for fixed trees). Default: report.

    if failures:
        msgs.append(
            "FAIL PREFIT_REACHABILITY critical/expect not REACHABLE: "
            + ",".join(failures)
        )
        msgs.append(
    ...
    msgs.append(
        ...
        "are REACHABLE from sys_top (presence only — not correctness)"
    )
    return 0, msgs
...
def main() -> int:
    ...
    if args.self_test:
        # Self-test: critical decoder modules must NOT all be REACHABLE on this
        # tree if it still has the historical hole; expect_ok must be REACHABLE.
        # Do not use require_critical=True here — that would exit before teeth check.
        _rc, msgs = run_gate(root, config, require_critical=False)
        for m in msgs:
            # Suppress the require_critical=False "PASS all critical" line —
            # self-test judges teeth separately.
            if m.startswith("PASS PREFIT_REACHABILITY all critical"):
                print("SELFTEST_NOTE full-gate PASS suppressed (teeth mode)")
                continue
            if m.startswith("FAIL PREFIT_REACHABILITY critical"):
                # expect_ok failure only in this mode
                print(m)
                continue
            print(m)
        ...
        red_teeth = any(
            st in {"ABSENT", "NOT_IN_QIP", "PRUNED"} for st in crit.values()
        )
        pruned_cavlc = any(
            n.startswith("h264_cavlc") and st == "PRUNED" for n, st in crit.items()
        )
        green_ok = all(st == "REACHABLE" for st in ok.values()) and len(ok) > 0
        print(
            f"SELFTEST_TEETH red_on_critical={int(red_teeth)} "
        ...
        if not red_teeth:
            print(
                "FAIL SELFTEST: critical decoder set has no ABSENT/NOT_IN_QIP/PRUNED "
                "— gate has no teeth on this tree (or decoder already landed)"
            )
            print("true rc=1")
            return 1
        if not green_ok:
            print(
                "FAIL SELFTEST: expect_reachable modules not REACHABLE — "
                f"{ok} (gate false-RED on known-good)"
            )
            print("true rc=1")
            return 1
        print(
            "PASS SELFTEST PREFIT_REACHABILITY teeth=1 "
            "(critical decoder non-reachable; expect_ok REACHABLE; "
            "CAVLC PRUNED class detected=%d)" % int(pruned_cavlc)
        )
        print("true rc=0")
        return 0
