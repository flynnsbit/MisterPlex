...
def read_table(path: Path, header: str, max_rows: int, limit: int = 16 * 1024**2) -> list[list[str]]:
    lines = read_evidence(path, limit).decode().splitlines()
...
def collect_extended_evidence(
    ...
    if len(summary_lines) != len(fields) or set(summary) != fields or \
            summary["schema"] != "misterplex.timing-extended.v1":
    ...
    kinds = {"set_clock_groups", "set_false_path", "set_max_delay", "set_min_delay",
             "set_multicycle_path"}
    for index, file, line, kind, code, command in read_table(
            directory / "Plex.exception-commands.tsv", "id\tfile\tline\tkind\tcode\tcommand", 512):
        ci, ln = int(index), int(line)
        if (ci in commands or ci < 0 or file not in active_sources or ln < 1 or
                kind not in kinds or code != "0" or not command):
            raise ValueError("invalid/incomplete exception command")
        commands[ci] = {"id": ci, "file": file, "line": ln, "kind": kind,
    ...
    for name in ("extended.summary", "audit-index.tsv", "scoped-index.tsv"):
        data = read_evidence(directory / name, 2 * 1024**2)
    ...
    return {"schema": summary["schema"], "scope": "h264-mb-rbsp-stub-v1",
            ...
            "reports": reports, "audits": audits, "metadata": metadata,
...
def parse_detailed_rows(directory: Path) -> list[PathSlack]:
    ...
    if manifest["schema"] not in {"misterplex.timing-paths.v1", "misterplex.timing-paths.v2"}:
    ...
    if manifest["schema"] == "misterplex.timing-paths.v2" and \
            reporter["schema"] != "misterplex.timing-reporter.v2":
    ...
    for name, expected in reporter["files"].items():
        if hashlib.sha256(read_evidence(reporter_dir / name, 1024**2)).hexdigest() != expected:
