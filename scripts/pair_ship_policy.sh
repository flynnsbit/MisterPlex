...
# bank1: DDR frame-store bank1 phys (geometry half of the pair), DERIVED at load
#   time from host/libmisterplex/ddr_frame_layout.hpp so this policy can never
#   drift from the layout the daemon and RTL actually use. Re-hardcoding the
#   addresses here is what the runtime-DDR-literal invariant gate forbids.
...
# Derive bank-1 physical bases from the single source of truth.
_PAIR_LAYOUT_HPP="${_PAIR_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}/host/libmisterplex/ddr_frame_layout.hpp"
_pair_layout_const() {
  # $1 = constexpr identifier -> decimal value (hex literal in the header)
  local v
  v="$(grep -oE "${1}[[:space:]]*=[[:space:]]*0x[0-9A-Fa-f_]+" "$_PAIR_LAYOUT_HPP" \
       | head -n1 | grep -oE '0x[0-9A-Fa-f_]+' | tr -d '_')"
  [ -n "$v" ] || { echo "pair_ship_policy: missing $1 in $_PAIR_LAYOUT_HPP" >&2; return 3; }
  printf '%d' "$((v))"
}
_PAIR_DDR_BASE=$(_pair_layout_const kDdrFramePhysBase) || return 3 2>/dev/null || exit 3
_PAIR_SPI_STRIDE=$(_pair_layout_const kPlex240pSpiRgb565BankStride) || return 3 2>/dev/null || exit 3
_PAIR_DDR_STRIDE=$(_pair_layout_const kPlex480pYuv420pBankStride) || return 3 2>/dev/null || exit 3
PAIR_BANK1_SPI=$(printf '0x%08x' $(( _PAIR_DDR_BASE + _PAIR_SPI_STRIDE )))
PAIR_BANK1_DDR=$(printf '0x%08x' $(( _PAIR_DDR_BASE + _PAIR_DDR_STRIDE )))
...
PAIR_ID_DDR_STAMPED=ddr-c5382bee-509b0c75
PAIR_ID_DDR_HIST=ddr-c5382bee-e9f79de2
PAIR_DEFAULT_ROLLBACK="${PAIR_DEFAULT_ROLLBACK:-$PAIR_ID_SPI_HYBRID}"
PAIR_DEFAULT_PROMOTE="${PAIR_DEFAULT_PROMOTE:-$PAIR_ID_DDR_STAMPED}"
