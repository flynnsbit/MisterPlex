...
python3 tools/derived_h264_slice_fixture.py generate \
  --input local_artifacts/derived_validation/derived_realcontent_624x480_baseline_ref1_nob_1800f.264 \
...
asset exists in `local_artifacts/derived_validation/` or `build/arm-profile-sample/`,
it also verifies the full 1800-frame manifest; otherwise the full check is
reported as optional info, not as a pass.

```bash
tests/unit/test_derived_validation_hashes.sh
```

If the full asset has been cleaned from both `local_artifacts/` and `build/`,
regenerate it using `docs/derived-validation-assets.md`.
