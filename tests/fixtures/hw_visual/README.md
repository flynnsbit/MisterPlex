...
- `plex_real_baseline_320x240_57674f2e_mjpeg720_golden.png` — **retired rollback
  record**, captured from known-good rollback `57674f2e4c11551898275e99bd4c3067`
  ...
  hardware gate used to compare only ROI `11,0,160,120` because pixels outside the stable
  decoded top-left region can vary after core reloads on this rollback RBF.
  This file is valid only for that rollback md5 and is no longer a default gate
  for the current YUV420 pipeline. Its `*_visual_golden_v1.json` sidecar records
  the source RBF md5; consumers must refuse if the loaded `/media/fat/_Utility/Plex.rbf`
  md5 differs.
...
- `plex_visual_640x480_golden.png` — decoded frame presented as the product path should
  ...
  not used by the default gate. Its sidecar marks it as a software reference, not
  a hardware capture; scheduled hardware grading must still declare the loaded RBF md5.
