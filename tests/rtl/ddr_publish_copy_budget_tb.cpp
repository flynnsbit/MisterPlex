...
int main(int argc, char** argv) {
	...
	CHECK(top.fab_m10k == 2, "PREREG fabric bounce M10K=2 (64b→2×M10K, not bits/10240)");
	...
	CHECK(top.t_fab_vs_present == 5760u, "PREREG T_fabric_vs_present 5.760 ms");
	...
	CHECK(top.fab_cont_beats_arm, "fabric contended PRE-REG beats ARM");
	CHECK(top.fab_fits_24, "fabric ideal fits 24fps budget");
	CHECK(top.fab_cont_fits_24, "fabric contended PRE-REG fits 24fps");
	CHECK(top.pl330_fits_24, "PL330 EST fits 24fps budget");
	// At PREREG pins PL330 EST is slower than contended fabric (9216 > 5760)
	CHECK(!top.pl330_beats_fab_cont, "PL330 EST does not beat fabric contended PRE-REG");
	...
	std::printf(
		...
		"t_pl330=9.216ms EST t_fab_rw=3.840ms t_fab_vs_present=5.760ms PREREG; "
		"fabric contends present; PL330 does not; device BW unverified; "
		"VERDICT_PREREG=fabric_contended_beats_pl330_est_on_time_pl330_wins_m10k0\n");
	return 0;
}
