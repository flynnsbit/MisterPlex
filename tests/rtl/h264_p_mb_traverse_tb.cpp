...
int main(int argc, char** argv) {
	...
	if (std::strcmp(mode, "fault_bad_skip") == 0) {
		...
		std::printf("FAIL mut_bad_skip: RED as required mb_count=%d err=%d (intentional)\n",
		            r.mb_count, r.error);
		return 1;
	}
	...
	if (std::strcmp(mode, "fault_drop_last") == 0) {
		...
		std::printf("FAIL mut_drop_last: RED under-count mb_count=%d (intentional)\n",
		            r.mb_count);
		return 1;
	}
