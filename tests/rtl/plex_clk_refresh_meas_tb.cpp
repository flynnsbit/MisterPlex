...
// POS_240:      H1650×V750 + period→240 + CE/DE/lines OK → PASS product
// NEG_16:       period→~162 FAIL trap
// NEG_SHARED30: period→242 FAIL shared-PLL 30 MHz trap (24.242 Hz)
...
enum Expect { EXP_PASS_240, EXP_FAIL_16, EXP_SHARED30, EXP_FAIL_RASTER };
...
static int run_case(VerilatedContext* ctx, const char* name, int period_sys,
	...
	const int shared30_band = (fps_x10 >= 242 && fps_x10 <= 244);
	...
	std::printf("%s: cycles=%d frames=%d fps_x10=%d flags=0x%02x "
	            ...
	            "pass=%d shared30=%d trap=%d pix_ok=%d fps_ok=%d ce_ok=%d de_ok=%d raster_ok=%d\n",
	            name, cycles, frames, fps_x10, flags,
	            top->meas_ce_frame, top->meas_de_frame, top->meas_lines,
	            top->meas_active, top->meas_ce_line,
	            pass_band, shared30_band, trap_band, pix_ok, fps_ok, ce_ok, de_ok, raster_ok);
	...
	} else if (exp == EXP_SHARED30) {
		ok = valid && shared30_band && !pass_band && !fps_ok;
		std::printf("%s %s: expected FAIL_SHARED30_TRAP (24.242 not product)\n",
		            ok?"PASS":"FAIL", name);
	} else {
		ok = valid && !raster_ok;
...
int main(int argc, char** argv) {
	...
	{
		VerilatedContext ctx; ctx.commandArgs(argc, argv);
		rc |= run_case(&ctx, "NEG_SHARED30", P_242, EXP_SHARED30, good, 30.0e6 / 20.0e6);
	}
	{
		VerilatedContext ctx; ctx.commandArgs(argc, argv);
