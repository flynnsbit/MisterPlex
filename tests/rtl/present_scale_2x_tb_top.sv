...
module present_scale_2x_tb (
	...
	present_scale_2x dut (
		.clk(clk), .reset(reset), .ce_pix(ce_pix),
		.hc(hc), .py(py), .in_content(in_content),
		.store_x(store_x), .store_y(store_y),
		.store_x1(store_x1), .store_y1(store_y1),
		.wx0(wx0), .wx1(wx1), .wy0(wy0), .wy1(wy1), .de_r(de_r)
	);
endmodule
