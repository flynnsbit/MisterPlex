...
int main(int argc, char** argv) {
    ...
    try {
        ...
        s.d.scan_hold = 1;
        s.push({0, 0, 1, 0x65});
        for (unsigned i = 4; i < capacity; ++i) s.push({payloadByte(i)});
        s.wait(4);
        check(s.d.fifo_full && s.d.fifo_level == (capacity > 65535 ? 65535 : capacity) &&
              s.d.bytes_seen == 0 && !s.d.done,
              "whole-AU staging overflowed or full occupancy wrapped to zero");
        s.d.scan_hold = 0;
        s.wait(capacity * 4);
        check(s.d.length == capacity - 4 && s.d.bytes_seen == capacity &&
        ...
        std::cout << "PASS AU frontend: held next-NAL byte, owned RBSP, explicit EOF, "
                     ...
                     "and explicit capacity overflow; bytes=" << capacity << "\n";
        return 0;
