...
module stream_path_full_frame_tb #(
	...
	assign trace_slice_qp = dut.gen_decode_stub.stub.lat_qp;
	assign trace_residual_tc = dut.gen_decode_stub.stub.lat_res_tc;
	assign trace_residual_t1 = dut.sl_place_t1;
	assign trace_residual_dc = dut.gen_decode_stub.stub.lat_res_dc;
	assign trace_residual_csum = residual_csum;
	assign native_inter_valid = dut.gen_decode_stub.stub.inter_capture_valid;
	assign native_inter_frame_idx = dut.gen_decode_stub.stub.frames_out;
	assign native_inter_mb_x = dut.gen_decode_stub.stub.lat_p_mb_x;
	assign native_inter_mb_y = dut.gen_decode_stub.stub.lat_p_mb_y;
	assign native_inter_p_skip = dut.gen_decode_stub.stub.lat_p_skip;
	assign native_inter_part_mode = dut.gen_decode_stub.stub.lat_p_part_mode;
	genvar trace_i;
	generate
		for (trace_i = 0; trace_i < 16; trace_i = trace_i + 1) begin : gen_trace
			assign trace_residual_coeff[trace_i] =
				(FAULT_TRACE_COEFF0_PLUS1 && trace_i == 0) ?
					(dut.gen_decode_stub.stub.lat_coeff[trace_i] + 9'sd1) : dut.gen_decode_stub.stub.lat_coeff[trace_i];
			assign trace_idct_dequant[trace_i] = dut.gen_decode_stub.stub.idct_dequant[trace_i];
			assign trace_idct_residual[trace_i] = dut.gen_decode_stub.stub.idct_residual[trace_i];
			assign trace_recon_px[trace_i] = dut.gen_decode_stub.stub.recon_px[trace_i];
		end
	endgenerate
	genvar native_y_i;
	generate
		for (native_y_i = 0; native_y_i < 256; native_y_i = native_y_i + 1) begin : gen_native_y
			assign native_inter_pred_y[native_y_i] = dut.gen_decode_stub.stub.dpb_pred_y[native_y_i];
		end
	endgenerate
	genvar native_c_i;
	generate
		for (native_c_i = 0; native_c_i < 64; native_c_i = native_c_i + 1) begin : gen_native_c
			assign native_inter_pred_u[native_c_i] = dut.gen_decode_stub.stub.dpb_pred_u[native_c_i];
			assign native_inter_pred_v[native_c_i] = dut.gen_decode_stub.stub.dpb_pred_v[native_c_i];
		end
	endgenerate
	...
	wire [31:0] dpb_wr_addr_raw = dut.gen_decode_stub.stub.dpb_mem_waddr;
	wire [31:0] dpb_cur_base    = dut.gen_decode_stub.stub.dpb_current_base;
	assign native_i420_wr_en     = dut.gen_decode_stub.stub.dpb_mem_we;
	assign native_i420_wr_offset = dpb_wr_addr_raw - dpb_cur_base;
	assign native_i420_wr_data   = dut.gen_decode_stub.stub.dpb_mem_wdata;
	assign native_i420_wr_frame  = dut.gen_decode_stub.stub.frames_out;
	...
	always @(posedge clk) begin
		if (dpb_prefill_en)
			dut.gen_decode_stub.stub.dpb_mem[dpb_prefill_addr[17:0]] <= dpb_prefill_data;
	end
endmodule
