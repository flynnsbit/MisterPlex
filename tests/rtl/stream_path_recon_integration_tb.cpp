...
int main(int argc, char** argv) {
    ...
    if (argc != 4) {
        std::cerr << "usage: " << argv[0] << " normal|latency-red|escape-red bitstream.264 mb_golden.json\n";
        return 2;
    }
    ...
    try {
        ...
        const int predOnly = std::strtol(parseStringAfter(golden, "first_pred_only_signature8_hex").c_str(), nullptr, 16);
        if (mode == "normal") {
            if (!r.reconValid) {
        ...
        if (mode == "latency-red") {
            if (!r.reconValid) {
                std::cerr << "FAIL latency red-check setup: recon_valid never asserted\n";
                return 1;
            }
            if (trueRecon != 0x3b || predOnly != 0x00) {
                throw std::runtime_error("shared golden latency signatures changed unexpectedly");
            }
            if (r.residualCsum != 0x14) {
                std::cerr << "FAIL latency red-check setup: residual_csum got 0x" << std::hex
                          << r.residualCsum << " want 0x14" << std::dec << "\n";
                return 1;
            }
            if (r.reconSig != predOnly || r.reconSig == trueRecon) {
                std::cerr << "FAIL latency red-check: expected integrated stale-handoff signature 0x"
                          << std::hex << predOnly << " with true golden 0x" << trueRecon
                          << ", got 0x" << r.reconSig << std::dec << "\n";
                return 1;
            }
            std::cout << "OK latency red-check: >=2-NAL stream exposes parser-to-decode handoff "
                      << "residual_csum=0x14 but recon_sig=0x00, rejecting true golden 0x3b\n";
            return 0;
        }
        if (mode == "escape-red") {
