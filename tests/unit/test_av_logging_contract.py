...
def main() -> int:
    ...
    require(
        re.search(
            r'"media: A/V resync drop wall_s=".*?'
            r'" drift_ms=" \+ std::to_string\(avDriftMs_\.load\(\)\)',
            MEDIA,
            re.S,
        )
        is not None,
    ...
    require(
        '"media: audio latency ms=" + std::to_string(latMs) +' in MEDIA
        and '" queued_B=" + std::to_string(queuedEma)' in MEDIA,
    ...
    require(
        '"media: A/V audio_active=" +' in MEDIA
        and '" waited_ms=" + std::to_string(waited)' in MEDIA,
        "startup log must show whether the audible clock became active and how long it waited",
    )
    require(
        'vfReq.fps_filter = "fps=" + std::to_string(fpsNum_) + "/" +' in MEDIA
        and "std::to_string(fpsDen_)" in MEDIA,
    ...
    require(
        ...
        and "PLXD timeout waiting for released bank" in FPGA
