...
// PLXD at doorbell+0x128 (480p: 0x300FF128; L4: 0x3047F128). Residue at the legacy address desyncs
// free_bank_mask / bank targeting.
//
// Cases:
//   A) product 480p doorbell → PLXD 0x300FF128; L4 → 0x3047F128 (not 0x3007F128)
...
int main() {
    ...
    // Default product = 480p dual-header (doorbell 0x300FF000).
    const DdrFrameLayout product = productDdrFrameStoreLayout();
    expect(ddrFrameLayoutMatchesProductSilicon(product),
           "product layout matches silicon contract");
    expect(product.doorbell_phys == kPlex480pYuv420pDoorbellPhys,
           "product doorbell is 0x300FF000");
    expect(product.doorbell_phys == 0x300FF000u, "doorbell literal 0x300FF000");
    ...
    expect(plxd == 0x300FF128u, "product PLXD phys == 0x300FF128");
    expect(plxd != 0x3007F128u, "product PLXD must NOT be legacy 0x3007F128");
    expect(plxf == 0x300FF118u, "product PLXF phys == 0x300FF118");
    expect(plxs == 0x300FF100u, "product PLXS phys == 0x300FF100");
    expect(plxd == product.doorbell_phys + mailbox_abi::kPlxdOffset,
           "PLXD == doorbell + kPlxdOffset");

    // Explicit L4 720p: doorbell 0x3047F000 → PLXD 0x3047F128.
    const DdrFrameLayout l4 = plex720pDdrFrameStoreLayout();
    expect(ddrFrameLayoutMatchesL4Silicon(l4), "L4 layout matches 720p silicon");
    expect(l4.doorbell_phys == 0x3047F000u, "L4 doorbell 0x3047F000");
    expect(bankReleaseMailboxPhys(l4.doorbell_phys) == 0x3047F128u, "L4 PLXD 0x3047F128");
    ...
    expect(product.u_offset == static_cast<uint32_t>(kPlex480pUPlaneOffset),
           "U plane offset 299520 product 480p");
    expect(product.u_offset == 299520u, "U offset literal 480p");
    expect(l4.u_offset == 921600u, "L4 U offset 921600");
    ...
    std::cout << "PASS test_ddr_bank_mailbox_phys product_PLXD=0x300FF128 "
