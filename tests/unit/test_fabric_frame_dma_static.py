...
def main() -> int:
    ...
    if "PRODUCT_NO_STUB" not in macros:
        ...
        return 1
    if "fabric_frame_dma_en" not in CFG.read_text(encoding="utf-8", errors="replace"):
        print("FAIL: cfg stamp missing fabric_frame_dma_en", file=sys.stderr)
        return 1

    # Preferred path documents direct reader vs mover
    if "source→bank" not in mux and "source->bank" not in mux and "source" not in mux.lower():
        # require contrast comment present
        if "source" not in mux:
