...
for f in "$RTL" "$CAVLC_RTL" "$IQ_IDCT_RTL" "$INTRA_PRED_RTL" "$NB_CTX_RTL" "$DECODE_TOP_RTL" "$DPB_RTL" "$INTER_RTL" "$INTER_PART_RTL" "$PSKIP_RTL" "$MB_ROUTE_RTL" "$INTRA16_RTL" "$QIP" "$TOP" "$TB"; do
  if [[ ! -f "$f" ]]; then
