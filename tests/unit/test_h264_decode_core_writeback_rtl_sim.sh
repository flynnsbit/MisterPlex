...
for f in "$RTL" "$CAVLC_RTL" "$IQ_IDCT_RTL" "$INTRA_PRED_RTL" "$DECODE_TOP_RTL" "$DPB_RTL" "$INTER_RTL" "$QIP" "$TOP" "$TB"; do
  if [[ ! -f "$f" ]]; then
...
"$RUN_VERILATOR" --cc --exe --build \
  ...
  -CFLAGS "-std=c++17 -O2" \
  "$TOP" "$CAVLC_RTL" "$IQ_IDCT_RTL" "$INTRA_PRED_RTL" "$DECODE_TOP_RTL" "$INTER_RTL" "$DPB_RTL" "$RTL" "$TB"
