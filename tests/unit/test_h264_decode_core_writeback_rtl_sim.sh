...
INTER_NB_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_inter_nb_ctx.sv"
...
for f in "$RTL" "$CAVLC_RTL" "$IQ_IDCT_RTL" "$DPB_RTL" "$INTER_RTL" "$INTER_NB_RTL" "$QIP" "$TOP" "$TB"; do
  if [[ ! -f "$f" ]]; then
...
"$RUN_VERILATOR" --cc --exe --build \
  ...
  -CFLAGS "-std=c++17 -O2" \
  "$TOP" "$CAVLC_RTL" "$IQ_IDCT_RTL" "$INTER_RTL" "$INTER_NB_RTL" "$DPB_RTL" "$RTL" "$TB"
