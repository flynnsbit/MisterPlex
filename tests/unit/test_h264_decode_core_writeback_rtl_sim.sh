...
IQ_IDCT_SEQ_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_iq_idct_seq.sv"
INTRA_PRED_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_intra_pred.sv"
NB_CTX_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_intra_nb_ctx.sv"
DECODE_TOP_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_decode_top.sv"
DPB_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_dpb.sv"
INTER_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_inter_pred.sv"
INTER_PART_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_inter_part.sv"
PSKIP_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_pskip_mv.sv"
MB_ROUTE_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_mb_recon_route.sv"
INTRA16_RTL="$ROOT/fpga/Plex_MiSTer/rtl/h264_intra16_dc.sv"
...
for f in "$RTL" "$CAVLC_RTL" "$IQ_IDCT_RTL" "$IQ_IDCT_SEQ_RTL" "$INTRA_PRED_RTL" "$NB_CTX_RTL" "$DECODE_TOP_RTL" "$DPB_RTL" "$INTER_RTL" "$INTER_PART_RTL" "$PSKIP_RTL" "$MB_ROUTE_RTL" "$INTRA16_RTL" "$QIP" "$TOP" "$TB"; do
  if [[ ! -f "$f" ]]; then
...
"$RUN_VERILATOR" --cc --exe --build \
  ...
  -CFLAGS "-std=c++17 -O2" \
  "$TOP" "$CAVLC_RTL" "$IQ_IDCT_RTL" "$IQ_IDCT_SEQ_RTL" "$INTRA_PRED_RTL" "$NB_CTX_RTL" "$DECODE_TOP_RTL" "$INTER_RTL" "$INTER_PART_RTL" "$PSKIP_RTL" "$MB_ROUTE_RTL" "$INTRA16_RTL" "$DPB_RTL" "$RTL" "$TB"
set +e
