#!/usr/bin/env bash
# Executes the multiplier-free transform / dequant RTL under Verilator and
# compares it against the multiply-and-shift expressions it replaced.
set -euo pipefail
...
if [[ "$VERILATOR_RC" -eq 127 ]]; then
  cat >&2 <<SKIP
SKIP RTL SIM: Verilator not found; transform/dequant real RTL simulation was NOT run.
