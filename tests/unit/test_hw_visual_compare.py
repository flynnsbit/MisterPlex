...
def main() -> int:
    ...
    stale_status = WORK / "status_stale_bytes4.txt"
    stale_status.write_text(
        "status has_frame=1 has_audio=0 has_stream=1 underrun=0 "
        "has_idr=1 stub_busy=0 sps_valid=1 pps_valid=1 nalu=4 last_nal=0x05 "
        "slice_type=2 mb0=0 qp=25 res_ok=1 res_tc=8 res_t1=3 res_dc=-24 "
        "res_csum=20 recon_sig=59 recon_dbg=0x3b ddr_busy=0 sps=320x240 bytes_in=4\n",
        encoding="utf-8",
    )
    stale_delivery = run(
        "compare",
    ...
    print("PASS delivery freshness gate rejects bytes_in=4 phantom-green status")
    ...
    print("PASS fresh delivery counters and shared {bank,format,seq} token allow grading")
