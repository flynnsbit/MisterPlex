...
for f in "$RTL_IQ" "$RTL_HYBRID" "$RTL_DECODE" "$RTL_DEBLOCK_MB" "$RTL_REF_COMMIT" "$RTL_INTER" "$RTL_DEBLOCK" "$RTL_DPB" "$QIP" "$TB_IQ" "$TOP_IQ" "$TB_DECODE" "$TOP_DECODE" "$FIXTURE"; do
  if [[ ! -f "$f" ]]; then
...
"$RUN_VERILATOR" --cc --exe --build \
  ...
  -CFLAGS "-std=c++17 -O2" \
  "$TOP_DECODE" "$RTL_IQ" "$RTL_INTER" "$RTL_DEBLOCK" "$RTL_DEBLOCK_MB" "$RTL_DPB" "$RTL_REF_COMMIT" "$RTL_HYBRID" "$RTL_DECODE" "$TB_DECODE"
