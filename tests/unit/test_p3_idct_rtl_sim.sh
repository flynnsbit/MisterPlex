...
for f in "$RTL_IQ" "$RTL_DECODE" "$RTL_HYBRID" "$RTL_RECON_STORE" "$RTL_INTER" "$RTL_DEBLOCK" "$RTL_DPB" "$QIP" "$TB_IQ" "$TOP_IQ" "$TB_DECODE" "$TOP_DECODE" "$FIXTURE"; do
  if [[ ! -f "$f" ]]; then
    echo "RTL SIM ERROR: missing required file: $f" >&2
    exit 2
  fi
done
if ! grep -q 'rtl/h264_iq_idct_4x4.sv' "$QIP" || ! grep -q 'rtl/decode_stub.sv' "$QIP" || ! grep -q 'rtl/h264_hybrid_mb_own.sv' "$QIP" || ! grep -q 'rtl/h264_inter_pred.sv' "$QIP" || ! grep -q 'rtl/h264_dpb.sv' "$QIP"; then
...
"$RUN_VERILATOR" --cc --exe --build \
  ...
  -CFLAGS "-std=c++17 -O2" \
  "$TOP_DECODE" "$RTL_IQ" "$RTL_INTER" "$RTL_DEBLOCK" "$RTL_DPB" "$RTL_DECODE" "$RTL_HYBRID" "$RTL_I16_DC" "$RTL_I16_DCS" "$RTL_DQS" "$RTL_BYTE_RAM" "$RTL_I_SINK" "$RTL_INTRA" "$RTL_RECON_STORE" "$TB_DECODE"
