...
class DetailedTiming(unittest.TestCase):
    def setUp(self):
        self.work = ROOT / "build" / ("timing-policy-" + uuid.uuid4().hex)
    ...
    def extend_bundle(self, overrides=None, decoder_target=False):
        ...
        static = {
            "timing-nodes": "type\tname\tdecoder\nreg\temu|spath|mb_ctrl|counter[0]\t1\n"
                            "port\tout_led\t0\nreg\tconfig\t0\n",
            "sdc-sources": "file\nsys_top.sdc\n",
            "exception-commands": "id\tfile\tline\tkind\tcode\tcommand\n"
                                  f"0\tsys_top.sdc\t1\tset_false_path\t0\tset_false_path -to {{{target}}}\n"
                                  "1\tsys_top.sdc\t2\tset_clock_groups\t0\t"
                                  "set_clock_groups -exclusive -group sys_collection -group ddr_collection\n",
        ...
        (self.timing / "extended.summary").write_text(
            f"schema=misterplex.timing-extended.v1\nscoped_reports={len(scoped_index)-1}\n"
        ...
        provenance["schema"] = "misterplex.timing-reporter.v2"
    ...
    def test_effective_sources_must_be_supplied_and_fully_observed(self):
        ...
        self.assertIn("SDC execution coverage mismatch", stderr)
