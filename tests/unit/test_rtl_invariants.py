...
def check_mailboxes() -> None:
    ...
    check(
        sv_expr_uses_doorbell_offset(sv_param_expr(ddr_fs, "IDENTITY_MAILBOX_PHYS"), 0x130),
        "ddr_frame_store PLXC identity mailbox must derive from DOORBELL_PHYS + 0x130",
    )
    ...
    # --- PLXC core identity (doorbell-relative build stamp) ---
    plxc_off = cpp_const(spec_text, "kPlxcOffset")
    check(plxc_off == 0x130, f"kPlxcOffset must be 0x130 (got 0x{plxc_off:X})")
    check(cpp_const(spec_text, "kPlxcMagic") == 0x504C5843,
          "PLXC magic must be 0x504C5843 'PLXC'")
    check(cpp_const(spec_text, "kPlxcAddr") == 0x3007F130,
          "legacy example PLXC address must be 0x3007F130")
    core_id_raw = read(ROOT / "host" / "libmisterplex" / "core_identity.hpp")
    check("kCoreIdentityMailboxPhys" in core_id_raw and "kPlxcAddr" in core_id_raw,
          "core_identity.hpp must define kCoreIdentityMailboxPhys from kPlxcAddr")
    product_plxc = product_doorbell + plxc_off
    check(product_plxc == 0x300FF130,
          f"product PLXC must be 0x300FF130 (got 0x{product_plxc:08X})")
    check(product_plxc != 0x3007F130,
          "product PLXC must not collapse to legacy absolute 0x3007F130")
    check("coreIdentityMailboxPhys(ddrLayout_.doorbell_phys)" in spi,
          "fpga_spi readCoreIdentity must use coreIdentityMailboxPhys(doorbell)")
    check("kCoreIdentityMailboxPhys - ddrLayout_.phys_base" not in spi,
          "fpga_spi must not map PLXC via legacy absolute kCoreIdentityMailboxPhys")
    check("MAGIC_C" in ddr_fs, "ddr_frame_store must define MAGIC_C for PLXC")
    check("IDENTITY_MAILBOX_W" in ddr_fs or "IDENTITY_MAILBOX_PHYS" in ddr_fs,
          "ddr_frame_store must address IDENTITY mailbox")
    check("core_identity_stamp.svh" in read(DDR_FRAME_STORE),
          "ddr_frame_store must include generated core_identity_stamp.svh")
    stamp_svh = ROOT / "fpga" / "Plex_MiSTer" / "rtl" / "core_identity_stamp.svh"
    check(stamp_svh.is_file(), "core_identity_stamp.svh must exist (run gen_core_identity_stamp.py)")
    stamp_txt = stamp_svh.read_text()
    check("GENERATED by scripts/gen_core_identity_stamp.py" in stamp_txt,
          "core_identity_stamp.svh must be generator output, not hand-edited placeholder")
    check("SOURCE_GIT=" in stamp_txt, "stamp must record SOURCE_GIT")
    check("PLACEHOLDER" not in stamp_txt.upper() or "SOURCE_GIT=" in stamp_txt,
          "stamp must not be a PLACEHOLDER")
