...
for f in "$QIP" "$RTL_STREAM" "$RTL_INGEST" "$RTL_DDR" "$RTL_FIFO" "$RTL_SCAN" "$RTL_SPS" "$RTL_PPS" \
         "$RTL_SLICE" "$RTL_SYNTAX" "$RTL_CAVLC" "$RTL_DECODE" "$RTL_IQ" "$RTL_INTER" "$RTL_DEBLOCK" "$RTL_DPB" "$RTL_CORE" "$TOP" "$TB" "$BITSTREAM" "$REF"; do
  if [[ ! -f "$f" ]]; then
...
for rtl in rtl/stream_path.sv rtl/stream_ingest.sv rtl/ddr_bitstream_reader.sv rtl/bitstream_fifo.sv rtl/nalu_scanner.sv \
           ...
           rtl/h264_iq_idct_4x4.sv rtl/h264_inter_pred.sv rtl/h264_deblock.sv rtl/h264_dpb.sv rtl/h264_decode_core.sv; do
...
"$RUN_VERILATOR" --cc --exe --build \
  ...
  "$RTL_SLICE" "$RTL_SYNTAX" "$RTL_CAVLC" "$RTL_CORE" "$RTL_DECODE" "$RTL_IQ" "$RTL_INTER" "$RTL_DEBLOCK" "$RTL_DPB" "$TB"
