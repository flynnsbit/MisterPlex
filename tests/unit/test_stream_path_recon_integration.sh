...
for f in "$QIP" "$RTL_STREAM" "$RTL_INGEST" "$RTL_DDR" "$RTL_FIFO" "$RTL_SCAN" "$RTL_SPS" "$RTL_PPS" "$RTL_CAVLC" \
         "$RTL_SLICE" "$RTL_DECODE" "$RTL_DECODE_TOP" "$RTL_DECODE_CORE" "$RTL_INTER_PART" "$RTL_PSKIP" "$RTL_MB_ROUTE" "$RTL_INTRA16" "$RTL_IQ" "$RTL_INTER" "$RTL_INTRA" "$RTL_NB_CTX" "$RTL_DEBLOCK" "$RTL_DPB" "$TOP" "$TB" "$BITSTREAM" "$REF"; do
  if [[ ! -f "$f" ]]; then
...
for rtl in rtl/stream_path.sv rtl/stream_ingest.sv rtl/ddr_bitstream_reader.sv rtl/bitstream_fifo.sv rtl/nalu_scanner.sv \
           ...
           rtl/h264_decode_top.sv rtl/h264_decode_core.sv rtl/h264_iq_idct_4x4.sv rtl/h264_inter_pred.sv rtl/h264_inter_part.sv rtl/h264_pskip_mv.sv rtl/h264_mb_recon_route.sv rtl/h264_intra16_dc.sv rtl/h264_intra_pred.sv rtl/h264_intra_nb_ctx.sv rtl/h264_deblock.sv rtl/h264_dpb.sv; do
...
"$RUN_VERILATOR" --cc --exe --build \
  ...
  "$RTL_SLICE" "$RTL_DECODE" "$RTL_DECODE_TOP" "$RTL_DECODE_CORE" "$RTL_INTER_PART" "$RTL_PSKIP" "$RTL_MB_ROUTE" "$RTL_INTRA16" "$RTL_IQ" "$RTL_INTER" "$RTL_INTRA" "$RTL_NB_CTX" "$RTL_DEBLOCK" "$RTL_DPB" "$TB"
