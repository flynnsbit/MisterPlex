...
for f in "$QIP" "$RTL_STREAM" "$RTL_INGEST" "$RTL_DDR" "$RTL_FIFO" "$RTL_SCAN" "$RTL_RBSPWIN" "$RTL_SPS" "$RTL_PPS" "$RTL_CAVLC" \
...
for rtl in rtl/stream_path.sv rtl/stream_ingest.sv rtl/ddr_bitstream_reader.sv rtl/bitstream_fifo.sv rtl/nalu_scanner.sv rtl/h264_rbsp_window.sv \
...
"$RUN_VERILATOR" --cc --exe --build \
  ...
  -CFLAGS "-std=c++17 -O2" \
  "$TOP" "$RTL_STREAM" "$RTL_INGEST" "$RTL_DDR" "$RTL_FIFO" "$RTL_SCAN" "$RTL_RBSPWIN" "$RTL_SPS" "$RTL_PPS" "$RTL_CAVLC" \
