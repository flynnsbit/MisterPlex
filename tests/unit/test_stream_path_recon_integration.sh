...
RTL_EXPORT="$ROOT/fpga/Plex_MiSTer/rtl/h264_recon_export.sv"
...
for f in "$QIP" "$RTL_STREAM" "$RTL_INGEST" "$RTL_DDR" "$RTL_FIFO" "$RTL_SCAN" "$RTL_SPS" "$RTL_PPS" \
         "$RTL_SLICE" "$RTL_HYBRID" "$RTL_DECODE" "$RTL_EXPORT" "$RTL_TRAV" "$RTL_CAVLC" "$RTL_DEBLOCK_MB" "$RTL_REF_COMMIT" "$RTL_IQ" "$RTL_INTER" "$RTL_DEBLOCK" "$RTL_DPB" "$TOP" "$TB" "$BITSTREAM" "$REF"; do
  if [[ ! -f "$f" ]]; then
...
for rtl in rtl/stream_path.sv rtl/stream_ingest.sv rtl/ddr_bitstream_reader.sv rtl/bitstream_fifo.sv rtl/nalu_scanner.sv \
           ...
           rtl/decode_stub.sv rtl/h264_recon_export.sv rtl/h264_p_mb_traverse.sv rtl/h264_cavlc_residual.sv rtl/h264_deblock_mb.sv rtl/h264_dpb_ref_commit.sv \
...
"$RUN_VERILATOR" --cc --exe --build \
  ...
  "$RTL_SLICE" "$RTL_HYBRID" "$RTL_DECODE" "$RTL_EXPORT" "$RTL_TRAV" "$RTL_CAVLC" "$RTL_DEBLOCK_MB" "$RTL_REF_COMMIT" "$RTL_IQ" "$RTL_INTER" "$RTL_DEBLOCK" "$RTL_DPB" "$TB"
