...
def separation_score(rows, x0: int, y0: int, mask) -> float:
    ...
    if sep < 0:
        return 0.0
    # Parent-proven defect: blank bright panel scored ~0.58 with recovered=PAUSED
    # because score = 0.55*sep + 0.45*bright — on a flat field sep≈0 and bright
    # alone crossed the 0.40 accept threshold. Require real ink/background
    # separation before brightness can contribute (no glyphs ⇒ no PASS).
    if sep < 0.08:
        return 0.0
...
def find_string(rows, text: str, *, coarse_y: int = 2, left_label_pass: bool = True):
    ...
    # Accept only when score clears floor AND font resolved from ink span —
    # template score alone invented PAUSED on blank bright panels (parent).
    font_ok = font_m in ("8x13", "12x16") and scale_m is not None
    recovered = text if (score >= 0.40 and font_ok) else ""
    if score >= 0.40 and not font_ok:
        meta["refuse"] = "score_without_resolved_font"
