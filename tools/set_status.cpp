...
void printRaw(const uint8_t raw[16]) {
    ...
    // PRESENT_CLK_PIX_PLL refresh measure (packed into raw[14]/[15] when PLL on)
    // flags: {0,de_ok,ce_ok,trap16,pll_on,fps_ok,pix_ok,valid}
    // Product 30 MHz H1650 → 24.242 Hz → fps_x10≈242; exact-24 → 240; trap → ~162
    {
        ...
        std::printf(
            ...
            ce_ok, de_ok);
        if (fps_x10 >= 241 && fps_x10 <= 244 && valid && fps_ok && pix_ok && !trap16)
