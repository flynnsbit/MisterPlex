...
void printRaw(const uint8_t raw[16]) {
    ...
    // PRODUCT_NO_STUB + clk_pix live: raw[14]=meas_fps_x10, raw[15]=meas_flags
    ...
    std::printf(
        ...
        "valid=%d pix_ok=%d fps_ok=%d pll_on=%d\n",
        fps_x10, fps_x10 / 10.0, fl, valid, pix_ok, fps_ok, pll_on);
    if (fps_x10 >= 230 && fps_x10 <= 250 && valid && fps_ok)
