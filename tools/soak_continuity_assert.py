...
Identity (source):
  ...
  session_epoch  — process_epoch.stream_seq (changes on new stream; use --require-single-session-epoch for counter soaks)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "FAIL if session_epoch is missing or not exactly one distinct value "
            "(per-stream counters reset; soak must be one session_epoch — parent P4)"
        ),
    )
    ...
    if args.require_single_session_epoch and rc in (RC_OK, RC_NO_DATA):
        se = info.get("unique_session_epochs") or []
        if not se:
            rc = RC_NO_DATA
            msg = (
                "CONTINUITY_NO_DATA: --require-single-session-epoch set but no "
                "session_epoch= samples (cannot prove single-session soak)"
            )
        elif len(se) != 1:
            rc = RC_FAIL
            msg = (
                "CONTINUITY_FAIL: session_epoch not single-valued: "
                + str(se)
                + " (per-stream counters reset; soak spans multiple sessions)"
            )
